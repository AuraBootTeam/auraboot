...
# AuraBoot Environment Reset and Initialization Script
# This script performs a complete environment reset:
# 1-2. Stop backend + frontend services
# 3.   Reset database (drop + recreate)
# 4.   Start backend + wait for health check
# 4.5  Bootstrap system via API (admin user + tenant)
# 5.   Start frontend + wait for ready
# 6.   Seed test pages, dashboard, multi-role users (pure API, no Playwright)
...
PLUGINS_TO_IMPORT=(core-meta core-bpm core-aurabot page-manager platform-admin org-management e2e-test-order crm-starter showcase agent-control-plane acp-showcase)
