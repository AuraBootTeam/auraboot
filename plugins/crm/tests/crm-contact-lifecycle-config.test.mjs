...
const fields = new Map(
  readJson("config/fields/crm_contact_common.json").map((field) => [
    field.code,
    field,
  ]),
);
const bindings = new Map(
  readJson("config/bindings/crm_contact_common.json").map((binding) => [
    binding.fieldCode,
    binding,
  ]),
);
const commands = new Map(
  readJson("config/commands/crm_contact_common.json").map((command) => [
    command.code,
    command,
  ]),
);
const dicts = new Map(
  readJson("config/dicts.json").map((dict) => [dict.code, dict]),
);
...
const accountDetailPage = readJson(
  "config/pages/crm_account_common_detail.json",
);
...
const activityCommands = new Map(
  readJson("config/commands/crm_activity_common.json").map((command) => [
    command.code,
    command,
  ]),
);
...
test("contacts expose an explicit active/inactive lifecycle instead of overloading deletion", () => {
  ...
  assert.equal(
    fields.get("crm_ct_primary_account_key").constraints.unique,
    true,
  );
  ...
  assert.equal(
    commands.get("crm:set_primary_contact").handler,
    "crm:set_primary_contact",
  );
  assert.equal(commands.get("crm:delete_contact").type, "custom");
  assert.equal(
    commands.get("crm:delete_contact").handler,
    "crm:delete_contact",
  );
});
...
test("contact ownership is resolvable, required and defaults to the current user without blocking reassignment", () => {
  ...
  const ownerField = identity.fields.find(
    (field) => field.field === "crm_ct_owner",
  );
...
test("contact saved views expose all, current-user and department ownership scopes", () => {
  ...
  assert.deepEqual(
    contactViews.map((view) => view.viewKey),
    [
  ...
  assert.deepEqual(
    contactViews.map((view) => view.isDefault),
    [true, false, false],
  );
  assert.deepEqual(
    contactViews.slice(1).map((view) => view.quickFilterOrder),
    [1, 2],
  );
  ...
  const manager = readJson("config/roles.json").find(
    (role) => role.code === "crm_sales_manager",
  );
  assert.equal(
...
test("contact list exposes ownership, discoverable search and explicit empty-state recovery", () => {
  ...
  assert.equal(
    table.columns.some((column) => column.field === "crm_ct_owner"),
    true,
  );
...
test("contact pages expose a business-focused mobile list and detail journey", () => {
  const listUx = listPage.mobileUx.list;
  assert.deepEqual(
    listUx.views.map((view) => view.id),
    ["all", "active", "primary", "inactive"],
  );
  assert.deepEqual(listUx.defaultSort, [
    { field: "created_at", direction: "DESC" },
  ]);
  assert.equal(listUx.card.title.field, "crm_ct_name");
  assert.equal(
    listUx.card.subtitle.template,
    "{{crm_ct_title}} · {{crm_ct_email}}",
  );
  assert.deepEqual(
    listUx.card.badges.map((slot) => slot.field),
    ["crm_ct_status"],
  );
  assert.deepEqual(
    listUx.card.facts.map((slot) => slot.field),
    ["crm_ct_mobile", "crm_ct_phone"],
  );
  assert.deepEqual(listUx.card.actions, ["favorite", "more"]);

  const detailUx = detailPage.mobileUx.detail;
  assert.equal(detailUx.header.title.field, "crm_ct_name");
  assert.deepEqual(
    detailUx.tabs.map((tab) => tab.id),
    ["overview", "activity"],
  );
  assert.deepEqual(
    detailUx.sections.map((section) => section.id),
    ["relationship", "channels", "notes"],
  );
  assert.deepEqual(detailUx.bottomActions, {
    source: "capabilities",
    maxPrimary: 2,
  });

  const visibleProfile = JSON.stringify({ listUx, detailUx });
  assert.doesNotMatch(visibleProfile, /pid|uuid|json|hash|row_version/i);
});
...
test("account detail keeps the canonical editable contact related-list contract", () => {
  ...
  const related = contacts.blocks.find(
    (block) => block.id === "block_contacts",
  );
  ...
  assert.equal(
    related.subTable.columns.some((column) => column.field === "crm_ct_owner"),
    true,
  );
  assert.equal(
    related.subTable.columns.some((column) => column.field === "crm_ct_status"),
    true,
  );

  const actions = new Map(
    related.subTable.actions.map((action) => [action.code, action]),
  );
...
test("primary-contact and lifecycle actions are available from list and detail with state guards", () => {
  ...
  const listActions = new Map(
    table.columns
      ...
      .buttons.map((button) => [button.code, button]),
  );
  const toolbar = findBlock(detailPage, "crm_contact_detail_toolbar");
  const detailActions = new Map(
    toolbar.buttons.map((button) => [button.code, button]),
  );

  for (const actions of [listActions, detailActions]) {
    assert.equal(
      actions.get("set_primary").action.command,
      "crm:set_primary_contact",
    );
...
test("contact list exposes governed bulk edit, delete, export and update import", () => {
  ...
  assert.equal(
    table.table.bulkCapabilities.edit.permissionCode,
    "crm.contact.manage",
  );
  assert.equal(
    table.table.bulkCapabilities.export.permissionCode,
    "crm.contact.read",
  );
...
test("follow-up list distinguishes plans from records and detail exposes governed deletion", () => {
  const tabs = new Map(
    findBlock(activityList, "crm_act_tabs").tabs.map((tab) => [tab.key, tab]),
  );
  ...
  assert.equal(
    findBlock(activityDetail, "activity_history").subTable.parentField,
    "crm_act_parent_id",
  );
  ...
  assert.equal(
    activityCommands
      .get("crm:update_activity")
      .inputFields.includes("crm_act_type"),
    false,
  );
...
test("activity defaults to a valid table view and keeps a renderable calendar alternative", () => {
  const activityViews = savedViews.filter(
    (view) => view.modelCode === "crm_activity_common",
  );
