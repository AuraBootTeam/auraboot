...
test('CRM release manifest derives the complete RG-1 through RG-4 denominator', async () => {
  ...
  assert.equal(committed.scope.productDenominator.commands, 248);
  ...
  assert.equal(committed.scope.productDenominator['page-fields'], 1765);
  assert.equal(committed.scope.productDenominator['ui-actions'], 626);
  assert.equal(committed.scope.productDenominator.queries, 56);
  assert.equal(committed.scope.productVerdicts.pass, 739);
  assert.equal(committed.scope.productVerdicts.untested, 2754);
