...
test('CRM release manifest derives the complete RG-1 through RG-4 denominator', async () => {
  ...
  assert.equal(committed.scope.productDenominator['page-blocks'], 418);
  assert.equal(committed.scope.productDenominator['page-fields'], 1769);
  assert.equal(committed.scope.productDenominator['ui-actions'], 640);
  assert.equal(committed.scope.productDenominator.queries, 56);
  assert.equal(committed.scope.productVerdicts.pass, 736);
  assert.equal(committed.scope.productVerdicts.untested, 2781);
