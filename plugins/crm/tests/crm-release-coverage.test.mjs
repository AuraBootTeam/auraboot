...
test('CRM release manifest derives the complete RG-1 through RG-4 denominator', async () => {
  ...
  assert.equal(committed.scope.productDenominator.commands, 249);
  assert.equal(committed.scope.productDenominator.permissions, 114);
  assert.equal(committed.scope.productDenominator['page-blocks'], 415);
  assert.equal(committed.scope.productDenominator['page-fields'], 1770);
  assert.equal(committed.scope.productDenominator['ui-actions'], 633);
  assert.equal(committed.scope.productDenominator.queries, 56);
  assert.equal(committed.scope.productVerdicts.pass, 739);
  assert.equal(committed.scope.productVerdicts.untested, 2768);
