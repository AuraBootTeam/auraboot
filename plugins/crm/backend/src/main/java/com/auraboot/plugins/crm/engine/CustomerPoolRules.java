...
/** Pure, deterministic policy helpers shared by customer-pool commands and the recycle scanner. */
public final class CustomerPoolRules {

    private CustomerPoolRules() {
    }
    ...
    public static Set<String> userIds(Object csv) {
        ...
        Arrays.stream(encoded.split(","))
                .map(String::trim)
                .map(CustomerPoolRules::stripJsonQuotes)
