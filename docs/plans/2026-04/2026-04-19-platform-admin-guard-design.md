...
- `TimezoneMigrationController` — `/api/admin/timezone`
...
- 新增种子 SQL:integration test 的 `admin@example.com` 用户在默认 tenant 下绑定 `tenant_admin` 角色;`schema.sql`/`data-test.sql` 同步。
...
- E2E:`admin@example.com / Test2026x` 已是平台 admin,但需确认 `ab_user_role` 行已落库 — 否则 web-admin 所有调用 `/api/admin/*` 的页面全部 409。
...
2. **`TimezoneMigrationController`** 是运维一次性工具,是否应下线或挪到 `/api/admin/ops/**` + `platform_admin`?
