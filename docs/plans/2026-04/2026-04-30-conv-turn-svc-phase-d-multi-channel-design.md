...
|---|------|---------|---|
| Q-D.1 | 异步 → 同步 bridge | α @Async wrapper 调 sync runTurn | TBD |
| Q-D.2 | 非 SSE sink | α BroadcastResponseSink | TBD |
| Q-D.3 | sender_type 统一 | α 全部 agent + backfill | TBD |
| Q-D.4 | handoff 与 task 父子 | α 每跳建 task | TBD |
| Q-D.5 | IM TurnRequest 字段映射 | 表中映射 + 新增 InboundMode.PERSISTED_FROM_IM | TBD |
| Q-D.6 | mobile WS | α 不做 | TBD |
...
待 v2 (owner-decision-locked 版) 撰写。每 PR 模式照 C.3 系列：独立 commit + push + main fast-forward。

预估总时长：D.1 (1天) + D.2 (1.5天) + D.3 (2天) + D.4 (1天，可 defer) + D.5 (0.5天) ≈ 5-6 天工作量。
