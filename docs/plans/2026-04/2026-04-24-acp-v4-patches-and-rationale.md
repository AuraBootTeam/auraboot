...
**状态**：revised v1.3（2026-04-26 二轮 review fixes 已合入；step 0 验证项 unblock 前不能开干）
**日期**：2026-04-24（v1） / 2026-04-26（v1.2 + v1.3）
**关联文档**：
...
**初步倾向 A**，理由：
- triage 字段语义本就属于"消息接收时分类的元数据"，与 message 同生命周期
- IM 与 ACP 在 OSS 边界内本来就要打通（AuraBot 是 IM 群聊里的一个成员）
- 双消息存储违反 "single source of truth" 原则

**但 A 需要先验证 3 件事**（执行前 owner 必查）：
1. `ab_im_message.role` 字段是否已支持 `agent` 类型？还是仅 `user`/`assistant`？
2. `ab_im_message` 是否 tenant 隔离 + ACL 与 ACP 一致？
3. agent 发言事件流（SSE）当前是否落 `ab_im_message`？若否，落地点是哪？

**P1.4 决议落点**：本节列入执行清单 step 0（验证阶段）的必查项，未澄清前 step 6（A.0 + A.2）不能开干。

---

**待选项 A 通过验证后**，schemas/tables.sql 实际 ALTER（仅作示意，最终落点视 step 0 结论）：
...
| 步骤 | 阶段 | 动作 | 文件 |
|------|------|------|------|
| 0 | 验证（**blocker for step 6**） | (a) `psql \d ab_im_message` 确认 role 字段是否支持 `agent`，是否 tenant 隔离与 ACP 一致；(b) 检查 agent SSE 输出当前是否落 `ab_im_message`；(c) 若三项验证通过 → step 6 走选项 A（ALTER ab_im_message）；若失败 → owner 决定走选项 B（新建 `ab_agent_conversation` + `ab_agent_message`）或重新设计 | — |
