...
| 工具栏快捷筛选 | `web-admin/app/framework/meta/rendering/pages/list/ListToolbar.tsx` |
...
- quick filter 只服务默认视图；进入个人视图后不再高亮 preset，也不显示 saved / edited / reset preset 状态。
- 从个人视图点击 quick filter 时，清除 `view` 并进入默认视图 + `preset` 模式。
- 本地变更显示源 scope 和动作：
  - 有权限时保存到源视图
  - 复制为我的视图
  - 放弃变更
...
| --- | --- | --- | --- |
| SavedView targeted E2E | `PW_SKIP_WEBSERVER=1 PLAYWRIGHT_BASE_URL=http://127.0.0.1:5187 BACKEND_URL=http://127.0.0.1:6486 BE_PORT=6486 BFF_PORT=6187 NO_PROXY=localhost,127.0.0.1 PW_WORKERS=1 npx playwright test tests/e2e/saved-view/saved-view-management.spec.ts tests/e2e/saved-view/saved-view-quick-filters.spec.ts tests/e2e/saved-view/saved-view-follow-up-golden.spec.ts --project=chromium --no-deps` | `16 passed` | 覆盖个人视图主链路、放弃变更、隐藏列持久化、快捷筛选保存为个人视图、个人视图点击快捷筛选回默认 `preset` 模式 |
| Frontend unit/component | `pnpm test:unit:run -- app/framework/meta/rendering/pages/__tests__/ListPageContent.test.ts app/framework/meta/rendering/pages/list/__tests__/ListToolbar.i18n.test.tsx app/framework/meta/rendering/pages/list/__tests__/quickFilterPresets.test.ts app/framework/meta/rendering/pages/list/__tests__/dsl-list-i18n-resources.test.ts app/framework/smart/components/view/__tests__/ViewSelector.test.tsx app/framework/smart/hooks/__tests__/useSavedViews.test.ts app/framework/smart/utils/__tests__/savedViewPersistence.test.ts` | `476 files passed, 4875 tests passed` | 本次脚本实际跑成 web-admin 全量单测；覆盖强于原定向范围，包含 selector、默认视图/quick filter URL 边界、toolbar i18n、persistence 和 saved-view hooks |
| TypeScript | `pnpm typecheck` | PASS | React Router typegen 和 `tsc` 均通过 |
| Diff hygiene | `git diff --check` | PASS | 没有 whitespace error |
| e2e-truth audit | skip/fixme/direct-route/wait/retry/threshold grep + 实际 Playwright 输出核对 | PASS with known exception | 只声明 Personal-only scoped 完成；direct `/p/` 仅保留 `SV-PER-003b` URL 契约回归；历史 API-heavy specs 不冒充 UI 完成 |
