...
- target-scope 测试没有 skip/fixme/fixed wait/direct route/API-only fake pass。
...
7. 个人管理面板在 quota/fixture-heavy 数据下仍可读，且只验收 personal 10。
8. Feature matrix 更新，每个 delivered row 都有用户路径证据。
9. target SavedView 文件通过 e2e-truth audit。
10. 最终汇报包含精确命令、截图、已知风险，不再使用没有证据的“全部完成”口径。
