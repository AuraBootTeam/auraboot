...
- 快捷筛选只在默认视图模式下表达选中态。打开个人视图后，列表工具栏不应因为该个人视图来源于某个 preset 而高亮快捷筛选，也不显示 saved / edited / reset 这类 preset 生命周期状态。
- 用户在个人视图中点击快捷筛选时，应退出当前 `view`，切回默认视图模式并应用对应 `preset`。
- 当 URL 同时包含 `view` 和 `preset` 时，`view` 优先；列表页应移除 `preset`，避免快捷筛选和个人视图状态叠加成重复入口。
