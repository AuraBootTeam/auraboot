...
**Design**: historical 2026-04-19 L1→L2 promotion design, consolidated into this subsystem reference.
...
auraboot_memory_l1l2_leader_skipped_total{job_code, instance_id}
  # Emitted by orphan-scanner / demoter when leader-election loses; lets ops
  # see which instance is authoritative and that non-leaders correctly no-op.

auraboot_memory_l1l2_session_ended_total{tenant, outcome}
  outcome ∈ { succeeded, cancelled, failed }
  # Split by terminal state — tells apart "pipeline healthy only on success"
  # from "pipeline sees cancel/fail too".
```
...
*Addressed post Round-3*（原三项限制已随 Round-3 I1-I3 + terminal-state 覆盖消化）：
- ~~仅成功路径发事件~~ → `SessionEndedEvent` 现在在 SUCCEEDED / CANCELLED / FAILED 三条终态路径上都会触发；`session_ended_published_at` 原子 NULL→NOW() guard 保证幂等。
- ~~无 leader 可视性~~ → `auraboot_memory_l1l2_leader_skipped_total{job_code, instance_id}` 暴露非 leader 跳过次数，运维可直接判断哪台实例在干活。
- ~~orphan 扫描 ORDER BY 非确定~~ → Round-3 I1 把 scanner 的 ORDER BY 固定为 `pid ASC`，多实例观测结果稳定，并补了并发 race 集成测试。
...
| `72e6a5d2` | — | Round-3 I1/I2/I3 | Orphan scanner ORDER BY `pid ASC`（确定性）+ leader-skipped metric + 并发 race 集成测试 |
| `9f39193a` | — | Terminal-state event | `SessionEndedEvent` 在 cancel / fail 也发布；`session_ended_published_at` 原子 NULL→NOW() guard；新增 `auraboot_memory_l1l2_session_ended_total{tenant, outcome}` |

## Related docs + dashboards

- 设计与 Phase checklist：historical 2026-04-19 L1→L2 promotion design（§9.1/§9.1a/§9.2 反映已交付状态）, now consolidated into this subsystem reference.
