...
| NamedQuery DataScope 声明与执行 | `platform/src/main/java/com/auraboot/framework/meta/service/impl/NamedQueryServiceImpl.java` |
| 自定义 Controller/PF4J 授权 helper | `platform/src/main/java/com/auraboot/framework/meta/service/DataAccessAuthorizationHelper.java` |
...
| batchDelete | 使用 tenant + id set + DataScope + domain filter 的 scoped bulk；影响行数不匹配时 fail-closed。 |
| relation | 先校验源记录可见性，再对目标模型叠加 row/domain filter。 |
| export/options | 需要逐项确认是否已经统一接入 row filter，不能只验证 list。 |
| Dynamic aggregate/chart | 动态模型聚合查询叠加 row/domain filter；cache key 必须包含 data access context。 |
| NamedQuery execute/export/aggregate | 只有显式声明 `resourceCode/actionCode` 时才叠加 DataScope。 |
...
| 边界 | 一期处理 |
|---|---|
| NamedQuery `nq:*` 是独立查询引擎，不会天然继承 Dynamic row filter。 | 已按显式 `resourceCode` + `actionCode` 接入；缺声明时不得标记为受 DataScope 保护。 |
| 自定义 Controller/PF4J Handler 不会自动继承 Dynamic scope。 | 调用 `DataAccessAuthorizationHelper`，或只调用已经受控的 Dynamic 服务。 |
...
### M1.1: NamedQuery 与自定义 API 收敛

NamedQuery 和自定义 API 不能默认认为已经受 Dynamic scope 保护。本轮已完成最小收敛入口：

1. NamedQuery 持久化字段、DTO 和 plugin importer 均支持 `resourceCode` 与 `actionCode`。
2. NamedQuery execute/export 和 NamedQuery aggregate 在声明存在时调用统一 `DataPermissionEngine`。
3. PF4J 自定义 handler / 自定义 Controller 通过 `DataAccessAuthorizationHelper` 获取 list filter 或 record verdict。
4. E2E 块级矩阵仍要把 Dynamic、NamedQuery、自定义 API 分开验收，不能用 Dynamic list 通过来证明 NamedQuery 页面已安全。
...
| 缺少声明的 NamedQuery 不能被标记为已受 DataScope 保护。 | 当前允许作为边界存在；后续对受保护页面逐步加 validator 或 import warning。 |
...
| batch delete | 使用 scoped bulk delete，SQL 必须同时包含 tenant、id set、DataScope 和 domain filter。 |
| relation | 源记录先走 getById，可见后才查询关联目标。 |
| aggregate/chart | Dynamic aggregate 查询叠加 row/domain filter，并在权限引擎异常时 fail-closed。 |
| aggregate cache | cache key 必须包含 tenant/user/member/bypass-or-scoped，避免不同权限上下文复用结果。 |
| custom action | `count` 叠加 row/domain filter；`truncate` 返回 unsupported。 |
| named query | 显式 `resourceCode/actionCode` 声明后接入 DataScope；缺声明不得声称受保护。 |
| custom handler | 自定义入口调用 `DataAccessAuthorizationHelper`，不写 `if admin` 或手拼 `created_by`。 |
...
| NamedQuery block | 不会天然继承 Dynamic row filter；必须显式声明 `resourceCode` 和 `actionCode`。 |
| 自定义 Controller/PF4J handler | 不会自动获得 Dynamic scope；必须调用统一授权 helper 或复用受控 Dynamic 服务。 |
| Dynamic aggregate/chart | M1 已对动态模型聚合叠加 row/domain filter；NamedQuery aggregate 按显式声明接入。 |
| 导出、relation、options | 不能用 list E2E 结果推断已经安全；需要逐项补 block/API 级覆盖矩阵。 |
| batch delete | 不能只按 id 集合直接删除；当前使用 scoped bulk，并用影响行数检查 fail-closed。 |
...
| host 环境使用 `PW_PROFILE=full` 可能走 `/app/plugins` 默认路径。 | 普通 OSS targeted E2E 使用 `PW_PROFILE=oss` 并显式设置 `OSS_PLUGIN_ROOT`；RBAC 权限矩阵使用 `PW_PROFILE=rbac`。 |
| 全量 setup dependency 可能被无关插件或 page schema 阻塞。 | 使用 `PW_PROFILE=rbac` 的 targeted profile，只跑 bootstrap、multi-role users、test-fixtures、rbac-auth 和目标权限 spec；报告时必须说明不是 full Playwright 全套通过。 |
...
确认 RBAC profile 不依赖 `system_overview/page_schema`：
...
| 所有自定义 API 自动接管 | 边界太大，先用 `DataAccessAuthorizationHelper` 显式接入。 |
| 未声明 NamedQuery 自动推断 row filter | NamedQuery 有自己的字段、where、参数和 dataSource 语义，不能靠 SQL 猜资源。 |
...
| NamedQuery DataScope 接入 | 已采用显式 `resourceCode` + `actionCode` 声明，避免靠查询结构推断导致返工。 |
...
后续任务统一追踪在 `docs/backlog/2026-06-23-rbac-data-scope-follow-up-tasks.md`。本轮已收口 UI E2E 一期矩阵、`all` 后端租户边界、NamedQuery 显式 DataScope、自定义 Handler 授权 helper、scoped bulk delete 和 RBAC targeted setup；后续仅保留 relation 浏览器证据、跨租户浏览器 seed、`batchUpdate` 性能优化、小组长 scope 和 NamedQuery validator 等明确扩展项。
