...
| export/options/relation/aggregate | 需要逐项确认是否已经统一接入 row filter，不能只验证 list。 |

已知边界：

| 边界 | 一期处理 |
|---|---|
| `batchDelete` 直接按 id 集合删除时，必须补 row filter 或逐条走授权检查。 | 作为一期实现检查项，不允许作为已完成漏掉。 |
...
3. Dynamic list/count/detail/write 主链路统一执行 data scope。
...
| 验收项 | 标准 |
|---|---|
| backend tests | `DataScopeEvaluator`、`DataScopeService`、`DataPermissionEngine`、角色导入与权限继承测试通过。 |
...
| 聚合、导出、relation、options | 不能用 list E2E 结果推断已经安全；需要逐项补 block/API 级覆盖矩阵。 |
| batch delete | 不能只按 id 集合直接删除；必须补 row filter 或逐条执行授权检查。 |
