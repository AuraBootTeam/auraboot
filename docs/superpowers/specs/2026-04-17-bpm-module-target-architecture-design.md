...
### D6. BPMN namespace 约定

**决策**：

| Namespace | URI | 用途 |
|---|---|---|
| `smart:` | `http://smart.alibaba.com` | SmartEngine 引擎概念（assignee / condition / listener / 已存在） |
| `aura:` | `http://auraboot.com/bpmn` | AuraBoot 业务策略（withdrawPolicy / ccPolicy / formKey / requiredPermissions） |
...
<definitions xmlns="http://www.omg.org/spec/BPMN/20100524/MODEL"
             ...
             xmlns:aura="http://auraboot.com/bpmn"
             targetNamespace="http://auraboot.com/bpm">
  <process id="leave_request" isExecutable="true">
    <extensionElements>
      <aura:property name="withdrawPolicy" value="strict"/>
      <aura:property name="ccPolicy" value="all"/>
    </extensionElements>
    ...
    <userTask id="manager_approval" name="经理审批"
      ...
      <extensionElements>
        <aura:property name="formKey" value="leave-request-manager-form"/>
        <aura:property name="requiredPermissions" value="['LEAVE_APPROVE_MANAGER']"/>
      </extensionElements>
    </userTask>
  </process>
</definitions>
...
**约定**：
- 简单 string 值用 `<aura:property name="..." value="..."/>`
- 复杂 JSON 用 CDATA：`<aura:property name="formBinding"><![CDATA[ {...} ]]></aura:property>`
- 优先简单值；JSON 复杂时拆字段
...
| `WithdrawService` | 撤回 + WithdrawPolicy（strict/loose/none）；策略读 BPMN extension | `SmartEngine.ProcessCommandService.abort` + `BpmAuditService` + `BpmnExtensionParser` |
| `CcService` | 抄送 + CcPolicy 授权门；委托 SmartEngine NotificationService 发送和查询 | `SmartEngine.NotificationCommandService` + `BpmnExtensionParser` |
...
| `BpmnExtensionParser`（新增） | 解析 BPMN XML 中的 `aura:` namespace extension elements；按 deployment_id 缓存 | `BpmProcessDefinitionMapper` + Caffeine |
...
| Task 3 `WithdrawService` | ✅ 完成 | **改写** policy 读取：从 `def.getWithdrawPolicy()` 改为 `bpmnParser.getProcessProperty(deploymentId, "withdrawPolicy")` |
...
**新增 Task**：
- Task -1：`BpmnExtensionParser` 工具类 + 单元测试（Spec 1 阻塞性前置）
...
- 节点级业务策略（form binding / required permissions / cc override）必须放 `<extensionElements>` 中的 `<aura:property>`
- 禁止新建 `ab_bpm_node_config` 类似的"节点配置表"
- 流程级业务策略（withdrawPolicy / ccPolicy）必须放 `<process>` 元素的 extensionElements
...
| Q2 | `BpmnExtensionParser` 缓存失效粒度：deployment_id 还是 process_key？ | 倾向 deployment_id（不可变；部署新版本就是新 deployment_id） |
