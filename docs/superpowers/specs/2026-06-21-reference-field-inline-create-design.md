...
- 可选 `createFields: ["name","phone"]`:把表单 **过滤为精简快建字段子集**,适合「只填名称等几个必填」的顺手建。

> **主要技术风险点(需在 plan / spike 阶段先验证)**:`DslFormRenderer` 能否在一个 **独立 modal 容器** 内实例化,
> 拥有自己的 `SchemaRuntime` / stateManager,而不依赖、不污染外层宿主表单的 runtime。
> 这是本特性能否「干净复用」的关键。若验证发现耦合过深,退路是:
> 第一阶段先用 `FormDialog` + `createFields`(仅基础字段类型)覆盖轻量场景,完整表单复用作为 fast-follow。
...
```jsonc
{
  ...
  "createFields": ["name", "phone"],    // 可选:精简快建字段子集;不写则用目标模型完整表单
...
| 层 | 文件 | 改动 |
|---|---|---|
| 字段配置 schema | `app/framework/meta/schemas/types.ts` | `FieldConfig` 加 `allowCreate?` / `createFields?` / `createCommand?` 三个可选位 |
| 字段渲染层 | `app/framework/meta/rendering/RuntimeFieldRenderer.tsx` | reference 装配时识别 `allowCreate`,推导 `createCommand` / 解析 `createFields`,装配 `onCreateNew` 回调 + 权限 gate |
| 选择器组件 | `app/ui/smart/form/Select.tsx`(`SmartSelect`) | 接收 `canCreateNew` / `createLabel` / `onCreateNew`,渲染底部「+ 新建」项,点击触发回调;拿到结果后选中 |
...
RuntimeFieldRenderer 装配的 onCreateNew()
        │  打开 modal,内嵌 DslFormRenderer(目标模型 customer 表单 / createFields 子集)
        ▼
...
- **覆盖矩阵**:单选/多选 × 完整表单/createFields 子集 × 成功/失败/无权限。
...
3. `createFields` 子集渲染:是过滤目标模型 form 的字段,还是允许字段级覆盖(label/必填)—— 默认仅过滤,够用即止(YAGNI)。
