...
| 10 | 多 agent 协作 | `HandoffToolProvider`+`HandoffPermissionPolicy`+`HandoffResult`、群聊 `GroupChatAgentRouter`+`GroupChatTurnContextAssembler`+`GroupChatMessagePort`(SPI）+**真 IM-backed `GroupChatMessageAdapter`(8 方法全实现、`messageMapper.insert` 真持久化、`@Primary` 确定性胜出 #1465）**、ACP delegate | **M2/M3** | ✅ 群聊 port **非"待接"**——`GroupChatMessageAdapter` 早已是真实现,`NoOpGroupChatMessagePort` 只是 `@ConditionalOnMissingBean` 的 core-only 兜底;原风险=consumer 用 `getIfAvailable(NoOp::new)`(双 bean 会 `NoUniqueBeanDefinitionException`)+ NoOp 靠 `@ConditionalOnMissingBean`(scan-order 脆),已加 `@Primary` 消除 + 可证伪 wiring IT(去 `@Primary` 即红);多 agent 编排广度未真栈系统验证(🟡) |
