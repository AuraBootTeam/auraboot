...
| 14 | 产品化(数字员工界面) | `web-admin/app/plugins/core-ai-colleagues`(colleague 列表/向导/详情/对话/设置)、`/ai/colleagues`、agent-control-plane(mission/agent/tool CRUD DSL 页) | **M1/M2** | 🟠 **`core-ai-colleagues` 5 页全手写 tsx(3887 行专业化体验面:89 hub / 475 卡片网格 / 1034 向导 / 2081 5-tab 配置 / 208 流式聊天)**;§7 可行性矩阵已产出,owner 2026-07-24 决策**激进全转 DSL**(custom block 承载 rich UI)——跨栈多 session campaign,执行计划 `docs/backlog/2026-07-24-core-ai-colleagues-dsl-conversion.md`(基建=后端 pages.json + custom block 前后端注册 + 逐页 golden)。数字员工 golden 未进默认 CI(gap 2:自包含 runner 已固化) |
