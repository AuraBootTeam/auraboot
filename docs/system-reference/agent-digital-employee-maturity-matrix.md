...
| 6 | 技能资产层 | `ab_agent_skill`(skill_tools/execution_mode/actionability)、`AgentSkillService.resolveSkillTools`、`AgentTemplateSeeder`、bound skill→tool(通用 aurabot #1440 + 命名 agent #1449) | **M2** | 🔴 **`SkillEngine`(orchestration 执行器)生产零调用者**(`@Service` 但 grep main 引用=0)——orchestration execution_mode 已 seed 但运行时不执行,只 dsl_dispatch/atomic 路径真跑 |
...
1. **技能编排执行器 unwired**(维度 6):`SkillEngine` 生产零调用者,orchestration execution_mode 只 seed 不执行。要么接线到运行时,要么明确降级为「暂不支持 orchestration 模式」并从 seeder 去掉误导。
