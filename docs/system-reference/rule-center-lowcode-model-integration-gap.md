...
仍未关闭的产品 gap：这些用例覆盖 Strategy Studio + BPM + 用户引用字段 + 条件片段 lifecycle、用户引用字段进入 DecisionOps 执行日志事实快照、SLA/Automation/EventPolicy/Permission 的同类 authoring + 草稿保存 focused path、五入口事实目录一致性、SLA 原生配置页申请人规则 runtime Trace focused path、Automation 原生测试运行申请人字段 runtime Trace focused path，以及 EventPolicy 原生申请人规则 runtime Trace + 动作执行 focused path。通用业务 reference 字段保存/reload/API raw value 反查/Trace label 回看已由 2026-07-19 Reference / DecisionOps Trace 收口切片补齐为 focused evidence；仍需补的是 Permission/BPMN 各自真实运行 Trace / 动作执行 / runtime failure、BPMN Designer 节点属性面板同字段目录一致性，以及规则资产/消费方详情/执行日志三处 Trace 互跳截图。EventPolicy 仍需更广外部 provider live matrix，但申请人 reference 决策绑定 runtime Trace focused path 已关闭。
