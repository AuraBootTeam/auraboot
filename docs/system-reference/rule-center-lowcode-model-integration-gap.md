...
| 规则配置核心任务 | 如何配置条件表达式、嵌套组、字典值、空值、范围、引用字段、虚拟字段和权限字段 | 完整产品需求规格、必须支持的条件能力、规则条件配置链路、Gap 2/3/4/7/14 | 元数据和组件主干已通；2026-07-18 已补用户引用字段 `refTarget` 持久化/回读、后端 Trace label focused 证据和 Strategy Studio 场景字段；条件绑定的浏览器保存/reload lifecycle 仍未全通 |
...
仍未关闭的产品 gap：Strategy Studio 当前顶层“保存草稿/发布”保存的是 DMN decision table，`DecisionRuleBindingBlock` 条件配置只通过 `onChange` 输出 AST，顶层没有把它持久化成条件片段或消费方 binding。条件片段后端 lifecycle 目前有 create/createVersion/validate/publish，但没有 update draft；因此要做成产品级保存，需要明确策略：已有 DRAFT 是覆盖、创建新版本、还是跳转条件片段库继续编辑。这个 lifecycle 未定前，不能把 browser authoring 的“选择申请人”误报为“保存/reload 已闭环”。下一步需要补：真实浏览器从 Strategy Studio 或具体 SLA/BPM/Automation/EventPolicy/Permission 消费方选择申请人，保存到明确的规则资产或消费方 binding，reload 后回显用户 displayName，API 反查仍是 raw pid，运行后执行日志 UI 显示同一个用户 label，并完成三处 Trace 互跳截图。
