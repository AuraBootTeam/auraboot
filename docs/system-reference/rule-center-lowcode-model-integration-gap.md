...
| `ab_role_permission.conditions` 写入 | role import 绑定 permission 后读取 matching `permissionPolicies`，通过 `RolePermissionMapper.updateConditionsById(... conditions = #{json}::jsonb)` 写入 JSONB，并调用 `DecisionUsageIndexService.refreshSource("PERMISSION_POLICY", rolePermissionPid)` 刷新 usage-index | `PluginResourceImporterImplApplyTest#importRole_create_withPermissionPolicies` passed，断言 `updateConditionsById` 与 `refreshSource("PERMISSION_POLICY", "rp-policy-pid")` 被调用 | `DONE for backend import/runtime index contract`；后续浏览器 Trace 仍需按 RC-PERM 矩阵验证 |
...
| JSONB / Mapper 取证口径 | 本切片没有用 `JdbcTemplate ?::jsonb` 绕业务路径。`policy_schema` create 走平台 TypeHandler，update 走 mapper `::jsonb` 合同；`conditions` 写入走既有 `RolePermissionMapper.updateConditionsById` mapper `::jsonb` 合同 | targeted 后端组合 `PluginDirectoryLoaderRulesSlaTest` + `PluginResourceImporterImplApplyTest` + `PluginImportServiceImplCoreTest` + `DecisionUsageIndexServiceImplTest` + `ConditionFragmentServiceImplTest` 83/83 passed；`PluginResourceImporterImpl*` sibling tests 全部 passed；Jacoco 仅有既有 execution-data mismatch warning，Gradle 结果 `BUILD SUCCESSFUL` | `DONE for backend/plugin slice`；整体规则中心仍 `NOT MET / PARTIAL` |
...
| 规则资产复用 | 条件片段、DMN、规则绑定、动作计划成为可版本化、可影响分析、可追踪的共享资产 | 条件片段、DMN、Rule Binding、impact/usage index 有基础；SLA 和 Automation 各有 Rule Binding 保存/reload/impact 黄金切片 | RuleAsset/ConsumerBinding/VersionPolicy/Compatibility/Impact 的产品心智仍未完全统一 | 任一规则资产能显示引用方、版本策略、字段引用、输出 schema、最近执行和发布影响 |
...
| Gap 2 dict/enum 值输入统一 | `PARTIAL` | 条件片段和 Rule Binding 能带 allowedValues，不代表 DMN cell、测试上下文、Trace 全都 label/value 一致 | `wd_req_type` 在 ConditionBuilder、DMN、test context、Trace 各跑一次保存 value / 显示 label |
...
| RC-UX-01 | 中文和窄视口 | 632px 与桌面打开所有主入口和就地入口 | 主路径无 raw code、英文 fallback、横向溢出；抽屉不压缩画布 | `PARTIAL+`：BPMN Designer compact focused slice 已证明 632px 下内部组件栏/属性栏默认折叠为按钮，打开时以 overlay drawer 覆盖而不是重新挤压画布；仍需所有主入口、就地入口、全局 shell 侧栏和更多页面窄视口矩阵回归 |
...
### RC-DICT-01 / RC-TRACE-01 per-run fact metadata snapshot focused slice（2026-07-18，focused 已验证 / browser golden 待补）
...
验证状态：本节已经完成 focused 后端与前端组件级验证。后端首轮执行 `platform && ./gradlew :test --tests com.auraboot.framework.decision.DecisionRuntimeIntegrationTest.evaluate_persistsFactMetadataSnapshotForDictBackedModelFields --no-daemon` 曾因测试夹具没有创建物理字段而失败，修复为真实低码 service path 后同一命令 passed。前端 Trace metadata 展示随多选 authoring 一起运行：`web-admin && pnpm exec vitest run app/shared/decision/ui/__tests__/ConditionBuilder.test.tsx app/shared/decision/ui/__tests__/DecisionTableEditor.test.tsx app/ui/smart/decision/__tests__/ExecutionLogTraceBlock.test.tsx` passed，3 files / 41 tests passed。focused e2e-truth 静态扫描未命中 `test.skip/fixme/only`、`waitForTimeout`、`retries` 或 PUT/PATCH API 兜底；命中的 `threshold` / `baseline_version` 是既有 Java 测试业务变量/表字段。当前状态是 **focused 已验证 / browser golden 待补**，不能把 `RC-DICT-01`、`RC-TRACE-01` 或 full browser golden 改成 DONE。后续更广补测至少还需要跑：
...
后续仍未关闭：reference/user picker 字段、虚拟模型字段的 browser Trace、Strategy Studio 真实浏览器 Trace 回看、全消费方浏览器黄金矩阵，以及执行日志页面与规则资产/消费方详情之间的全量 Trace 互跳。

### RC-DICT-01 multi-select `IN / NOT_IN` authoring focused slice（2026-07-18，focused 已验证 / browser golden 待补）
...
验证状态：当前已完成 focused 前端组件验证。新 worktree 缺少依赖时先执行 `pnpm install --offline --frozen-lockfile` 生成 ignored `node_modules`；随后运行 `web-admin && pnpm exec vitest run app/shared/decision/ui/__tests__/ConditionBuilder.test.tsx app/shared/decision/ui/__tests__/DecisionTableEditor.test.tsx app/ui/smart/decision/__tests__/ExecutionLogTraceBlock.test.tsx`，3 files / 41 tests passed。focused e2e-truth 静态扫描对 touched 测试夹具未命中 `test.skip/fixme/only`、`waitForTimeout`、`retries` 或 PUT/PATCH API 兜底；命中的 `threshold` / `baseline_version` 是既有 Java 测试业务变量/表字段。状态是 **focused 已验证 / browser golden 待补**。后续补测应转向真实 Strategy Studio / 消费方 browser authoring 与后端 evaluator 成对验证；如需要局部回归，可复跑：
...
后续仍未关闭：本节自己的真实 Strategy Studio / 消费方 browser authoring 验证；reference/user picker 字段；虚拟模型字段的 browser Trace；全消费方浏览器黄金矩阵；执行日志页面与规则资产/消费方详情之间的全量 Trace 互跳；以及 `IN / NOT_IN` 从 UI 保存到后端 evaluator 的 browser + backend 成对证据。
...
- `platform && ./gradlew :test --tests com.auraboot.framework.decision.service.impl.DecisionTableDmnXmlServiceImplTest --no-daemon`：passed，4/4 tests passed，其中包含 `roundTripPreservesValueLabelsInAuraDmnMetadata`
- focused e2e-truth 静态扫描新增后端测试夹具：未命中 `test.skip/fixme/only`、`waitForTimeout`、`retries`、PUT/PATCH API 兜底、threshold/baseline

当前状态是 **focused 后端已验证 / browser golden 待补**。它已经证明 Aura DMN 扩展 metadata 的 XML round-trip 与安全解析 focused path 可运行；但尚未运行 Strategy Studio 真实浏览器导出/导入/roundTrip，也未覆盖更广 Drools/KIE runtime validation、Vitest、Playwright 或全消费方 Trace 矩阵。

2026-07-18 follow-up 实现状态：Strategy Studio browser golden 预跑暴露出两个更细的 round-trip gap。第一，浏览器端 DMN 编辑器可能把字典输入的 executable data type 规范化为 `enum`，产品合同应以 `dictCode / allowedValues / valueLabels` 是否保留为准，而不是强绑编辑器内部 `dataType=dict`。第二，后端 DMN import 过去把所有 `inputEntry` 都回写成 `operator=EQ, value="", feel=<DMN text>`，导致 `"annual", "sick"` 这种由本平台导出的简单 literal list 在导入后无法恢复为 UI 可编辑的 `IN` 多选值。本轮代码已把简单字符串、布尔、数字 literal 和 simple literal list 解析回 `EQ / IN` 结构化 cell，复杂 FEEL、范围、比较、`not(...)`、`null` 和 `-` 仍保留为 `feel`，避免过度解析。

验证口径：按 owner 当前要求，本轮先合并分支，Strategy Studio browser golden 与后端测试复跑均延后。已新增/调整 focused 单测夹具覆盖 simple literal list import；旧测试会话在停止补测前已经暴露 XML 序列化引号断言过脆的问题，断言已改为检查业务 raw value `annual/sick`，但没有再次复跑。因此本段只能标为 **implementation pending verification**，不能把 RC-DMN-01、Strategy Studio 真实浏览器导入导出或全消费方 Trace 矩阵提升为完成。

后续仍未关闭：Strategy Studio 真实浏览器 DMN 导出/导入/roundTrip 回显；DMN valueLabels 在规则资产、SLA/BPM/Automation/Permission 消费方、DecisionOps 执行日志三处 Trace 的全矩阵；reference/user picker 字段；虚拟模型字段 browser Trace；以及全消费方 browser + backend 成对证据。
