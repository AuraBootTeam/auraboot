...
仍未关闭的产品 gap：这些用例覆盖 Strategy Studio + BPM + 用户引用字段 + 条件片段 lifecycle、用户引用字段进入 DecisionOps 执行日志事实快照，以及 SLA/Automation/EventPolicy/Permission 的同类 authoring + 草稿保存 focused path。还需要补通用业务 reference 字段保存/reload/API raw value 反查/Trace label 回看、四类消费方各自真实运行 Trace / 动作执行 / runtime failure、以及规则资产/消费方详情/执行日志三处 Trace 互跳截图。
