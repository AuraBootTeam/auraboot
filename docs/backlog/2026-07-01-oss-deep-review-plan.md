...
| DR-20260701-W2-perf-002 | P1 | IM 会话列表 N+1(`ImConversationServiceImpl.listByUser` 1+4N) | **✅ 已修 + 真栈验证**:4 batched 查询(selectBatchIds + IN membership + `DISTINCT ON` 最后消息 + GROUP BY 成员数),纯重构保留精确输出。Mockito 单测证无 N+1、真 Postgres IT 证 DISTINCT ON 取 max-seq 正确、现有 48 测试更新到 batched mapper 后全绿。见提交 `perf(im): batch listByUser` |
...
| accepted & 已修 | **12 finding 组 = 16 处修复**(wave1:3 §9 + 1 loader 诊断 + 4 测试 gap + 1 inbox + 2 i18n;wave2:2 P1 安全 + 1 config-SQLi 硬化 + **2 perf N+1〔unread + IM 会话列表,均真栈验证〕**),**26 测试新增/扩展** |
...
| 延后(owner/重验/env-blocked,已分层非 completionism) | wave1:2(bootstrap-002 §4.1 / flyway-001 baseline);wave2:**仅 OEE perf**(env-blocked 缺 PCBA 表,需先导入插件栈)+ 1 跨仓 test-only + SSRF candidate。3 个 perf N+1 中 2 个已修验证,OEE 因缺表无法真栈验证 |
| 本地门禁 | oss-boundary / reset-init / jsonb / public-record-id / version-sync 全 ✅ |
| 前端 typecheck | react-router typegen && tsc ✅ |
| host-first 真栈 | unread + IM 会话列表 两个 perf N+1 均已用真 Postgres IT(aura_boot,@Transactional 回滚不扰并发)验证 SQL 正确(含 DISTINCT ON per-group top-1) |

status: 两轮 review(5+3 维)完成。全部**可安全修复且不与并发会话冲突**的 verified finding 已闭环 + 真栈/typecheck 验证(**16 处修复 / 26 测试**,含 wave2 两个 P1 安全 bug + config-SQLi 硬化 + **两个真栈验证的 perf N+1〔unread + IM 会话列表〕**)。剩余延后项均**显式价值分层**:OEE perf **env-blocked**(缺 PCBA 插件表,需先导入才能真栈验证,否则违反红线 ship 未验证 SQL)、bootstrap-002/flyway-001 需 owner 决策、跨仓 test-only、SSRF admin-gated——非闷头清的待办 backlog。交付于 PR #1126。
