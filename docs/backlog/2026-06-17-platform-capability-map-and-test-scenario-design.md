...
| 断言 | 层 | 类型 | 现状 |
|---|---|---|---|
| agg/groupBy/aggField/filter 解析正确,字段不幻觉 | L2 | T3 | ✅ **已测 `ChatBiIntentLiveIT`(真 DeepSeek 5/5:agg/group/field 全对 + 5/5 grounded 零幻觉)** |
...
> 12 个治理门,每个**正向放行 + 负向拦截**成对验证。
>
> **⚠️ 现状列已按实测修正(2026-06-17 gate 负向验证批次)**:本表"现状"初稿来自 gate 盘点 subagent,
> 其多条"E2E gap"经 §15 实地核对(读测试树)**被证伪——大半已有覆盖**:plan_hash 篡改已有
> `ApprovalGateP0FixIntegrationTest` P0c、ACL allow/deny 已有 `ToolAclIntegrationTest`(全面)、
> timeout enforcement 已有 `AgentApprovalGateServiceConcurrencyTest`(mock)、fail-secure 已有 P0FixIT。
> **真正缺的只有 Context 门零专测**——本批次已补 `ToolContextPolicyTest`(10/0/0)。下表为修正后状态。

| Gate | 拦截对象 | 正向 | 负向 | 现状(实测核对) |
|---|---|---|---|---|
| 1 Capability(L1) | 权限+capability ceiling | 权限足→放行 | 缺权→deny missing_permission | ✅ `ToolCapabilityPolicyTest` |
| 2 Argument(L2) | 参数类型/格式 | 合法→hash 稳定 | type 错→argument_invalid | ✅ `ToolArgumentPolicyTest` |
| 3 Context(L3) | recordId 归属/跨租户 | recordId∈context | 跨租户→context_tenant_mismatch / record→context_scope_violation | ✅ **`ToolContextPolicyTest`(2026-06-17 补,10/0/0)** |
| 4 Durability(L4) | 外部副作用边界 | claim acquired=true→执行 | 重试→replay 不重复 | ✅ `ToolDurabilityPolicyTest` |
| 5 Approval(L5) | L3/L4 风险工具 | 人审 pending→approved | 无批准→hold;无权审→deny | ✅ `ApprovalGateP0FixIntegrationTest`(plan_hash 篡改 P0c) |
| 6 ApprovalGate svc | 创建/批准/超时/幂等/plan_hash | CRUD+event→resume | fail-secure no-policy;hash mismatch→reject;超时 expire | ✅ P0FixIT + `...ConcurrencyTest`(超时 mock,**无真栈 IT=次要薄点**) |
| 7 ToolAclChecker | 5 维 ACL | 5 元组 match→allow | no-match+有规则→deny | ✅ `ToolAclIntegrationTest`(empty→allow/首规则→fail-secure deny/优先级 deny>allow,全面) |
| 8 RuntimeAuth | 效果类(WRITE/NET/SECRET) | plan 期预授权 | forbidden effect→deny | 🟡 contract-only(待核实是否实装) |
...
**gate 整体判定(实测修正版)**:12 门里 **Capability/Argument/Context/Durability/Approval/ApprovalGate-svc/
ACL/DurableLedger/幻觉/风险/权限 = 真拦,均有测**(plan_hash 篡改、ACL allow/deny、fail-secure、timeout
enforcement、Context 跨租户/record-scope 全覆盖,Context 由本批次补齐)。**残留薄点仅 2 条且次要**:
① ApprovalGate timeout enforcement 只有 mock 单测、无真栈 IT;② RuntimeAuth 效果级授权疑 contract-only,待核实是否实装。
**结论:gate 体系实测远比盘点声称的强,无高危 E2E 空洞。**
...
**❌ gap(无测/无路径)**:
- ~~**agent gate**:Context E2E、ACL、ApprovalGate 超时/篡改、RuntimeAuth~~ —— **已实测核对(2026-06-17),大半被证伪**:plan_hash/ACL/fail-secure/timeout 都已有测;Context 已补 `ToolContextPolicyTest`。**残留仅**:ApprovalGate timeout 真栈 IT(mock→真DB,次要)+ RuntimeAuth 是否实装(待核)。详见 §D 修正。
