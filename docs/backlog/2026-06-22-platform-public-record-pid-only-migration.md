...
- 2026-06-24 final refresh: current branch has hard-mode pid-only implementation green across static source scan, live OpenAPI scoped + component scan, OSS backend full, frontend `check` + full unit, schema SQL/drift, clean OSS artifact publish, enterprise targeted, enterprise full, and enterprise canonical docs governance. After rebasing onto latest OSS `origin/main` `8541efe602293966f7417156a1fc04c3f4b0be56`, maintainer direction is to skip another full rerun and merge after incremental gates. Post-rebase incremental backend tests passed `47` tests with `0` failures/errors, and `diff --check`, public-record contract scan, and docs governance passed.
...
- Post-rebase close-out on latest OSS `origin/main`:
  After rebase, OSS full v17 surfaced three latest-main/test-drift failures.
  The branch fixed the `AdminUserController -> UserMapper` architecture drift,
  updated stale AggregateQuery public-field fixture data to pid-only, and aligned
  the DynamicDataService coverage test with the current unsupported destructive
  custom-action policy. Per maintainer direction, no further full rerun was
  performed. Incremental `:test` filtered to the five affected classes passed:
  XML `47` tests / `0` failures / `0` errors / `0` skipped. `git diff --check`,
  `scripts/check-public-record-id-contracts.sh`, and
  `scripts/check-docs-governance.sh` also passed.
...
- OSS frontend full unit:
  ...
  Latest post-rebase result: `477` test files / `4880` tests passed / `0` failed. Note: pnpm 11 emits a stderr deprecation warning that breaks CLI tests expecting empty stderr, so final unit evidence uses pnpm `9.15.9`, which satisfies the repository `pnpm >=9` engine.
- OSS schema SQL gate:
  ...
  Latest post-rebase result: `schema.sql` has `10681` lines, `312` expected `CREATE TABLE` statements, and applies cleanly with `313` tables created.
- OSS schema drift gate:
  `PATH=/opt/homebrew/Cellar/postgresql@17/17.6/bin:$PATH scripts/db/check-schema-drift.sh --edition oss`.
  Latest post-rebase result: committed snapshot matches Flyway result at `v20260624021000`.
