...
基于最新 `origin/main`(`958cd5104`) 合并到 worktree `codex/public-record-pid-migration` 后重新扫描。
...
- OSS backend full test:
  `AURA_ENV=test IMPORT_TEST_FIXTURES=true ./gradlew --no-daemon cleanTest test`
  over a freshly recreated `aura_public_record_pid_oss_test` database after OSS Flyway migrate.
  XML summary: 1488 files / 11892 tests / 0 failures / 0 errors / 43 skipped.
- OSS frontend gate:
...
- OSS schema gate:
  ...
  Result: `schema.sql` applies cleanly; 312 tables created.
