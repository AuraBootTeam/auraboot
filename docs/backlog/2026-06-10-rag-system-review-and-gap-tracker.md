...
- Gate improvements from session deep-retro (`docs/retro/2026-06-11-rag-endgame-session-deep-retro.md`): ① `check-schema-migration-pairing.sh` — fail when a diff alters existing-table columns/constraints in schema.sql without a paired `database/migrations/` file (would have mechanically caught the missing retry-count migration); ② API response-shape change contract — PR must list grep-verified consumers of the changed path (CommandPalette-class silent breaks are invisible to tsc).
...
- **2026-06-11 (G10 DONE — mechanism + calibration; full effect pending embedding key)**: query-level no-answer floor on both paths, config-driven, default-on (`RagRetrievalProperties` `aurabot.rag.*` + `aurabot.d7.min-match-score`; new `KeywordCoverage` util; metric `rag.retrieval.rejection_floor_dropped`). Calibrated on the 52-query golden set for **zero per-path recall regression** (kFloorA=0.27 / minScoreB=0.15 / vFloorA=0.20). **Measured (eval re-run, floor on): correct-rejection 0 → 3/10, no-answer recall 0.300, Path A recall@5 0.600 / Path B 0.985 preserved.** Implementation pitfall caught by the real-stack eval: a first per-chunk filter regressed Path-A recall 0.600→0.300 (dropped the relevant low-coverage chunk while a wrong higher-coverage chunk survived); fixed to a query-level best-match gate. **The remaining 7 off-topic queries share high-frequency platform vocabulary (接口/同步/API) and cannot be rejected by keyword signal without recall loss (curve: 6/10 rejection costs a third of Path-A recall) — this needs the vector leg; the floor is already wired to `minVectorSimilarity` (default 0.20, inert until the embedding key lands).** Tests: `KeywordCoverageTest` + floor cases in `RagRetrievalServiceBranchTest`/`D7CompiledKnowledgeServiceTest` (default suite) + `RagEvaluationPhase2IT` no-answer matrix (env-gated). Detail + precision/recall curve: `docs/backlog/2026-06-11-rag-g10-rejection-floor.md`.
- **Owner item (unchanged, now also unblocks the rest of G10)**: configure the `ab_cloud_config` embedding key → rerun `RagEvaluationPhase2IT` in `live` mode → recalibrate `aurabot.rag.min-vector-similarity` from the live hit/neither similarity distributions; the vocabulary-overlap no-answer rejections follow without code changes.
