...
> **MD-3 page-gen path — RESOLVED (PR follows).** The designer AI (`AiPagePanel` quick commands + free-text, and the `AiPageGenerateDialog` modal) previously routed page generation through the general aurabot agent (`auraBotApi.chatStream`), which injects an agent system prompt + business tools → the model replied conversationally / called tools instead of emitting page DSL. Fix: a dedicated **tools-off** completion endpoint `POST /api/agent/nl-modeling/generate-page` (`NlModelingService.generatePageDsl` → `callLlm` with no tools, no agent prompt) that runs the caller's page-gen system prompt and returns the raw model text; both panel and modal now call it. **Verified host-first (real DeepSeek)**: the endpoint returns clean parseable page DSL (`kind:list, blocks:[filters,toolbar,table]`), and the designer "Add Chart" quick command produces a chart block **applied to the canvas + auto-saved** (no conversational reply). Unit 17/17 (incl. blank-input guard); tsc clean.
