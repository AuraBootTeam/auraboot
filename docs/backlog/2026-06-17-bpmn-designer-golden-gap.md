...
### 已知 BLOCKED → owner-deferred(2026-06-18 决策已闭环)
> 两项均为 SmartEngine 引擎特性,**owner 2026-06-18 决定保持现状**(niche / 投入大于即时价值);方案已存档,日后真有流程需要再做。BPMN 设计器 golden 目标本体不受影响,已交付。
- **SEQ-01 多元素顺序会签**:诊断已纠正(引擎有代码路径,非"做不了";多元素实测不迭代 = 精确双根因)。**owner 决定保持现状**,SEQ-01 维持 `@Disabled`,SEQ-02/03 通过;完整 4.0.2 候选缓存方案见 `2026-06-18-sequential-mi-countersign-analysis.md`(status: closed)。
- **GAP-252 receiveTask message / inclusiveGateway completionCondition**:引擎无 message parser + 无 SignalCatchService/correlation,UI 已 disabled readOnly + 文案。**owner 决定保持现状**(比 SEQ-01 更大引擎特性);有真实外部消息关联流程需求时再评估。
...
**剩余 2 项均为 SmartEngine 引擎特性,owner 2026-06-18 决定保持现状**(见上 §已知 BLOCKED → owner-deferred):SEQ-01 多元素顺序会签、GAP-252 receiveTask message。两项有完整方案/根因存档,非阻塞,有真实需求时再起。
