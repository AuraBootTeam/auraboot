...
| **G-B5** | 🔴 **PRODUCT/UPSTREAM** | [BE] | **rollback / add-sign / remove-sign 任务操作在真栈下 100% 失败**:`se_process_rollback_record.operator_user_id` 与 `se_assignee_operation_record.operator_user_id` 为 NOT NULL,但 SmartEngine 引擎方法(`transferWithReason` 之外的 `addTaskAssigneeCandidateWithReason` / rollback)**不写 operator_user_id 且 API/candidate 模型无 operator 入参**,AuraBoot 层无法注入 → `null value ... violates not-null constraint` → 命令 500。**之前被 `BpmTaskOperationTest` 的 `assumeTrue(false)` 静默 skip 掩盖**(G-T3 揭露)。 | `TaskService.java:341`(addSign)+ 真栈 IT 异常 `se_process_rollback_record`/`se_assignee_operation_record` operator_user_id NOT NULL;引擎 jar `addTaskAssigneeCandidateWithReason(taskId,tenantId,candidate,reason)` 无 operator 形参(`javap` 确认) | **BLOCKED-UPSTREAM**:需 SmartEngine fork 提供 operator 入参(或 thread-local operator context)。3 个 IT 已 `@Disabled` 标注根因;前端 UserTask 工具栏的「加签/减签/退回」行动点需在引擎修复前禁用或提示。验收=引擎修复后去 @Disabled,真栈断言操作记录 operator 落库。 |
...
- [~] S1 BE 假通过清理(G-T3)— **BpmTaskOperationTest DONE**(`bacad7cce`:18 tests 15 pass / 3 @Disabled BLOCKED-UPSTREAM / 0 fail,verified auraboot_42)。**剩余迭代**:其余含 `assumeTrue(false)` 的文件(`ProcessOrchestrationServiceExtTest` 9 / `ProcessOrchestrationServiceTest` 4 / `TriggerServiceTest` 3 / `CallbackServiceTest` 3 / `BpmTaskActionsFallbackTest` 3)+ 2 个整文件注释的服务测试(`ProcessEngineServiceTest` 23 / `TenantAwareProcessEngineServiceTest` 14)
- [x] S2a BE 转换器健壮(G-B2)— DONE(`6510264e5`:未知节点抛错,50/50 converter test)
- [ ] S2b BE 预校验端点(G-B1)— pending(iteration)
- [x] S3 FE 校验视觉反馈(G-U1)— DONE(`6d2e75ae6`:画布节点级 error/warning 高亮,24/24 vitest);G-U4 banner 可点击定位 pending
- [ ] S4 E2E 真 UI golden(G-T1/T2/T4/T5)— pending(需 Vite+BFF+backend 全栈 + Playwright;real palette drag + 属性面板真编辑 + import/export + undo/redo/validate/monitor + 截图)
- [ ] S5 决策项(G-U2/G-U3)— pending(iteration)
- [ ] RV 完成前全量复核 + 复核文档
- [ ] P6 复盘固化 + 收口 merge
...
| `bacad7cce` | G-T3 / G-B5 | BpmTaskOperationTest 消除 18 处 `assumeTrue(false)` 假通过;assignee 绑定 actor 使 5 操作真通过;揭露 G-B5 上游 operator_user_id bug(rollback/addSign/removeSign)并诚实 @Disabled | 18 tests:15 pass / 3 @Disabled / 0 fail(真栈 auraboot_42) |
