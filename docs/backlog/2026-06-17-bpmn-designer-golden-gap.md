...
| G-B3 | ✅ **FIXED** | [BE] | (原)deploy 失败 root cause 被 SmartEngine 吞,`BusinessException` 仅透传 message。**已修**:`ProcessDeploymentService.deploy` 失败时 `log.error(...,e)` dump 注入后 BPMN(含 processKey)+ 遍历 cause 链记日志(AuraBoot `BusinessException` 不链 JVM cause,正因如此才需直接 log throwable)。 | `ProcessDeploymentService:495-514` | `BpmDeployFailureDiagnosticsIT` **1/1**(ListAppender 断言 BPMN+cause+引擎根因可从日志恢复) |
| G-B4 | ✅ **FIXED** | [BE] | (原)`BpmAttachmentController` 是 stub。**已实现**:upload/list/delete(task + process),复用 `FileService`(`ab_file`+`ab_file_relation`+`StorageProvider`),entityType `BPM_TASK`/`BPM_PROCESS`,`@RequirePermission(WORKFLOW_*)`。 | 控制器 | `BpmAttachmentControllerTest` **4/4**(关联 entityType/list 映射/delete 委派);存储层由 `FileServiceImplTest` 真栈覆盖 |
| **G-B5** | ✅ **FIXED** | [BE] | (原)rollback / add-sign / remove-sign 在真栈下 100% 失败:引擎从 `taskInstance.getClaimUserId()` 取 operator,任务**已分派未认领**时 claimUserId=null → `se_process_rollback_record`/`se_assignee_operation_record` 的 `operator_user_id`(NOT NULL)约束违反 → 命令 500;曾被 `assumeTrue(false)` 静默 skip 掩盖。 | 引擎实际从 claimUserId 取 operator(`DefaultTaskCommandService` rollback:375/addSign:406/removeSign:437),未认领即 null | **已修复**:SmartEngine fork 加显式 `operatorUserId` 重载(`rollbackTask`/`add|removeTaskAssigneeCandidateWithReason`,旧签名委托回退 claimUserId;**PR #2 merged**),auraboot `TaskService` 三处传 `getCurrentUserId()`(真操作人)。**引擎已出正式版 SmartEngine 4.0.1**(PR #3 merged + tag `v4.0.1` + GitHub Release;非就地改 4.0.0),auraboot `build.gradle` 显式 pin 4.0.1(PR #753)。真栈验证:`BpmTaskOperationTest` **18 pass / 0 skip / 0 fail**;**全方位回归 @ 4.0.1**:完整 BPM 包套件 **112 classes / 534 tests / 0 fail / 2 skip**(上游 MI 顺序 @Disabled + LLM live IT 缺 key)+ **9/9 UI golden**。 |
...
### 引擎特性收尾(2026-06-18 第二轮:owner 改决定→完成全部 gap)
> owner 从"保持现状"改为"继续完成全部 gap"。下列项已实现并真栈验证;唯一保留 = inclusiveGateway completionCondition(理由见下)。
- **SEQ-01 多元素顺序会签** — ✅ **DONE @ SmartEngine 4.0.2**:`UserTaskBehavior` 顺序分支 enter 缓存全候选(activityInstance-scoped String 变量)+ `handleMultiInstance` 顺序时 `nrOfInstances` 取缓存全量 + complete 时从缓存读候选(回退 dispatcher)。非持久化变量存储自动回退旧行为。storage-custom 回归 56/0;auraboot `BpmMultiInstanceSequentialTest` **SEQ-01/02/03 3/3 通过 @ auraboot_50**;SEQ-01 已解禁。SmartEngine PR #4 merged + tag `v4.0.2` + GH Release。详 `2026-06-18-sequential-mi-countersign-analysis.md`(status: shipped)。
- **GAP-252 receiveTask message** — ✅ **DONE(无需改引擎)**:引擎的 receiveTask `enter` 已 park(返 needPause)、`signal()` 已 resume——缺的只是 message-name→execution 关联。落地:`JsonToBpmnConverter` 在 `<receiveTask>` 上 emit `messageRef`;`ProcessOrchestrationService.deliverMessage(piId,messageName,vars)` 用 designerJson 的 messageRef 关联 parked receiveTask + `ExecutionCommandService.signal` 续流;`POST /api/bpm/process-instances/{id}/messages` 端点;`ReceiveTaskEditor` messageRef/messageType 解禁。真栈 IT `BpmReceiveTaskMessageIT` **1/1 通过**(park→非匹配消息不动→匹配消息续流到 completed)。
- **GAP-252 inclusiveGateway completionCondition** — ⚪ **DEFERRED(有理由,非阻塞)**:引擎 inclusiveGateway join 是 169 行精密 unbalanced/embedded join-latch 逻辑;加 completionCondition 阈值早结是对核心网关 join 语义的高回归风险改动,而**标准 BPMN N-of-N inclusive join 已正确支持**。按 decision-defaults(长期视角 + 不为 niche 引入回归)保留标准默认;UI 字段保持 disabled + 准确文案。真有 M-of-N 早结需求再单独评估(引擎 `InclusiveGatewayBehavior` + parser)。
...
- [x] S1 BE 假通过清理(G-T3)— **全部 40 处 `assumeTrue(false)` 清零**(#714 BpmTaskOperationTest 18〔15 pass/3 @Disabled〕+ #718 BpmTaskActionsFallbackTest 3 / TriggerServiceTest 3 / CallbackServiceTest 3 / ProcessOrchestrationServiceTest 4 / ProcessOrchestrationServiceExtTest 9,后 5 文件 32 tests 全真通过 verified auraboot_42)。**scope-out(非迭代,理由记录)**:2 个整文件注释的 `ProcessEngineServiceTest`(23)/`TenantAwareProcessEngineServiceTest`(14)是 **Mockito mock-engine 单元测试**(`@Mock SmartEngine`+`@InjectMocks`),因 ProcessEngineService API 漂移而注释;红线偏好真栈 > mock,且 ProcessEngineService 真栈覆盖已由 orchestration/task IT 提供。**决策:不恢复 mock 版;若要补应写真栈 IT**(类似 ProcessDesignerJsonValidationIT)。**【2026-06-18 已完成 — BPMN验证#2】**:删除 2 个死 mock 文件,新增 `ProcessEngineServiceIT`(真栈 BaseIntegrationTest + 真 SmartEngine):start+get / **租户隔离 read(异租户→null)**/ suspend-resume / terminate / node-status / by-user —— **6/6 通过(auraboot_48)**。

## BPMN 全验证收口(2026-06-18,#2 + #3)

**#2 服务测试真栈化** — ✅ 见上(ProcessEngineServiceIT 6/6)。

**#3 designer 规格 L3 收口** — ✅ **L3 运行时已由专门后端 IT 覆盖**(设计器 browser spec 测 L1 designerJson + L2 BPMN XML,运行时 L3 正确交给后端 IT,非浏览器重做 = 好架构)。4.0.1 真栈验证:`HttpServiceTaskDelegateIntegrationTest` 4/4(http serviceTask 运行时)/ `DroolsServiceTaskDelegateIntegrationTest` 2/2(rule-task)/ `AlarmNotifyDeliveryRealEngineIntegrationTest` 1/1(notification 投递)/ `BpmMultiInstanceTest` 3/3(并发 MI 运行时)/ `CommandServiceTask*`(command)。**唯一真 L3 缺口 = receiveTask**(signal/message unblock):GAP-252 引擎无 message parser + 无 SignalCatchService/correlation,`messageRef` 已 disabled —— 引擎限制,需引擎加 message 支持才可解(同 G-B5 路径,记 backlog)。
...
- [x] RV 完成前全量复核 + 复核文档(/e2e-truth + 五项证据 + 截图;复核见 handover `docs/handover/HANDOVER-2026-06-18-bpmn-designer-golden.md`)
- [x] P6 复盘固化 + 收口 merge(18 auraboot PR + SmartEngine #2/#3 + v4.0.1 全 MERGED;handover #777)

## 收口(2026-06-18,owner 决策已闭环)

**BPMN 设计器 golden 目标 = 交付完成(shipped)。** 核心 + 全验证已交付:40 处 `assumeTrue(false)` 假通过清零、G-B5 真引擎 bug 修复出 SmartEngine v4.0.1、9/9 真浏览器 UI golden、`/validate` 预校验端点、画布节点级校验高亮、property-editor 真编辑/undo-redo/monitor/import-export golden、ProcessEngineServiceIT 真栈 6/6、L3 运行时后端 IT 全覆盖。

**更新(2026-06-18 第二轮,owner 改决定→完成全部 gap)**:此前 deferred 的引擎特性已全部完成(见上 §引擎特性收尾):**SEQ-01 多元素顺序会签**(SmartEngine 4.0.2,真栈 3/3)、**GAP-252 receiveTask message**(auraboot 关联 + 端点 + UI,真栈 1/1)、**G-B3** deploy 诊断(1/1)、**G-B4** attachment(4/4)。唯一带理由保留 = **inclusiveGateway completionCondition**(核心网关 join 高回归风险 niche 特性,标准 N-of-N 已支持)。全量 BPM 套件 @ 4.0.2 回归见 §最终测试报告 `docs/backlog/2026-06-18-bpmn-remaining-gaps-final-report.md`。
