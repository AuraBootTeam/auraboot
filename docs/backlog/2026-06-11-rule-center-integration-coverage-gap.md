...
### 2026-06-11 Round 13: Automation 历史 deep 断言收紧

状态: DONE。

痛点:

- `automation-deep.spec.ts` 的 `AT-005` 仍在旧 `/automation/{pid}` 页面查找泛化 `condition-builder`，找不到就 `test.skip`。这不是规则中心条件能力的有效证据，且真实 condition/control-condition 链路已由 `automation-designer-golden.spec.ts` 的 H1/S5/E2/N-CONDITION-EDGE 和 `automation/rule-binding-designer-host.spec.ts` 覆盖。
- 同文件 `AT-011/AT-012/AT-014` 对 API 反查过宽：GET 失败时可能静默通过，multi-action 只断言 `>=3`，priority 断言等价于永真。收紧后发现 Automation 当前后端 DTO/entity/create request 没有 `priority` 字段，说明 `AT-014` 是不存在产品能力的历史假覆盖。
- `automation-validation-gate.spec.ts` 在 dirty/save enable 等待里使用固定 `waitForTimeout(400)`，属于可替换的状态等待。

方案:

- 删除旧 `AT-005` skip-heavy 用例，不再把“条件构造器隐藏在旧 tab/section”当作当前完成证据。
- 将 `AT-011` 改为 API 失败即失败，并精确断言 3 个 action label 顺序。
- 将 `AT-012` 改为 API 失败即失败，精确断言 `triggerType=on_record_update`。
- 删除 `AT-014`，不再把不存在的 Automation priority 配置计入覆盖；如未来需要 automation priority，应先进入新产品 scope，补 DTO/entity/migration/UI 后再建真实测试。
- 将 validation gate 的固定等待替换为 `expect.poll(save.isEnabled)`，保留原本“initialData effect 可能重置 dirty”的重试语义。

验证:

- `pnpm typecheck` -> passed。
- `NO_PROXY=localhost,127.0.0.1 PLAYWRIGHT_BASE_URL=http://127.0.0.1:5174 BACKEND_URL=http://127.0.0.1:6443 pnpm exec playwright test -c playwright.quick.config.ts tests/e2e/automation/automation-deep.spec.ts tests/e2e/automation/automation-validation-gate.spec.ts --project=chromium --grep "AT-011|AT-012|VG-01|VG-02"` -> 4 passed。
- `/e2e-truth` 红线自查: `automation-deep.spec.ts` 与 `automation-validation-gate.spec.ts` 本轮相关红线 grep 未发现 `test.skip`、`test.fixme`、`waitForTimeout()`、宽松阈值、retry 或永真断言。
