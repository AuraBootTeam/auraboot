...
这份文档覆盖当前“补齐真实测试和必要平台能力”的收口范围。DMN 决策表编辑器深化已在 Round 8 关闭 P1 闭环；灰度发布长窗口指标容量设计已在 Round 9 关闭真实栈闭环。FEEL 内置函数 parity、全量设计器极限矩阵仍属于后续独立 scope，不能用这份补测文档宣称已经完成。
...
| BPMN Designer | DONE | `designers.spec.ts` 覆盖 BPMN import/export JSON 与 version history；`bpm-designer/designer-node-property-matrix.spec.ts` 覆盖 start/user/service/gateway/callActivity/edge 属性矩阵 | 导入后 UI 回显 key/name；新增矩阵保存后 GET `/api/bpm/process-definitions/{pid}` 反查 node/edge config，并 deploy 后 GET `/bpmn` 反查 XML | JSON 文件名和 `key/name/description/nodes/edges`；BPMN XML 校验 `commandServiceTaskDelegate`、`calledElement`、`aura.callMappings`、conditionExpression | Round 7 退役旧 `designer/bpmn-node-properties.spec.ts`，由分层 targeted specs 承担覆盖 |
...
| Dashboard Designer | DONE | `dashboard-export.spec.ts` 覆盖保存后 export buttons 与 Excel export | 保存 dashboard 后导出读取当前 widget config | `{dashboardTitle}.xlsx`，校验 ZIP header、sheet、rows | PDF 内容矩阵属于后续增强，不阻塞 Excel artifact gap |
...
| 优先级 | 项 | 原因 |
|---|---|---|
| P2 | FEEL 内置函数 parity | 当前已覆盖 unary-test、range/list、date/time/datetime/duration 有序比较；`date(...)`、`duration(...)`、条件表达式函数白名单仍需独立 scope |
| P2 | 全量设计器极限矩阵 | Page/BPMN/Automation/Dashboard 已有代表性组件、属性、保存、导入/导出 artifact 闭环；但所有拖拽组件类型、全部属性组合、PDF/Excel/JSON 多格式组合和异常恢复矩阵仍应按独立 goal 推进 |
