...
| 优先级 | 项 | 原因 |
|---|---|---|
| P1 | 全模块字段目录从业务 model metadata 直接读取 | 当前 catalog 来自决策 fieldRefs，不能替代完整业务模型 metadata |
| P1 | SLA breach/escalation 全流程 UI + scheduler 长窗口测试 | 需要稳定时间推进/调度窗口设计，不能混在短 targeted slice |
