...
| BPMN Designer | DONE(targeted) | `designers.spec.ts` 覆盖 BPMN import/export JSON 与 version history | 导入后 UI 回显 key/name；导出 payload round-trip | JSON 文件名和 `key/name/description/nodes/edges` | `bpmn-node-properties.spec.ts` 是历史测试套件治理债，不再作为当前产品功能阻塞项 |
...
| 优先级 | 项 | 原因 |
|---|---|---|
| P2 | BPMN 节点属性历史套件治理 | `bpmn-node-properties.spec.ts` 规模大且仍有 wait/threshold/skip 历史债；当前 BPMN 产品闭环由 targeted designer specs 覆盖，旧套件需单独按节点类型分批收紧 |
