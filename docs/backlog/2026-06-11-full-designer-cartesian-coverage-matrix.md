...
| dashboard | smart-recent | title, maxItems | P1: recent visits data | DONE(saved-payload): 2026-06-12 `DW-071` proves UI click-add, static dataSource edit, representative `visualization.maxItems` edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. Recent-visits runtime data still tracked separately. |
