...
| dashboard | smart-shortcuts | title, columns | P1: shortcut interaction | DONE(saved-payload): 2026-06-12 `DW-070` proves UI click-add, static dataSource edit, representative `visualization.columns` edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. Shortcut runtime interaction still tracked separately. |
