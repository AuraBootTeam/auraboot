...
- widget types spec 的 `>=1` 假覆盖已关闭:2026-06-12 `dashboard-widget-types.spec.ts` 改为精确 palette 清单、add 后 widget 数量 `+1`,并在画布上用 `data-widget-type` 断言实际新增 widget 类型;新增 `DW-037..DW-050` 覆盖 14 个 named widget 的 UI 添加、static dataSource 编辑、代表属性编辑、Save API、`/api/dashboards/{pid}` 读回、exact `type/componentType/default size`。
- registry workbench widgets 是否需要逐类型 runtime 语义断言;当前 14 个 named widget suite 和 tab persistence 不能外推到全部 37 个 registry 类型。
- 14 个 named widget 现在已有 representative saved-payload / property value matrix;仍未覆盖每个 widget 的全部 `configSchema` 字段,也不能外推到 37 个 registry/workbench widget 或 runtime 语义。
...
| 2026-06-12 Dashboard widget payload typecheck + redline | `pnpm typecheck`; grep `dashboard-widget-types.spec.ts` for `test.skip`, `test.fixme`, `waitForTimeout`, threshold/retry, `page.request.put`, `__reactProps`, debug output | typecheck passed with only existing Vite tsconfig-paths warning; target spec grep has no output. New `createWidgetDraft` product helper compiles and is used by both click-to-add and drag-to-drop; `WidgetRenderer` exposes `data-widget-type` for exact UI type evidence; `WidgetPropertyPanel` exposes stable `widget-prop-*` testids for exact property editing evidence. |
...
| Dashboard Designer | Dashboard designer load, chart palette/config, 14 named widget types add/property-panel checks, data source binding, resize/settings/validation/publish/unpublish/layout, interactions, Excel XLSX artifact parse, PDF header/content marker. Evidence: Dashboard 64 pass from the broader suite, plus `dashboard-widget-types.spec.ts` `50 passed` in the isolated widget payload fresh run. 2026-06-12 added `dashboard-tab-reorder.spec.ts` fresh run with DTR-002/DTR-003 enabled, covering drag reorder and user-preference reload persistence. 2026-06-12 added `dashboard-management.spec.ts` fresh run with DM-E03/E04/E07/E08/E10/E11/DM-E12 enabled, covering row action visibleWhen, publish/unpublish/delete lifecycle, row/edit/create navigation, and All/Personal/Global tab filtering; spec-level page schema PUT patch removed and `onRowClick/detailUrl` lives in setup fixture source. 2026-06-12 added widget payload precision: `dashboard-widget-types.spec.ts` no longer uses `>=1` for add/count; every add assertion checks exact count `+1` and exact `data-widget-type`; `DW-037..DW-050` prove 14 named widget UI add -> static dataSource edit -> representative property edit -> save -> backend readback preserves exact registry type/componentType/default size/staticData/edited visualization config. Product fix: click-to-add and drag-to-drop now share `createWidgetDraft`, preserving registry `defaultConfig`; `WidgetPropertyPanel` exposes `widget-prop-*` testids for property edit evidence. | Registry workbench widgets outside the 14 named widget suite are not individually DONE. 14 named widget representative saved-payload matrix is closed, but this does not claim all 37 widget payload/property combinations, all configSchema fields, or runtime semantic rendering DONE. |
...
- 2026-06-12 Dashboard widget payload DONE 只覆盖 14 named widgets 的 palette/add/property presence 收紧,以及 `DW-037..DW-050` representative saved-payload matrix: UI click-add、static dataSource edit、单个代表属性 edit、Save click、`/api/dashboards/{pid}` readback、exact type/componentType/default size/staticData/edited config。`page.request.get('/api/dashboards/{pid}')` 只用于保存后的后端读回证据;核心配置动作是浏览器 UI click-add、property panel edit、static dataSource edit 和 Save click。第一次 `Saved Payload` 红灯来自默认 aggregate dataSource 缺少 model 导致无法形成有效保存响应,后续改为 UI static dataSource 设置;这是产品路径校验,不是测试 flaky。它不覆盖所有 37 registry widget,不覆盖每个 widget 的全部 configSchema 字段,也不覆盖 runtime visual/semantic rendering。
...
3. Dashboard / Report Designer：Dashboard tab persistence、management row action/list tab lifecycle、Dashboard Excel/PDF artifact、14 named widget add/property presence + representative saved payload、Report table + non-table static Excel/PDF artifact 已完成;下一轮集中补 registry workbench widgets、non-named registry widget saved-payload/runtime 语义、14 named widget 的 exhaustive configSchema per-field matrix、Report model/namedQuery/API dataSource artifact 证据。
