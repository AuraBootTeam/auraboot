...
| dashboard | smart-recent | title, maxItems | P1: recent visits data | DONE(saved-payload), runtime TODO: 2026-06-12 `DW-071` proves UI click-add, static dataSource edit, representative `visualization.maxItems` edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. 2026-06-12 earlier smart-recent runtime probe was not counted: `POST /api/user-engagement` returned OK, but immediate `GET engagementType=recent_view&targetType=page` did not include the created fixture, and returned Long `id` is serialized as a JS-unsafe number causing DELETE precision risk. Track as separate engagement API contract gap before closing recent-visits runtime. |
...
| 2026-06-12 Dashboard widget runtime targeted E2E | host stack `:6443/:3501/:5174`, `NO_PROXY=localhost,127.0.0.1 PLAYWRIGHT_BASE_URL=http://127.0.0.1:5174 BACKEND_URL=http://127.0.0.1:6443 BFF_PORT=3501 PW_QUICK_WORKERS=1 pnpm exec playwright test -c playwright.quick.config.ts tests/e2e/dashboard/dashboard-widget-runtime.spec.ts --project=chromium` | `DWR-001` first run red because content/embed fixture omitted static `dataSource`, causing legacy normalize to keep `visualization` nested and widgets to show unconfigured states; after aligning fixture to saved-payload contract, rerun passed. `DWR-002` first run red because the saved-feedback assertion only accepted English while the current UI renders `刚刚保存`; after accepting the localized saved label, rerun passed. `DWR-003` first run red because `normalizeDashboard` dropped authored `shortcuts`, so viewer rendered default shortcuts; after product fix, rerun passed. `DWR-004` first run red because the test oracle selected a 16px action-menu SVG instead of the chart surface; after scoping the oracle under `[data-widget-type] .echarts-for-react/canvas/svg`, rerun passed. Latest full rerun: `4 passed (5.9s)`. Browser opens `/dashboards/view/{code}` and asserts progress `75%`, leaderboard `9.8K`, rich text content, image/iframe DOM props, countdown labels, stats-row workbench mount, stats-card `inbox_pending` leaving loading placeholder, quick-note UI save -> `PUT /api/user-notes` -> feedback -> reload persistence, shortcut link click navigation to `/dashboards`, and advanced widget runtime surfaces/content for wordcloud/combo/nps/gallery/kanban. |
| 2026-06-12 Dashboard widget runtime typecheck + audit | `pnpm typecheck`; redline grep on `dashboard-widget-runtime.spec.ts` + `dashboardService.test.ts`; `node /Users/ghj/work/auraboot/auraboot-enterprise/scripts/page-golden-audit.mjs <absolute-spec-path>`; `git diff --check` | typecheck passed with only existing Vite tsconfig-paths warning; both redline grep commands had no output; page-golden-audit `PASSED` with `E2E-REQUEST-RATIO` warning (`UI interaction count 2` vs `page.request count 5`) because API is used for fixture create/publish/cleanup and quick-note original-value restore while core evidence is viewer DOM assertions, chart surface rendering, gallery/kanban content, stats-card loading exit, textarea UI save/reload, and shortcut link click navigation; diff hygiene passed. |
...
- 2026-06-12 Dashboard widget runtime DONE 只覆盖 `DWR-001` 的 8 个代表 widget: `smart-progress`, `smart-leaderboard`, `smart-rich-text`, `smart-image`, `smart-iframe`, `smart-countdown`, `smart-stats-row`, `smart-stats-card`。`page.request.post('/api/dashboards')` / `post('/publish')` / `delete` 只用于 deterministic fixture create/publish/cleanup,因为本轮验证对象是 published Dashboard Viewer consumer runtime,不是设计器 UI authoring;核心证据来自浏览器打开 `/dashboards/view/{code}` 后的 `data-widget-type`、业务文本、DOM 属性、workbench testid 和 loading placeholder exit 断言。page-golden-audit 的 `E2E-REQUEST-RATIO` warning 已分类为 setup-heavy runtime fixture 警告,不是 API 替代 viewer DOM 证据。
...
- 2026-06-12 `smart-recent` runtime 未计入 DONE。此前未计数的 smart-recent probe 显示 engagement API 仍有独立契约风险: `POST /api/user-engagement` 返回 OK 后,同一 storageState 下 `GET /api/user-engagement?engagementType=recent_view&targetType=page` 未读到刚创建的 fixture;同时 `id` 以 JSON number 返回,对 ASSIGN_ID Long 会在 JS 中丢精度,导致 `DELETE /api/user-engagement/{id}` 可能删除不到记录。该问题应先用后端/API contract test 收口,再补 viewer recent runtime E2E。
