...
| export | Excel / PDF / JSON if supported | file naming、content | download、parse、round-trip where supported | xlsx/pdf/json artifact | PARTIAL: Dashboard Excel/PDF done; Report table Excel/PDF artifacts done; Report non-table/dataSource export semantics remain TODO |
...
- Report 非 table block/dataSource 的导出语义仍未完成,不能外推为 Report export 全量 DONE。
...
| 2026-06-12 Report Excel/PDF backend unit | `./gradlew :test --tests com.auraboot.framework.bi.ReportExportServiceTest` in `platform` | `3 tests` passed; parses generated XLSX and asserts filename, content type, ZIP header, sheet title, header cells, and static data rows; parses generated PDF with `PDFTextStripper` and asserts title/table text; missing `extension.reportDsl` throws validation error |
...
| 2026-06-12 Report Operations E2E | same isolated stack, `report-designer-deep.spec.ts --project=chromium-deep --no-deps -g "Report Operations"` | `8 passed (2.1m)`; covers operation buttons, save API trigger, export button visibility, Report Excel artifact parse, and Report PDF artifact download/header/page marker |
| 2026-06-12 Report Excel/PDF e2e-truth redline | grep `report-designer-deep.spec.ts` for `test.skip`, `test.fixme`, `waitForTimeout`, threshold/retry, direct `/p/`, and `page.request.*` classification | no skip/fixme/fixed wait/threshold/retry/direct `/p/` output; `page.request.post('/api/pages')` creates deterministic report fixture and `page.request.delete` cleans it; core artifact evidence is browser export click + download + XLSX/PDF artifact assertions |
...
| Report Designer | 10 report block types (`data-table`, `grouped-table`, `stat-card`, `rich-text`, `cross-tab`, `chart`, `barcode`, `watermark`, `page-header`, `page-footer`) plus save/export operation smoke/deep. Evidence: Report 98 pass after fixed-wait removal. 2026-06-12 added Report table Excel artifact closure: `ReportExportServiceTest` backend workbook parse, `BffProxyService.test.ts` binary proxy regression, `RPT-OP-07` browser export click + download filename + ZIP header + `xlsx` sheet/header/rows parse, and Report Operations 7-pass run. 2026-06-12 added Report table PDF artifact closure: OSS `/api/reports/export/pdf`, backend PDFBox renderer + `PDFTextStripper` parse, `RPT-OP-08` browser export click + download filename + PDF header/page marker, and Report Operations 8-pass run. Product fixes: saved full Report DSL in `extension.reportDsl`, preserved PageSchema create extension, binary-proxied `POST /api/reports/export/excel|pdf`, and replaced the dangling `/api/print/render-html` frontend PDF call with saved-report export. | Report table Excel/PDF artifacts are DONE, but non-table block export semantics and model/namedQuery/API dataSource rows are still TODO. Do not claim Report export all-formats/all-blocks completion. |
...
- Report export 仍不能声明全量完成:grouped-table/cross-tab/stat/chart/rich-text/barcode/watermark/header/footer 的导出语义、以及 model/namedQuery/API dataSource rows 尚未补 byte-level artifact 证据。
...
3. Dashboard / Report Designer：Dashboard tab persistence、management row action/list tab lifecycle、Dashboard Excel/PDF artifact、Report table Excel/PDF artifact 已完成;下一轮集中补 registry workbench widgets、widget payload/property precision、Report 非 table block export 和 model/namedQuery/API dataSource artifact 证据。
