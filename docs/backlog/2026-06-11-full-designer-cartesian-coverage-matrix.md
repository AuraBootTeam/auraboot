...
| dashboard | smart-quick-note | title | P1: note persistence | DONE(saved-payload): 2026-06-12 `DW-073` proves UI click-add, static dataSource edit, title edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. Note persistence runtime still tracked separately. |
...
| 2026-06-12 Dashboard widget runtime targeted E2E | same isolated stack, `pnpm exec playwright test -c playwright.quick.config.ts tests/e2e/dashboard/dashboard-widget-runtime.spec.ts --project=chromium` | first run red because content/embed fixture omitted static `dataSource`, causing legacy normalize to keep `visualization` nested and widgets to show unconfigured states; after aligning fixture to saved-payload contract, rerun `1 passed (2.0s)`. Browser opens `/dashboards/view/{code}` and asserts progress `75%`, leaderboard `9.8K`, rich text content, image/iframe DOM props, countdown labels, and stats-row workbench mount. |
| 2026-06-12 Dashboard widget runtime typecheck + audit | `pnpm typecheck`; redline grep on `dashboard-widget-runtime.spec.ts`; `node /Users/ghj/work/auraboot/auraboot-enterprise/scripts/page-golden-audit.mjs <absolute-spec-path>`; `git diff --check` | typecheck passed with only existing Vite tsconfig-paths warning; redline grep no output; page-golden-audit `PASSED` with `E2E-REQUEST-RATIO` warning because API is used for fixture create/publish/cleanup while core evidence is viewer DOM assertions; diff hygiene passed. |
...
3. Dashboard / Report Designer：Dashboard tab persistence、management row action/list tab lifecycle、Dashboard Excel/PDF artifact、37 widget add/property presence + representative saved payload、Report table + non-table static Excel/PDF artifact、Report model/namedQuery/API Excel dataSource artifact 已完成;下一轮集中补 Dashboard widget runtime 语义、37 widget exhaustive configSchema per-field matrix、workbench live-data/interaction seam、Report PDF visual-fidelity / JSON round-trip export semantics。
