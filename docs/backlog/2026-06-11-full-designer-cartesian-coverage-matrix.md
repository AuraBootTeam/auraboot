...
- 2026-06-12 `smart-recent` runtime 已由 `DWR-005` 计入 representative DONE,但边界只覆盖 engagement API recent-page 数据进入 published viewer 后的 link title/href 渲染。此前未计数的 smart-recent probe 被拆成后端契约红绿和前端缓存稳定性: `UserEngagementControllerIntegrationTest` 证明 `POST -> GET recent_view` 同用户/租户读回成立,并修复 `ASSIGN_ID Long` `id` 以 JSON number 返回的 JS 精度风险;`DWR-005` 通过清理 localStorage recent cache、等待 viewer 的浏览器 `GET /api/user-engagement` 响应、再断言 DOM,避免把缓存旧值误判为产品缺陷。
...
- 2026-06-12 `smart-pipeline` / `smart-leads` / `smart-activities` runtime 已由 `DWR-008` / `DWR-010` / `DWR-011` / `DWR-012` 计入 representative DONE,但边界只覆盖 CRM starter live-data + canonical navigation + empty/unavailable frontend states + frontend stage/status/type variant rendering: command fixture -> API readback -> published viewer -> browser-triggered `/api/workbench/pipeline`、`/api/dynamic/crm_lead/list`、`/api/dynamic/crm_activity/list` -> DOM render,stage/lead/activity click -> `/p/...` route,以及 deterministic route fulfillment 覆盖六个 pipeline stages、四个 lead statuses、四个 activity types。`page.request.post('/api/plugins/import/import-directory-sync')` / command APIs / dashboard lifecycle / dynamic list readback 只用于 deterministic fixture setup/readback/cleanup;`DWR-011` / `DWR-012` 的 `page.route` 只用于 deterministic 空态/CRM unavailable/variant response injection,不替代 `DWR-008` 的 live-data happy path。它仍不覆盖 CRM permissions negative path。
