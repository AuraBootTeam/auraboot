...
| dashboard | smart-announcement | title | P1: announcement render | DONE(saved-payload): 2026-06-12 `DW-072` proves UI click-add, static dataSource edit, title edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. Announcement runtime render still tracked separately. |
...
- 2026-06-12 Dashboard widget runtime DONE 只覆盖 `DWR-001` / `DWR-004` / `DWR-005` 的 14 个代表 widget: `smart-progress`, `smart-leaderboard`, `smart-rich-text`, `smart-image`, `smart-iframe`, `smart-countdown`, `smart-stats-row`, `smart-stats-card`, `smart-wordcloud-chart`, `smart-combo-chart`, `smart-nps-chart`, `smart-gallery`, `smart-kanban`, `smart-recent`。`page.request.post('/api/dashboards')` / `post('/publish')` / `delete` 只用于 deterministic fixture create/publish/cleanup,因为本轮验证对象是 published Dashboard Viewer consumer runtime,不是设计器 UI authoring;核心证据来自浏览器打开 `/dashboards/view/{code}` 后的 `data-widget-type`、业务文本、DOM 属性、workbench testid、loading placeholder exit、chart surface、gallery/kanban content 和 recent link DOM 断言。page-golden-audit 的 `E2E-REQUEST-RATIO` warning 已分类为 setup-heavy runtime fixture 警告,不是 API 替代 viewer DOM 证据。
