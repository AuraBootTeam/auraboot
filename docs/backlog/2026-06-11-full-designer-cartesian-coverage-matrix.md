...
| dashboard | smart-pipeline | title | P1: workbench widget render | DONE(saved-payload + live CRM runtime): 2026-06-12 `DW-065` proves UI click-add, static dataSource edit, title edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. 2026-06-12 `DWR-008` imports/probes `crm-starter`, seeds account/lead/opportunity/activity through command APIs, verifies `/api/workbench/pipeline` contains the seeded `qualification` amount, opens published `/dashboards/view/{code}`, waits for the viewer browser `GET /api/workbench/pipeline`, asserts `smart-pipeline` renders the stage row, and proves no raw `workbench.pipeline.*` key leaks to the UI. Product fixes: Workbench pipeline now queries `crm_opp_expected_amount` and aligns stage order with CRM starter `discovery/qualification/proposal/negotiation/closed_won`. |
| dashboard | smart-leads | title | P1: workbench widget render | DONE(saved-payload + live CRM runtime): 2026-06-12 `DW-066` proves UI click-add, static dataSource edit, title edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. 2026-06-12 `DWR-008` verifies seeded lead readback through `/api/dynamic/crm_lead/list`, opens published viewer, waits for the viewer browser `GET /api/dynamic/crm_lead/list`, and asserts the unique lead company/contact render in `smart-leads`. Product fix: widget now uses canonical `/api/dynamic/crm_lead/list` instead of stale `/crm_lead/list`. |
| dashboard | smart-activities | title | P1: workbench widget render | DONE(saved-payload + live CRM runtime): 2026-06-12 `DW-067` proves UI click-add, static dataSource edit, title edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. 2026-06-12 `DWR-008` verifies seeded activity readback through `/api/dynamic/crm_activity/list`, opens published viewer, waits for the viewer browser `GET /api/dynamic/crm_activity/list`, and asserts the unique activity subject renders in `smart-activities`. Product fix: widget now uses canonical `/api/dynamic/crm_activity/list` and maps CRM starter `crm_act_*` fields. |
...
- 2026-06-12 `smart-pipeline` / `smart-leads` / `smart-activities` runtime 已由 `DWR-008` 计入 representative DONE,但边界只覆盖 CRM starter live-data seam: command fixture -> API readback -> published viewer -> browser-triggered `/api/workbench/pipeline`、`/api/dynamic/crm_lead/list`、`/api/dynamic/crm_activity/list` -> DOM render。`page.request.post('/api/plugins/import/import-directory-sync')` / command APIs / dashboard lifecycle / dynamic list readback 只用于 deterministic fixture setup/readback/cleanup;核心证据来自 viewer DOM 和 browser-triggered CRM API responses。它不覆盖 pipeline stage click navigation、lead/activity row click navigation、CRM permissions negative path 或所有 CRM stage/status visual variants。
