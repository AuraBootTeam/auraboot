...
| export | Excel / PDF / JSON if supported | file naming、content | download、parse、round-trip where supported | xlsx/pdf/json artifact | PARTIAL: Dashboard Excel/PDF done; Report table Excel artifact done; Report PDF remains TODO |
...
- Report PDF artifact、Report 非 table block/dataSource 的导出语义仍未完成,不能外推为 Report export 全量 DONE。
...
| report | table | dataSource, columns | P0: save/API/export artifact | DONE: 2026-06-12 `RPT-OP-07` browser click downloads parsable XLSX from saved `extension.reportDsl`; backend `ReportExportServiceTest` parses workbook title/header/rows; BFF binary proxy regression covered |
...
| Report Designer | `report-designer-deep.spec.ts` 的 fixed wait 已替换为 UI/API active wait,并与 `report-designer-smoke.spec.ts` fresh run 通过。2026-06-12 `RPT-OP-07` 从旧的“等待 export API 或 alert”升级为真实 browser download + xlsx parse;产品侧新增 OSS `/api/reports/export/excel`,并修复 PageSchema create extension 丢失与 BFF binary proxy 对 `POST /api/reports/export/excel` 的二进制转发。 | Report 10 类 block + 操作 smoke/deep 作为本轮证据;Report table Excel artifact 可计入 DONE;Report PDF artifact 和非 table export 语义仍是 TODO。 |
...
| 2026-06-12 Report Excel backend unit | `./gradlew :test --tests com.auraboot.framework.bi.ReportExportServiceTest` in `platform` | `2 tests` passed; parses generated XLSX and asserts filename, content type, ZIP header, sheet title, header cells, and static data rows; missing `extension.reportDsl` throws validation error |
...
| 2026-06-12 Report Operations E2E | same isolated stack, `report-designer-deep.spec.ts --project=chromium-deep --no-deps -g "Report Operations"` | `7 passed (1.8m)`; covers operation buttons, save API trigger, export button visibility, and Report Excel artifact parse |
| 2026-06-12 Report Excel e2e-truth redline | grep `report-designer-deep.spec.ts` for `test.skip`, `test.fixme`, `waitForTimeout`, threshold/retry, direct `/p/`, and `page.request.*` classification | no skip/fixme/fixed wait/threshold/retry/direct `/p/` output; `page.request.post('/api/pages')` creates deterministic report fixture and `page.request.delete` cleans it; core artifact evidence is browser export click + download + XLSX parse |
...
| Dashboard Designer | Dashboard designer load, chart palette/config, 14 named widget types add/property-panel checks, data source binding, resize/settings/validation/publish/unpublish/layout, interactions, Excel XLSX artifact parse, PDF header/content marker. Evidence: Dashboard 64 pass. 2026-06-12 added `dashboard-tab-reorder.spec.ts` fresh run with DTR-002/DTR-003 enabled, covering drag reorder and user-preference reload persistence. 2026-06-12 added `dashboard-management.spec.ts` fresh run with DM-E03/E04/E07/E08/E10/E11/DM-E12 enabled, covering row action visibleWhen, publish/unpublish/delete lifecycle, row/edit/create navigation, and All/Personal/Global tab filtering; spec-level page schema PUT patch removed and `onRowClick/detailUrl` lives in setup fixture source. | Registry workbench widgets outside the 14 named widget suite are not individually DONE. Widget type `>=1` assertions remain existence semantics, not full payload/property cartesian coverage. |
| Report Designer | 10 report block types (`data-table`, `grouped-table`, `stat-card`, `rich-text`, `cross-tab`, `chart`, `barcode`, `watermark`, `page-header`, `page-footer`) plus save/export operation smoke/deep. Evidence: Report 98 pass after fixed-wait removal. 2026-06-12 added Report table Excel artifact closure: `ReportExportServiceTest` backend workbook parse, `BffProxyService.test.ts` binary proxy regression, `RPT-OP-07` browser export click + download filename + ZIP header + `xlsx` sheet/header/rows parse, and Report Operations 7-pass run. Product fixes: saved full Report DSL in `extension.reportDsl`, preserved PageSchema create extension, and binary-proxied `POST /api/reports/export/excel`. | Report table Excel artifact is DONE, but Report PDF artifact, non-table block export semantics, and model/namedQuery/API dataSource rows are still TODO. Do not claim Report export all-formats/all-blocks completion. |
...
- Report export 仍不能声明全量完成:PDF artifact、grouped-table/cross-tab/stat/chart/rich-text/barcode/watermark/header/footer 的导出语义、以及 model/namedQuery/API dataSource rows 尚未补 byte-level artifact 证据。
...
3. Dashboard / Report Designer：Dashboard tab persistence、management row action/list tab lifecycle、Dashboard Excel/PDF artifact、Report table Excel artifact 已完成;下一轮集中补 registry workbench widgets、widget payload/property precision、Report PDF artifact、Report 非 table block export 和 model/namedQuery/API dataSource artifact 证据。
