...
|---|---|---|---|---|---|
| widget | KPI / table / chart / text / markdown / iframe if supported | title、dataSource、refresh、style | 添加、编辑、删除、复制、排序 | dashboard config | PARTIAL: Dashboard 37-widget add/saved-payload + exhaustive `configSchema` field presence matrix DONE; runtime semantics remain partial by widget family |
...
- registry / workbench widget 的 saved-payload representative matrix 已覆盖到 37 个类型;2026-06-12 `DWR-001` / `DWR-004` / `DWR-005` / `DWR-006` / `DWR-007` / `DWR-008` / `DWR-009` 已补 22 个代表 widget runtime visual/semantic/live-data evidence,但仍不能外推为 37 个类型逐项 runtime rendering DONE。
...
| dashboard | smart-pipeline | title | P1: workbench widget render | DONE(saved-payload + live CRM runtime + navigation/empty state): 2026-06-12 `DW-065` proves UI click-add, static dataSource edit, title edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. 2026-06-12 `DWR-008` imports/probes `crm-starter`, seeds account/lead/opportunity/activity through command APIs, verifies `/api/workbench/pipeline` contains the seeded `qualification` amount, opens published `/dashboards/view/{code}`, waits for the viewer browser `GET /api/workbench/pipeline`, asserts `smart-pipeline` renders the stage row, and proves no raw `workbench.pipeline.*` key leaks to the UI. 2026-06-12 `DWR-010` clicks `pipeline-stage-qualification` and asserts canonical `/p/crm_opportunity?filters=...crm_opp_stage=qualification`; `DWR-011` route-fulfills empty stages and asserts `pipeline-crm-unavailable` without raw i18n leakage. Product fixes: Workbench pipeline now queries `crm_opp_expected_amount`, aligns stage order with CRM starter, and uses `/p/crm_opportunity` for CRM navigation. |
| dashboard | smart-leads | title | P1: workbench widget render | DONE(saved-payload + live CRM runtime + navigation/empty state): 2026-06-12 `DW-066` proves UI click-add, static dataSource edit, title edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. 2026-06-12 `DWR-008` verifies seeded lead readback through `/api/dynamic/crm_lead/list`, opens published viewer, waits for the viewer browser `GET /api/dynamic/crm_lead/list`, and asserts the unique lead company/contact render in `smart-leads`. 2026-06-12 `DWR-010` clicks the seeded lead row and asserts `/p/crm_lead/view/{recordId}`; `DWR-011` route-fulfills an empty lead list and asserts `leads-empty` without raw i18n leakage. Product fixes: widget now uses canonical `/api/dynamic/crm_lead/list`, supports `pid/id/recordId`, exposes stable row testids, and navigates to the dynamic detail route. |
| dashboard | smart-activities | title | P1: workbench widget render | DONE(saved-payload + live CRM runtime + navigation/empty state): 2026-06-12 `DW-067` proves UI click-add, static dataSource edit, title edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. 2026-06-12 `DWR-008` verifies seeded activity readback through `/api/dynamic/crm_activity/list`, opens published viewer, waits for the viewer browser `GET /api/dynamic/crm_activity/list`, and asserts the unique activity subject renders in `smart-activities`. 2026-06-12 `DWR-010` clicks the seeded activity row and asserts `/p/crm_activity/view/{recordId}`; `DWR-011` route-fulfills an empty activity list and asserts `activities-empty` without raw i18n leakage. Product fixes: widget now uses canonical `/api/dynamic/crm_activity/list`, maps CRM starter `crm_act_*` fields, supports `pid/id/recordId`, exposes stable row testids, and falls back to the activity detail route when no related record fields exist. |
...
- 2026-06-12 `smart-pipeline` / `smart-leads` / `smart-activities` runtime 已由 `DWR-008` / `DWR-010` / `DWR-011` 计入 representative DONE,但边界只覆盖 CRM starter live-data + canonical navigation + empty/unavailable frontend states: command fixture -> API readback -> published viewer -> browser-triggered `/api/workbench/pipeline`、`/api/dynamic/crm_lead/list`、`/api/dynamic/crm_activity/list` -> DOM render,以及 stage/lead/activity click -> `/p/...` route。`page.request.post('/api/plugins/import/import-directory-sync')` / command APIs / dashboard lifecycle / dynamic list readback 只用于 deterministic fixture setup/readback/cleanup;DWR-011 的 `page.route` 只用于 deterministic 空态/CRM unavailable response injection,不替代 live-data happy path。它不覆盖 CRM permissions negative path 或所有 CRM stage/status visual variants。
