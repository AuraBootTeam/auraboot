...
- registry / workbench widget 的 saved-payload representative matrix 已覆盖到 37 个类型;仍需逐类型 runtime 语义断言,当前证据不能外推为 runtime visual/semantic rendering DONE。
...
| dashboard | smart-wordcloud-chart | title, shape, colorTheme, gridSize | P1: property panel + render | DONE: 2026-06-12 `DW-046` proves UI click-add, static dataSource edit, representative `visualization.colorTheme` property edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. |
| dashboard | smart-combo-chart | title, smooth, stack, dataZoom, axes | P1: property panel + render | DONE: 2026-06-12 `DW-047` proves UI click-add, static dataSource edit, representative `visualization.yAxisLeft.name` property edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. |
| dashboard | smart-nps-chart | title, scoreField, legend, ringWidth | P1: property panel + render | DONE: 2026-06-12 `DW-048` proves UI click-add, static dataSource edit, representative `visualization.ringWidth` property edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, default `NPS` title, and default size. |
| dashboard | smart-gallery | title, columns, image/title/description fields | P1: property panel + render | DONE: 2026-06-12 `DW-049` proves UI click-add, static dataSource edit, representative `visualization.columns` property edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. |
| dashboard | smart-kanban | title, group/title/description fields | P1: property panel + render | DONE: 2026-06-12 `DW-050` proves UI click-add, static dataSource edit, representative `visualization.groupField` property edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. |
...
| 2026-06-12 Dashboard widget runtime targeted E2E | same isolated stack, `pnpm exec playwright test -c playwright.quick.config.ts tests/e2e/dashboard/dashboard-widget-runtime.spec.ts --project=chromium` | `DWR-001` first run red because content/embed fixture omitted static `dataSource`, causing legacy normalize to keep `visualization` nested and widgets to show unconfigured states; after aligning fixture to saved-payload contract, rerun passed. `DWR-002` first run red because the saved-feedback assertion only accepted English while the current UI renders `刚刚保存`; after accepting the localized saved label, rerun passed. `DWR-003` first run red because `normalizeDashboard` dropped authored `shortcuts`, so viewer rendered default shortcuts; after product fix, rerun passed. Latest rerun after adding stats-card runtime check: `3 passed (4.5s)`. Browser opens `/dashboards/view/{code}` and asserts progress `75%`, leaderboard `9.8K`, rich text content, image/iframe DOM props, countdown labels, stats-row workbench mount, stats-card `inbox_pending` leaving loading placeholder, quick-note UI save -> `PUT /api/user-notes` -> feedback -> reload persistence, and shortcut link click navigation to `/dashboards`. |
| 2026-06-12 Dashboard widget runtime typecheck + audit | `pnpm typecheck`; redline grep on `dashboard-widget-runtime.spec.ts` + `dashboardService.test.ts`; `node /Users/ghj/work/auraboot/auraboot-enterprise/scripts/page-golden-audit.mjs <absolute-spec-path>`; `git diff --check` | typecheck passed with only existing Vite tsconfig-paths warning; redline grep no output; page-golden-audit `PASSED` with `E2E-REQUEST-RATIO` warning because API is used for fixture create/publish/cleanup and quick-note original-value restore while core evidence is viewer DOM assertions, stats-card loading exit, textarea UI save/reload, and shortcut link click navigation; diff hygiene passed. |
...
3. Dashboard / Report Designer：Dashboard tab persistence、management row action/list tab lifecycle、Dashboard Excel/PDF artifact、37 widget add/property presence + representative saved payload、stats-card runtime render、quick-note runtime persistence、shortcut runtime navigation、Report table + non-table static Excel/PDF artifact、Report model/namedQuery/API Excel dataSource artifact 已完成;下一轮集中补 Dashboard widget runtime 语义、37 widget exhaustive configSchema per-field matrix、其它 workbench live-data/interaction seam、Report PDF visual-fidelity / JSON round-trip export semantics。
