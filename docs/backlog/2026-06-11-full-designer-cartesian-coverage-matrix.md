...
- 2026-06-12 Page Designer workbench DONE 只覆盖 schema-level static dataSource + `/p/c/{pageKey}` runtime for 7 workbench block types,以及 state/context/dataSource 三路联动;不代表 Page Designer 全 17 个 block 类型完成。
...
2. Page Designer：因为 DSL 页面和 typed custom block 依赖它；下一轮优先补剩余 10 个非 workbench block 的运行时语义,不要把本轮 7 个 workbench DONE 外推为 17/17。
