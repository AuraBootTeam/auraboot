...
| dashboard | smart-inbox | title, maxItems, itemTypes | P1: workbench widget render | DONE(saved-payload): 2026-06-12 `DW-063` proves UI click-add, static dataSource edit, representative `visualization.maxItems` edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. Workbench runtime render still tracked separately. |
| dashboard | smart-calendar | title | P1: workbench widget render | DONE(saved-payload): 2026-06-12 `DW-064` proves UI click-add, static dataSource edit, title edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. Workbench runtime render still tracked separately. |
...
- 2026-06-12 Dashboard widget runtime DONE 只覆盖 `DWR-001` / `DWR-004` / `DWR-005` / `DWR-006` 的 15 个代表 widget: `smart-progress`, `smart-leaderboard`, `smart-rich-text`, `smart-image`, `smart-iframe`, `smart-countdown`, `smart-stats-row`, `smart-stats-card`, `smart-wordcloud-chart`, `smart-combo-chart`, `smart-nps-chart`, `smart-gallery`, `smart-kanban`, `smart-recent`, `smart-announcement`。`page.request.post('/api/dashboards')` / `post('/publish')` / `delete` 只用于 deterministic fixture create/publish/cleanup,因为本轮验证对象是 published Dashboard Viewer consumer runtime,不是设计器 UI authoring;核心证据来自浏览器打开 `/dashboards/view/{code}` 后的 `data-widget-type`、业务文本、DOM 属性、workbench testid、loading placeholder exit、chart surface、gallery/kanban content、recent link DOM 和 announcement title/content/pinned/urgent badge DOM 断言。page-golden-audit 的 `E2E-REQUEST-RATIO` warning 已分类为 setup-heavy runtime fixture 警告,不是 API 替代 viewer DOM 证据。
...
3. Dashboard / Report Designer：Dashboard tab persistence、management row action/list tab lifecycle、Dashboard Excel/PDF artifact、37 widget add/property presence + representative saved payload、15 representative widget runtime render、quick-note runtime persistence、shortcut runtime navigation、recent-visit runtime data rendering、announcement runtime data rendering、Report table + non-table static Excel/PDF artifact、Report model/namedQuery/API Excel dataSource artifact 已完成;下一轮集中补 remaining Dashboard widget runtime 语义、37 widget exhaustive configSchema per-field matrix、其它 workbench live-data/interaction seam、Report PDF visual-fidelity / JSON round-trip export semantics。
