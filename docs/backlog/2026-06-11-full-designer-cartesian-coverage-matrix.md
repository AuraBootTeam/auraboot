...
- registry / workbench widget 的 saved-payload representative matrix 已覆盖到 37 个类型;2026-06-12 `DWR-004` 已补 `smart-wordcloud-chart` / `smart-combo-chart` / `smart-nps-chart` / `smart-gallery` / `smart-kanban` 代表 runtime visual/semantic evidence,但仍不能外推为 37 个类型逐项 runtime rendering DONE。
...
| dashboard | smart-my-process | title, maxItems | P1: BPM workbench seam | DONE(saved-payload): 2026-06-12 `DW-068` proves UI click-add, static dataSource edit, representative `visualization.maxItems` edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. BPM workbench runtime seam still tracked separately. |
| dashboard | smart-process-stats | title | P1: BPM workbench seam | DONE(saved-payload): 2026-06-12 `DW-069` proves UI click-add, static dataSource edit, title edit, Save API, `/api/dashboards/{pid}` readback, exact `type/componentType`, and default size. BPM workbench runtime seam still tracked separately. |
...
- 2026-06-12 Dashboard widget runtime DONE 只覆盖 `DWR-001` / `DWR-004` / `DWR-005` / `DWR-006` / `DWR-007` / `DWR-008` 的 20 个代表 widget: `smart-progress`, `smart-leaderboard`, `smart-rich-text`, `smart-image`, `smart-iframe`, `smart-countdown`, `smart-stats-row`, `smart-stats-card`, `smart-wordcloud-chart`, `smart-combo-chart`, `smart-nps-chart`, `smart-gallery`, `smart-kanban`, `smart-recent`, `smart-announcement`, `smart-inbox`, `smart-calendar`, `smart-pipeline`, `smart-leads`, `smart-activities`。`page.request.post('/api/dashboards')` / `post('/publish')` / `delete` 只用于 deterministic fixture create/publish/cleanup,因为本轮验证对象是 published Dashboard Viewer consumer runtime,不是设计器 UI authoring;核心证据来自浏览器打开 `/dashboards/view/{code}` 后的 `data-widget-type`、业务文本、DOM 属性、workbench testid、loading placeholder exit、chart surface、gallery/kanban content、recent link DOM、announcement title/content/pinned/urgent badge DOM、inbox row/type badge DOM、calendar red dot DOM、CRM pipeline stage row、CRM lead company/contact、CRM activity subject 和 browser-triggered CRM API responses。page-golden-audit 在本 checkout 缺失,因此 `DWR-008` 不声明该 gate 通过;已有 `E2E-REQUEST-RATIO`/API-heavy warning 均分类为 setup-heavy runtime fixture 警告,不是 API 替代 viewer DOM 证据。
...
3. Dashboard / Report Designer：Dashboard tab persistence、management row action/list tab lifecycle、Dashboard Excel/PDF artifact、37 widget add/property presence + representative saved payload、20 representative widget runtime render、quick-note runtime persistence、shortcut runtime navigation、recent-visit runtime data rendering、announcement runtime data rendering、inbox/calendar runtime data rendering、CRM pipeline/leads/activities runtime data rendering、Report table + non-table static Excel/PDF artifact、Report model/namedQuery/API Excel dataSource artifact 已完成;下一轮集中补 remaining Dashboard widget runtime 语义、37 widget exhaustive configSchema per-field matrix、剩余 BPM workbench live-data/interaction seam(`smart-my-process` / `smart-process-stats`),以及 `smart-pipeline`/`smart-leads`/`smart-activities` 的 click-navigation/negative-path 深水区、Report PDF visual-fidelity / JSON round-trip export semantics。
