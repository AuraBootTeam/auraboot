...
### 4.2 Gap
A-G1 两 traceId 不通(🟡 **ab_ai_trace 已桥接** `94b6ef0a3` + 审计三表已 stamp,**仅 Kafka 跨消息待接**)/ A-G2 ✅ **审计三表 trace_id/span_id stamp 已接**(`c12dd028f` command/`d3be6a867` admin/`8efbc05e6` query;列 `49da15ebe`)/ A-G3 ✅ 日志已带 traceId(`ed7f823d2`)/ A-G4 Kafka 断链(🟡 **producer 侧已注入 W3C traceparent** `a53d24cce`,单测 3/3;consumer 侧跨语言 round-trip IT 待做)/ A-G5 ✅ **P0 已实证(见 §4.1)** / A-G6 🟡 **GenAiUsageRecord 成本账本(部分)**(`d52d1e189`:`ab_gen_ai_usage` 迁移 + 实体/mapper + `GenAiPricing` 单测 4/4 + recordGeneration 捕获已接)——**golden 抓到捕获点 gap(endgame 价值)**:真栈 chat 真调 DeepSeek 但 `ab_ai_trace` tokens=0、无 generation span → **streaming chat 主 generation 不经 `recordGeneration`(只 tool-loop 经)**;**streaming 捕获已接**(`949045ae9`:共享 `GenAiUsageRecorder`〔DRY〕+ `ChatTurnRuntime.streamProviderResponse` aggregate 后捕获)但 **re-golden 又 0 行**——二次抓到真因:**streaming/reactor 线程 `MetaContext` tenant=null**(§2.6 异步丢上下文,同 ab_ai_trace 桥接坑),recorder 静默 skip。后续两轮 golden(tenant fallback `selectTenantByTraceId` + 修 streamProvider 传真 traceId `2e6eb44fe`)**仍 0 行** → **5 轮 golden(obs-golden-54..58)锁定真根因**:默认 AuraBot chat 有 tools,走 **tool-loop adapter 路径**(`AuraBotChatService:457` tools 非空分支),绕过 `recordGeneration` 与 `streamProvider` 两个插桩点。**正确架构修复 = 在 `LlmProvider.chat/streamChat` chokepoint 装饰捕获**(所有 LLM 调用路径共经此接口;reactive `Flux<LlmChunk>` tap 取 stream-end usage + 显式 tenant 穿线避 §2.6),非逐路径补。**已交付**:schema/实体/mapper/`GenAiPricing`〔单测 4/4〕/`GenAiUsageRecorder`〔DRY 共享〕/no-tool streamProvider 路径捕获。**剩 chokepoint decorator(§12)**。
### 4.3 目标
...
| **A-G6 LLM 成本账本** 🟡 `d52d1e189`(部分;golden 抓到 gap) | `ab_gen_ai_usage`+`GenAiPricing`(单测 4/4)+ recordGeneration 捕获已接 | **golden(2026-06-20 起栈 obs-golden-55)抓到**:真栈 chat 真调 DeepSeek 但 trace tokens=0、`ab_gen_ai_usage` 0 行 → **streaming chat 主 generation 不经 `recordGeneration`(只 tool-loop 经)**。**修复**:在 `AuraBotChatService` streaming LLM-response(stream-end usage,从 `LlmChatResponse` 取 input/output/cache tokens)处补 usage 捕获 → 再 golden 断言 `ab_gen_ai_usage` 行。〔A-G1 桥接此轮 golden 复验无回归:otel_trace_id 仍正确写入〕 |
