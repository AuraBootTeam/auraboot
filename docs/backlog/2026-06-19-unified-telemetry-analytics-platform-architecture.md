...
| **M1 行为采集底座** 🟢 后端 ingestion 已 golden(`b8b41d582`) | **已交付**:`/api/collect`(`BehaviorCollectController/Service`)+ `ab_behavior_event` PG store(§5.5 冻结 event-first 信封:event_name/category、`ui_element_id` 稳定键、interaction_id/caused_by、trace_id、props jsonb)+ server 补全 tenant/user + 幂等 unique(tenant_id,event_id)。**剩**:`@aura/track` 前端 SDK + 自动 pageview/click + `aura.behavior.events.v1` Kafka 解耦层 + UV/PV dashboard(DSL)+ server outcome publisher(outbox) | **golden(obs-golden-64)**:pageview+click 批 → `{accepted:2}` 2 行(tenant 补全、ui_element_id=crm.lead-list.toolbar.create、interaction_id 串因果、props 真 jsonb);幂等重发 accepted:1 总行仍 2 ✅。剩前端 SDK/dashboard 真浏览器 golden 待 |
