...
| **A-G6 LLM 成本账本** ✅ `d52d1e189` | `ab_gen_ai_usage`(billing 权威,§2.5)+ `GenAiPricing` 价格表(纯函数单测 4/4)+ recordGeneration 捕获(tenant/trace/model/tokens/computed cost);`:compileJava` 过 | **consolidated golden 下一步(host stack 起栈一次,验多项)**:① chat turn → 断言 `ab_gen_ai_usage` 行(tokens+cost)〔A-G6〕② seed model+命令/查询 → 审计三表 `trace_id`==`X-Trace-Id`〔B1 闭合〕 |
