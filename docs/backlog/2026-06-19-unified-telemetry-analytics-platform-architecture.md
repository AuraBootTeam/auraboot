...
| **M1 行为采集底座** 🟢 后端 ingestion 已 golden(`b8b41d582`) | **已交付**:`/api/collect`(`BehaviorCollectController/Service`)+ `ab_behavior_event` PG store(§5.5 冻结 event-first 信封:event_name/category、`ui_element_id` 稳定键、interaction_id/caused_by、trace_id、props jsonb)+ server 补全 tenant/user + 幂等 unique(tenant_id,event_id)。**UV/PV 分析 API 已交付**(`abd96c60f`:`GET /api/analytics/behavior/overview`〔PV/UV/sessions/total〕+`/top-events`,tenant-scoped)。**剩**:`@aura/track` 前端 SDK + 自动 pageview/click + `aura.behavior.events.v1` Kafka 解耦层 + UV/PV dashboard(DSL)+ server outcome publisher(outbox)+ **匿名/未登录 /api/collect**(真匿名多访客 UV 需,当前仅 authenticated) | **golden**:obs-golden-64 ingestion(2 行,props jsonb,幂等);obs-golden-65 分析 API overview `{total:6,pv:4,uv:1,sessions:2}` == DB 逐字段、top-events 正确(UV=1 因 6 事件同一登录 user,COALESCE 取 user_id;匿名多访客 UV 待 anon collect)。剩前端 SDK/dashboard 真浏览器 golden 待 |
