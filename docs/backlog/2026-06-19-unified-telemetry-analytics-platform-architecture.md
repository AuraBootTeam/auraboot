...
| **A-G6 成本→trace 关联** 🟡 `09ed25fb2`(分析 API ✅ `49f49714d`) | 分析层 `GET /api/ai/usage/summary`(per-model rollup,golden obs-golden-60 API==DB)✅;`UsageRecordingLlmProvider` 经 Micrometer `Tracer` best-effort 盖 usage.trace_id | **golden(obs-golden-61)**:同步 LLM 调用点(scoring/NL-modeling 请求线程 span 活跃)trace_id 可盖;**async AuraBot chat 仍 null**(X-Trace-Id 已设但 span 未传播到 async turn 线程,只 MetaContext 重设——§2.6,同 A-G1)。**修复**:把 otelTraceId 像 A-G1(`ChatRequest.otelTraceId`)穿进 `LlmChatRequest` 供 decorator 读。primary 成本账本不受影响(已 GOLDEN) |
