...
### 4.2 Gap
A-G1 两 traceId 不通(🟡 **ab_ai_trace 已桥接** `94b6ef0a3` + 审计三表已 stamp,**仅 Kafka 跨消息待接**)/ A-G2 ✅ **审计三表 trace_id/span_id stamp 已接**(`c12dd028f` command/`d3be6a867` admin/`8efbc05e6` query;列 `49da15ebe`)/ A-G3 ✅ 日志已带 traceId(`ed7f823d2`)/ A-G4 Kafka 断链(🟡 **producer 侧已注入 W3C traceparent** `a53d24cce`,单测 3/3;consumer 侧跨语言 round-trip IT 待做)/ A-G5 ✅ **P0 已实证(见 §4.1)** / A-G6 ✅ **LLM 成本账本 GOLDEN**(`d0fe00e6f`):**6 轮真栈 golden(obs-golden-54..59)逐层定位**正确架构修复 = **`LlmProvider.chat/streamChat` chokepoint 装饰** `UsageRecordingLlmProvider`(由 `LlmProviderFactory.getProvider` 应用,覆盖所有 LLM 路径〔no-tool / tool-loop / continuation / scoring / NL modeling〕;tenant seam-snapshot 避 §2.6,`Flux<LlmChunk>` tap 取 stream-end usage)。**golden-green 实证**:真 DeepSeek chat → `ab_gen_ai_usage` 真行(tenant `326666819614150656`、deepseek-chat、in 1132 / out 28、**amount 0.000336 USD**=`GenAiPricing` 算、pricing_version 2026-06-20),无重复计数(1 chat = 1 行)。配套:`ab_gen_ai_usage` 迁移 + `GenAiPricing`〔纯函数单测 4/4〕+ `GenAiUsageRecorder`〔DRY〕。**分析层 query API 已交付**(`49f49714d`:`GET /api/ai/usage/summary` per-model token/cost rollup,tenant-scoped;golden obs-golden-60 API 返值逐字段==DB 真值〔deepseek-chat 2 calls / 2242 in / 43 out / 0.000652〕,并抓修一个 nullable-param Postgres SQL bug)。〔过程:前 5 轮 golden 依次排除 recordGeneration-only → streaming 未捕获 → tenant-null → traceId-null,逐步定位默认 chat 走 tool-loop adapter 绕过 per-path 插桩,详 commit `d52d1e189`/`949045ae9`/`2e6eb44fe`/`d0fe00e6f`〕
### 4.3 目标
...
| 切片 | 精确触点 | 验证 |
|---|---|---|
| **P1 trace 关联 ✅ 完整 feature** | ab_ai_trace 桥接 `94b6ef0a3`(✅ 运行时验证)+ 审计三表 stamp `c12dd028f`/`d3be6a867`/`8efbc05e6`(编译过;`Tracer.currentSpan()` 同步请求线程,机制同已验桥接)+ 日志 `ed7f823d2` + 迁移 `49da15ebe` | 审计三表完整路径验证(请求→审计行 `trace_id`==`X-Trace-Id`)待 seed model+命令/查询触发——**独立设置任务,非代码缺口**;ab_ai_trace 桥接已端到端验证 |
| **A-G6 LLM 成本账本** ✅ GOLDEN `d0fe00e6f` | `LlmProvider.chat/streamChat` chokepoint 装饰 `UsageRecordingLlmProvider`(`LlmProviderFactory.getProvider` 应用,覆盖全 LLM 路径;tenant seam-snapshot + Flux tap)+ `ab_gen_ai_usage` 迁移 + `GenAiPricing`(单测 4/4)+ `GenAiUsageRecorder` | **golden-green(round 6,obs-golden-59)**:真 DeepSeek chat → `ab_gen_ai_usage` 行(tenant 326666819614150656、deepseek-chat、in1132/out28、amount 0.000336 USD、pricing_version 2026-06-20),无重复计数(1 chat=1 行)。6 轮 golden 定位见 §4.2 |
| **A-G6 成本→trace 关联** ✅ GOLDEN `085b8b23f`(分析 API `49f49714d`) | 分析层 `GET /api/ai/usage/summary`(per-model rollup)✅;**§2.6 seam-snapshot**:otelTraceId 存入 `MetaContext.OTEL_TRACE_ID`(`AuraBotController` 在 `MetaContext.setContext` 旁设,stream+resume 两 worker),`UsageRecordingLlmProvider` 优先读它(async worker 线程可用)+ fallback active span(同步调用点) | **golden(obs-golden-62,真 DeepSeek chat)**:usage.trace_id == X-Trace-Id(af81e6cd…)**MATCH**,且 JOIN `ab_gen_ai_usage.trace_id == ab_ai_trace.otel_trace_id` 成功 → 成本行可关联到会话 trace + 鹰眼全图。**A-G6 端到端闭环**:捕获 + 分析 API + trace 关联全 GOLDEN |
| **统一关联视图(鹰眼)** ✅ GOLDEN `659ced991` | `GET /api/observability/correlation/{traceId}`(`CorrelationController/QueryService`)按 `trace_id` JOIN 三域:LLM 成本(`ab_gen_ai_usage`)+ 行为(`ab_behavior_event`)+ 审计(`ab_admin_event_log`),tenant-scoped。整平台核心价值=一个 trace → 关于它的全部 | **golden(obs-golden-67)**:真 chat(trace T=429c6268…)写 5 LLM usage 行(tool-loop 每次调用各带 cost,全在 T 下→证 chokepoint 捕获每次)+ POST behavior(trace_id=T)→ 视图返 `{llmUsage:5, behaviorEvents:1〔ask_clicked〕, auditEvents:0}` 全按 T 关联 ✅。**进程内域鹰眼闭环** |
| **B1 审计三表 golden** 🟢 admin 已验 | command/admin/query 审计 trace_id stamp(`c12dd028f`/`d3be6a867`/`8efbc05e6`) | **golden(obs-golden-63)**:`POST /api/admin/environments/{pid}/lock` → `ab_admin_event_log`(action_type=environment.lock,success=t,**trace_id=dfc8218f… == X-Trace-Id MATCH**,span_id 有)✅。command/query 审计用**同一请求线程 `Tracer.currentSpan()` 模式**(写在请求线程 span 活跃处,机制同 admin 已证)——full command/query 触发需 seed model+命令(独立设置任务,低风险) |
| **A-G4 Kafka 透传** 🟡 producer ✅ `a53d24cce` | **producer 侧已做**:`AuraEventBus.forwardToMq` 注入 W3C `traceparent`(纯 `W3cTraceparent` helper,单测 3/3;KafkaMqProvider 保持哑传输不加 tracing 依赖)。剩 **consumer 侧续 span**(在下游跨语言 worker)+ **跨语言 round-trip 契约测试**(MQ=kafka,Kafka 红线) | round-trip IT:trace 过 Kafka 不断 |
...
| **M1 行为采集底座** 🟢 shipped through Kafka decouple(PR #1023,merge `4e2bef61e`) | **已交付**:`/api/collect` + `/api/collect/keyed`; `@auraboot/track` public SDK / global bundle; keyed site registry; `ab_behavior_event` PG store(§5.5 event-first 信封,event_name/category,`ui_element_id`,interaction_id/caused_by,trace_id,props jsonb); server 补全 tenant/user 或 site-key 解析 tenant/anon; 幂等 unique(tenant_id,event_id); **enqueue → `aura.behavior.events.v1` → async consumer → PG**; malformed/constraint → `aura.behavior.quarantine.v1` → `ab_behavior_quarantine`; UV/PV 分析 API + dashboard golden。**剩余独立线**:server outcome publisher(outbox) + M2 UI 元素身份治理 / ClickHouse / real-time stream 等后续 | **golden / IT**:obs-golden-64/65; SP3/SP4 browser goldens; PR #1023 post-merge local no-Docker回归: `check-schema-sql.sh --local`, compileJava/compileTestJava, targeted behavior+tenant-ignore tests, native Kafka IT(when broker available), 9 behavior Playwright goldens, track SDK 19 vitest + global build/env-lint。后续增强见 `docs/backlog/2026-06-22-behavior-kafka-post-merge-followups.md` |
