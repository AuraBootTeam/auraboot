...
| OSS branch | `codex/watch-field-history-pid` |
...
| Current merge rule | Fresh OSS and enterprise gates are green against the same final artifact. Stage, commit, push, and PR may proceed; do not merge to main until PR topology and review/CI are clean. |
...
| OSS frontend unit | `pnpm --dir web-admin test:unit:run` via pnpm `9.15.9` shim | `477` files / `4885` tests / `0` failed. |
| OSS schema SQL | `check-schema-sql.sh --local` with Postgres 17 psql on PATH | `schema.sql` 10674 lines, 312 expected CREATE TABLEs, 313 tables created, clean apply. |
| OSS schema drift | `scripts/db/check-schema-drift.sh --edition oss` | Committed snapshot matches Flyway result at `v20260623001000`. |
...
| G1 | `DONE` | OSS backend full final | OSS full v16 on `aura_public_record_pid_oss_full_20260624_v16`: `BUILD SUCCESSFUL in 20m 49s`; XML `1510` files / `12021` tests / `67` skipped / `0` failures / `0` errors. | Use v16 as current OSS backend full evidence. |
...
| G5 | `DONE` | Schema and migration gates | Final schema SQL and drift gates passed with Postgres 17 psql on PATH. | Schema artifacts match Flyway result at `v20260623001000`. |
...
| G11 | `IN_PROGRESS` | PR topology | Existing OSS PR #1060 remains stacked on #1059; enterprise docs PR #657 exists; enterprise consumer PR is not created yet. | Stage/commit/push this final state, then update existing OSS/docs PRs and create the enterprise consumer PR. Do not merge to main until review/CI/topology are clean. |
...
| G13 | `IN_PROGRESS` | Final commit/merge gate | Worktrees are verification-green and still dirty. | Stage/commit/push/PR now; merge only after PR topology/review/CI are clean. |
...
2. Stage/commit/push OSS, enterprise standards, and enterprise consumer
   branches separately.
3. Update existing OSS PR #1060 and enterprise standards PR #657 through push.
4. Create the enterprise consumer PR.
5. Merge only after PR topology/review/CI are clean.
