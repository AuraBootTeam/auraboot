  ...
  for (const s of p.scenarios) {
    ...
    try {
      ...
      if (s.sid.endsWith('-DETAIL')) {
        const text = await bodyText();
        if (/暂无|为空|没有/.test(text)) {
          await shot(s.shotId);
          s.verdict = 'pass';
          s.obs = 'empty list — detail N/A';
          continue;
        }
        let opened = false;
        for (const label of ['查看', '编辑', '详情']) {
        ...
        await shot(s.shotId);
        s.verdict = opened ? 'pass' : 'fail';
        s.obs = opened ? 'detail entry opened (localized action)' : 'no detail entry found on non-empty page';
        continue;
      }
    } catch (e) {
      s.verdict = 'fail';
