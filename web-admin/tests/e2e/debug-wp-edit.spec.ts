...
test('debug wp edit URL', async ({ page, request }) => {
  ...
  if (pid) {
    await page.goto(`/dynamic/ap-work-package/${pid}/edit`, { waitUntil: 'domcontentloaded' });
    console.log('URL after goto:', page.url());
    await page.waitForTimeout(2000);
    console.log('URL after wait:', page.url());
    const heading = await page.locator('h1, h2, [data-testid="dynamic-page-edit"]').first().textContent().catch(() => 'not found');
