...
function findTask(recs: any[], projId: string, fileId: string): any | undefined {
  return recs.find(
    (r: any) =>
      String(r.bom_task_project_id || '') === String(projId) ||
      String(r.bom_task_raw_file_id || '') === String(fileId),
  );
}
...
async function countStandardLines(page: Page, taskId: string): Promise<number> {
  ...
  const records = body?.data?.records || body?.data?.data?.records || body?.data || [];
...
test.describe('Quote/BOM bad-file convert + permissions page (BOM-04 / SYS-04) @smoke', () => {
  ...
  test('BOM-04 bad file conversion fails / surfaces an error (not silent success)', async ({
    browser,
  }) => {
    const { context, page } = await openQuoteRolePage(browser, users['eng']);
    try {
      const proj = await post(page, 'bom:create_project', {
        bom_project_name: `Bad ${uid}`,
        bom_pcba_code: `BAD-${uid}`,
      });
      ...
      const conv = await post(page, 'bom:start_conversion', {
        ...
        bom_task_source_package: 'badfile_neg',
      ...
      if (conv.status < 400) {
        ...
        await expect
          .poll(
            async () => {
              ...
              if (mine) {
                ...
                taskId = String(mine.pid || mine.id || '');
                if (
                  /^(failed|error|rejected|invalid|adjustment_required|format_exploration_required)$/i.test(
                    st,
                  )
                ) {
                  return st;
                }
                if (/^(completed|succeeded|success)$/i.test(st)) return `unexpected:${st}`;
              }
              return '';
            },
          ...
          .toMatch(
            /^(failed|error|rejected|invalid|adjustment_required|format_exploration_required)$/i,
          );
        expect(taskId, 'BOM-04: rejected conversion still exposes an auditable task').toBeTruthy();
        expect(
          await countStandardLines(page, taskId),
          'BOM-04: junk workbook creates no usable standard lines',
        ).toBe(0);
      }
      expect(
        conv.status >= 400 || conv.status === 200,
        'BOM-04: start is either rejected immediately or accepted for audited async rejection',
      ).toBeTruthy();
    } finally {
      await context.close();
    }
  ...
  test('SYS-04 admin permissions page renders role/capability management', async ({ browser }) => {
    ...
    try {
      ...
      const hasSurface =
        /权限|角色|能力|capabilit|role|permission|分配|授权/i.test(text) ||
        (await page.getByRole('combobox').count()) > 0 ||
        (await page
          .locator(
            'table tbody tr, [role="row"], [data-testid*="capability"], [data-testid*="role"]',
          )
          .count()) > 0;
      expect(
        hasSurface,
        'SYS-04: role/capability management surface visible to admin',
      ).toBeTruthy();
    } finally {
      await ctx.close();
    }
  ...
  test('SYS-04b non-admin is denied the capability-assignment API', async ({ browser }) => {
    ...
    try {
      ...
      const candidates = [
        {
          url: '/api/permission/capabilities',
          method: 'put',
          body: { roleCode: 'qo_sales', capabilityCodes: ['bom.cap.library_manage'] },
        },
        {
          url: '/api/permission/roles/qo_sales/capabilities',
          method: 'put',
          body: { capabilityCodes: ['bom.cap.library_manage'] },
        },
      ];
      ...
      test
        ...
        .annotations.push({
          type: 'note',
          description: `SYS-04b assignment statuses=${statuses.join(',')}`,
        });
      const denied = statuses.every(
        (s) => s === 401 || s === 403 || s === 404 || s === 405 || s >= 400,
      );
      expect(
        !anyAllowed && denied,
        `SYS-04b: non-admin capability-assignment denied (statuses=${statuses.join(',')})`,
      ).toBeTruthy();
    } finally {
      await context.close();
    }
