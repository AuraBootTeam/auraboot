...
import { openQuoteDetailFromList, seedDownloadableQuote } from './quote-e2e-helpers';
...
test.describe('PCBA quote Excel download', () => {
  ...
  test('downloads the Jiejia 3-sheet workbook from the quote detail page', async ({
    ...
    await saveWorkbookDownload(download, savedPath, testInfo, 'quote-standard');
    validateQuoteWorkbook(savedPath, { expectedFirstBomUnitPrice: 1.25, fixedCounts: { apertures: 3, holes: 2 } });
    // Keep this test-owned quote and file as release-gate evidence.
  });
});
