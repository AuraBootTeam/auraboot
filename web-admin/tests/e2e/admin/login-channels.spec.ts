...
async function gotoLoginChannels(page: Page) {
  ...
  await page.locator('h1').filter({ hasText: /登录渠道管理/ }).waitFor({ state: 'visible', timeout: 8000 });
  await page.locator('[data-testid="login-channels-save-btn"]').waitFor({ state: 'visible', timeout: 8000 });
  await page.locator('.animate-spin').waitFor({ state: 'detached', timeout: 10000 }).catch(() => {});
  await page.locator('[data-testid^="login-channel-"]').first().waitFor({ state: 'visible', timeout: 8000 });
}
...
test.describe('Login Channel Management', () => {
   ...
   */
  test('LC-001: should display page structure with all channels @smoke', async ({
    page,
  }) => {
    ...
    // Page title
    await expect(
      page.locator('h1').filter({ hasText: /登录渠道管理/ }),
    ).toBeVisible();

    // All 6 channel items
    for (const ch of ALL_CHANNELS) {
      await expect(
        page.locator(`[data-testid="login-channel-${ch}"]`),
      ).toBeVisible();
    }

    // Save button
    await expect(
      page.locator('[data-testid="login-channels-save-btn"]'),
    ).toBeVisible();
  });
   ...
   */
  test('LC-002: should toggle channel and show unsaved indicator', async ({
    page,
  }) => {
    await gotoLoginChannels(page);

    const smsToggle = page.locator(
      '[data-testid="login-channel-toggle-sms"]',
    );
    await expect(smsToggle).toBeVisible();

    // Record current state
    const wasEnabled =
      (await smsToggle.getAttribute('aria-checked')) === 'true';
   ...
   */
  test('LC-003: should save channel configuration @smoke', async ({
    page,
  }) => {
    await gotoLoginChannels(page);
   ...
   */
  test('LC-004: should reorder channels with arrow buttons', async ({
    page,
  }) => {
    await gotoLoginChannels(page);

    const firstChannel = page.locator(
      '[data-testid="login-channel-email_password"]',
    );
   ...
   */
  test('LC-005: should reflect enabled channels on login page', async ({
    page,
    browser,
  }) => {
    ...
    try {
      ...
      const emailTab = freshPage
        .locator('[data-testid="login-tab-email_password"], button:has-text("密码"), button:has-text("Email")')
        .first();
