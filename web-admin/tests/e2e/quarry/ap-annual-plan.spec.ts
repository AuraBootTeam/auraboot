...
async function createPlanScenario(
  ...
  const subPlansResp = await page.request.get(
    `/api/dynamic/ap-sub-plan/list?pageSize=50&filters=${encodeURIComponent(
  ...
  if (opts?.withWorkPackage) {
    const addWpResult = await executeCommandViaApi(
      page,
      'ap:add_work_package',
      {
...
async function waitForAnyWorkPackage(page: any, subPlanIds: string[], timeoutMs = 10000): Promise<boolean> {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    for (const subPlanId of subPlanIds) {
      const resp = await page.request.get(
        `/api/dynamic/ap-work-package/list?pageSize=1&filters=${encodeURIComponent(
...
async function submitAnnualPlanWithFallback(page: any, planPid: string): Promise<{ code: string; recordId: string }> {
  try {
    return await executeCommandViaApi(page, 'ap:submit_annual_plan', {}, planPid, 'state_transition');
  } catch (error: any) {
    const msg = String(error?.message ?? '');
    if (!msg.includes('HTTP 422') || (!msg.includes('BadParam') && !msg.includes('Bad parameter'))) throw error;

    const detailResp = await page.request.get(`/api/dynamic/ap-annual-plan/${planPid}`);
    ...
    return executeCommandViaApi(page, 'ap:submit_annual_plan', payload, planPid, 'state_transition');
  }
}

async function waitPlanStatus(page: any, planPid: string, expected: string, timeoutMs = 10000): Promise<boolean> {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const detailResp = await page.request.get(`/api/dynamic/ap-annual-plan/${planPid}`);
    const detailBody = await detailResp.json().catch(() => ({}));
...
async function approveAnnualPlanWithFallback(page: any, planPid: string): Promise<{ code: string; recordId: string }> {
  try {
    return await executeCommandViaApi(page, 'ap:approve_annual_plan', {}, planPid, 'state_transition');
  } catch (error: any) {
    const msg = String(error?.message ?? '');
    if (!msg.includes('HTTP 422') || (!msg.includes('BadParam') && !msg.includes('Bad parameter'))) throw error;
    ...
    return executeCommandViaApi(page, 'ap:approve_annual_plan', payload, planPid, 'state_transition');
  }
}

async function rejectAnnualPlanWithFallback(page: any, planPid: string): Promise<{ code: string; recordId: string }> {
  try {
...
test.describe('AP Annual Plan (4-Layer Architecture)', () => {
  ...
  test('should navigate to annual plan list page and see record', async ({ page }) => {
    ...
    const resp = await page.request.get(`/api/dynamic/ap-annual-plan/${planPid}`);
  ...
  test('should add work package with auto 12 monthly amounts', async ({ page }) => {
    ...
    const monthlyResp = await page.request.get(
      `/api/dynamic/ap-monthly-amount/list?pageSize=50&filters=${encodeURIComponent(
        JSON.stringify([{ fieldName: 'ap_work_package_id', operator: 'EQ', value: workPackagePid }]),
      )}`,
    );
  ...
  test('should submit plan (draft → submitted)', async ({ page }) => {
    const scenario = await createPlanScenario(page, baseYear + 2, { withWorkPackage: true, namePrefix: 'Submit Plan' });
  ...
  test('should reject plan (submitted → rejected)', async ({ page }) => {
    const scenario = await createPlanScenario(page, baseYear + 3, { withWorkPackage: true, namePrefix: 'Reject Plan' });
    expect(await waitForAnyWorkPackage(page, scenario.subPlanPids, 12000)).toBe(true);
  ...
  test('should re-submit after rejection and approve', async ({ page }) => {
    const scenario = await createPlanScenario(page, baseYear + 4, { withWorkPackage: true, namePrefix: 'Approve Plan' });
    expect(await waitForAnyWorkPackage(page, scenario.subPlanPids, 12000)).toBe(true);
    ...
    let detailResp = await page.request.get(`/api/dynamic/ap-annual-plan/${scenario.planPid}`);
  ...
  test('should cascade delete plan → sub-plans → work-packages → monthly', async ({ page }) => {
    ...
    const deleteResult = await executeCommandViaApi(
      page, 'ap:delete_annual_plan', {}, delPlanPid, 'delete',
    );
  ...
  test('should display annual plan detail page', async ({ page }) => {
    ...
    const viewAction = targetRow.locator(
      '[data-testid="row-action-view"], a, button:has-text("查看")',
    ).first();
