...
const WEB_BASE_URL = process.env.PLAYWRIGHT_BASE_URL || 'http://localhost:5278';
const BACKEND_URL = process.env.SPRING_BOOT_URL || 'http://127.0.0.1:6578';
...
test.describe('Open Platform golden journey', () => {
  ...
  test('covers application, installation, credential and revocation states', async ({
    ...
    await expect(scopeChecks).toHaveCount(2);
    await scopeChecks.nth(1).check();
    ...
    const tokenResponse = await page.request.post(`${BACKEND_URL}/oauth2/token`, {
      form: {
        ...
        scope: 'automation.events.write openapi.profile.read',
      },
    });
    ...
    await credentialList.getByRole('button', { name: '吊销' }).click();
    ...
    await apiReference.goto(
      `${BACKEND_URL}/swagger-ui/index.html?urls.primaryName=open-platform`,
    );
    ...
    await expect
      .poll(() => page.getByTestId('sidebar').evaluate((element) => element.getBoundingClientRect().right))
