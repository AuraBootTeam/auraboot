...
async function navigateToProjectWorkspace(
  ...
  // DSL row click navigates to detail page, not workspace.
  // Navigate directly to the custom workspace route.
  if (projectPid) {
    ...
    return; // Cannot navigate to workspace without pid
  }
...
test.describe('PM Workspace Interactions', () => {
  ...
  test('PM-INT-08: List view filter bar is visible with priority and status dropdowns', async ({
    page,
  }) => {
    await navigateToProjectWorkspace(page, projectName, projectPid);
