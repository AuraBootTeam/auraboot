...
async function createCrossContainerListBlockPage(
  ...
  const actionBarAnchorTableBlock = {
    id: 'action_bar_anchor_table',
    blockType: 'table',
    title: 'Action bar anchor table',
    layout: { span: 12 },
    blocks: [],
  };
...
async function createCrossContainerActionLeafListPage(
  page: Page,
  parentType: ActionLeafParentType,
  options: { emptyTarget?: boolean } = {},
): Promise<CreatedDesignerPage> {
  const id = uniqueId(`udw_${parentType.replace('-', '_')}_action_leaf_move`);
  const pageKey = `udw_${parentType.replace('-', '_')}_action_leaf_move_${id}`;
  const isActionBar = parentType === 'action-bar';
  const sourceParentId = isActionBar ? 'action_bar_source' : 'table_source';
  const targetParentId = isActionBar ? 'action_bar_target' : 'table_target';
  const sourceAnchorId = 'source_action_submit';
  const candidateId = 'action_move_candidate';
  const targetLeafId = 'target_action_view';
  const makeAction = (leafId: string, actionType: string, label: string) => ({
    id: leafId,
    blockType: 'action',
    actionType,
    props: { label },
  });
  const makeParent = (parentId: string, title: string, blocks: TestBlock[]) => ({
    id: parentId,
    blockType: parentType,
    title,
    region: isActionBar ? 'toolbar' : undefined,
    layout: { span: 12 },
    blocks,
  });
  const sourceParent = makeParent(sourceParentId, 'Source actions', [
    makeAction(sourceAnchorId, 'submit', 'Source anchor'),
    makeAction(candidateId, 'refresh', 'Move candidate'),
  ]);
  const targetParent = makeParent(
    targetParentId,
    'Target actions',
    options.emptyTarget ? [] : [makeAction(targetLeafId, 'view', 'Target leaf')],
  );

  const resp = await page.request.post('/api/pages', {
    data: {
      name: `UDW ${parentType} action leaf move ${id}`,
      pageKey,
      title: `UDW ${parentType} action leaf move ${id}`,
      kind: 'list',
      modelCode: 'page_schema',
      schemaVersion: 3,
      blocks: [
      ...
      extension: {
        e2e: true,
        scenario: `unified-designer-${parentType}-action-leaf-cross-container-move`,
      },
    },
  });
  expect(resp.ok(), await resp.text()).toBe(true);
...
function getActionLeafParentBlockId(parentType: ActionLeafParentType, role: 'source' | 'target') {
  return parentType === 'action-bar' ? `action_bar_${role}` : `table_${role}`;
}

async function createCrossParentActionLeafListPage(
  page: Page,
  route: ActionLeafCrossParentRoute,
  options: { emptyTarget?: boolean } = {},
): Promise<CreatedDesignerPage> {
  const sourceKey = route.source === 'action-bar' ? 'ab' : 'tbl';
  const targetKey = route.target === 'action-bar' ? 'ab' : 'tbl';
  const id = uniqueId('udw_ax');
  const pageKey = `udw_ax_${sourceKey}_${targetKey}_${id}`;
  const sourceParentId = getActionLeafParentBlockId(route.source, 'source');
  const targetParentId = getActionLeafParentBlockId(route.target, 'target');
  const makeAction = (leafId: string, actionType: string, label: string) => ({
    id: leafId,
    blockType: 'action',
    actionType,
  ...
  const makeParent = (
    ...
    blockType: parentType,
    title,
    region: parentType === 'action-bar' ? 'toolbar' : undefined,
    layout: { span: 12 },
    blocks,
  });
  const sourceParent = makeParent(route.source, sourceParentId, 'Source actions', [
    makeAction('source_action_submit', 'submit', 'Source anchor'),
    makeAction('action_move_candidate', 'refresh', 'Move candidate'),
  ]);
  const targetParent = makeParent(
    route.target,
    targetParentId,
    'Target actions',
    options.emptyTarget ? [] : [makeAction('target_action_view', 'view', 'Target leaf')],
  );
...
test.describe('Unified designer — kind collapse, i18n, model binding', () => {
  ...
  test('undoes and redoes deleting a saved palette-created container before saving readback', async ({ page }) => {
    ...
    await expect(page.getByTestId('block-delete-tabs_new_tabs')).toBeVisible();
    await page.getByTestId('block-delete-tabs_new_tabs').click();
    await expect(page.getByTestId('canvas-block-tabs_new_tabs')).toHaveCount(0);
    await expect(page.getByTestId('canvas-block-tab_new_tab')).toHaveCount(0);
    await expect(page.getByTestId('designer-dirty-state')).toHaveText('未保存');
  ...
  test('undoes and redoes moving a form-section subtree from a tab before a form-root sibling', async ({
    page,
  }) => {
    const { pageKey: formKey, pid } = await createCrossContainerFormSectionPage(page, {
      source: 'tab',
    ...
    await expect(sourceTab.getByTestId('canvas-block-form_section_move_candidate')).toHaveCount(0);
    await expect(formRoot.getByTestId('canvas-block-form_section_move_candidate')).toBeVisible();
    expect(await isBeforeInDom(formRoot, 'form_section_move_candidate', 'section_target')).toBe(
      true,
    );
    await expect(page.getByTestId('designer-dirty-state')).toHaveText('未保存');
    await waitForDesignerDragToSettle(page);
    ...
    await expect(sourceTab.getByTestId('canvas-block-form_section_move_candidate')).toBeVisible();
    expect(await isBeforeInDom(formRoot, 'tabs_holder', 'section_target')).toBe(true);
    await expect(page.getByTestId('designer-dirty-state')).toHaveText('已保存');
    await expect(page.getByTestId('designer-redo')).toBeEnabled();
  ...
  test('undoes and redoes moving a form-section subtree from form root inside an empty tab', async ({
    page,
  }) => {
    const { pageKey: formKey, pid } = await createCrossContainerFormSectionPage(page, {
      source: 'form',
    ...
    const targetTab = page.getByTestId('canvas-block-tab_empty');
    await expect(targetTab.getByTestId('canvas-block-form_section_move_candidate')).toBeVisible();
    expect(await isBeforeInDom(formRoot, 'tabs_holder', 'form_section_move_candidate')).toBe(true);
    await expect(page.getByTestId('designer-dirty-state')).toHaveText('未保存');
    await waitForDesignerDragToSettle(page);

    await clickDesignerToolbarButton(page, 'designer-undo');

    await expect(targetTab.getByTestId('canvas-block-form_section_move_candidate')).toHaveCount(0);
    await expect(formRoot.getByTestId('canvas-block-form_section_move_candidate')).toBeVisible();
    expect(await isBeforeInDom(formRoot, 'form_section_move_candidate', 'tabs_holder')).toBe(true);
    await expect(page.getByTestId('designer-dirty-state')).toHaveText('已保存');
    await expect(page.getByTestId('designer-redo')).toBeEnabled();
  ...
  test('undoes and redoes a list action-bar subtree move-before between tab and list root', async ({
    ...
    await page.getByTestId('designer-mode-layout').click();
    await dragCanvasBlockBeforeHeader(page, 'action_bar_move_candidate', 'action_bar_anchor_table');

    const listRoot = page.getByTestId('canvas-block-list_root');
    const sourceTab = page.getByTestId('canvas-block-tab_source');
    await expect(sourceTab.getByTestId('canvas-block-action_bar_move_candidate')).toHaveCount(0);
    await expect(listRoot.getByTestId('canvas-block-action_bar_move_candidate')).toBeVisible();
    expect(await isBeforeInDom(listRoot, 'action_bar_move_candidate', 'action_bar_anchor_table')).toBe(
      true,
    );
    await expect(page.getByTestId('designer-dirty-state')).toHaveText('未保存');
    await waitForDesignerDragToSettle(page);
  ...
  test('undoes and redoes a list action-bar subtree move-inside from list root into an empty tab', async ({
    page,
  }) => {
    const { pageKey: listKey, pid } = await createCrossContainerListBlockPage(page, 'action-bar', {
      source: 'list',
  ...
  for (const parentType of ['action-bar', 'table'] as const) {
    test(`undoes and redoes moving an action leaf before another action in a ${parentType}`, async ({
      page,
    }) => {
      const sourceParentId = parentType === 'action-bar' ? 'action_bar_source' : 'table_source';
      const targetParentId = parentType === 'action-bar' ? 'action_bar_target' : 'table_target';
      const sourceAnchorId = 'source_action_submit';
      const candidateId = 'action_move_candidate';
      const targetLeafId = 'target_action_view';
      const { pageKey: listKey, pid } = await createCrossContainerActionLeafListPage(
        page,
        parentType,
      );
      await openDesigner(page, listKey);
      ...
      const savedSource = findBlock(savedBlocks, sourceParentId);
      const savedTarget = findBlock(savedBlocks, targetParentId);
      const movedAction = findBlock(savedBlocks, candidateId);
  ...
  for (const route of [
    { source: 'action-bar', target: 'table' },
    { source: 'table', target: 'action-bar' },
  ] as const) {
    test(`undoes and redoes moving a cross-parent action leaf before another action from ${route.source} to ${route.target}`, async ({
      page,
    }) => {
      const sourceParentId = getActionLeafParentBlockId(route.source, 'source');
      const targetParentId = getActionLeafParentBlockId(route.target, 'target');
