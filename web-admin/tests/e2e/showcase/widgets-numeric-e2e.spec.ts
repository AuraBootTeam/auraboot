...
async function navigateToDesignerViaMenu(page: Page, pid: string, pageKey: string): Promise<void> {
  ...
  const listResp = page.waitForResponse(
    (r) => (r.url().includes('/api/dynamic/page_schema/list') || (r.url().includes('/dynamic/page_schema_list') && r.url().includes('/list'))),
    { timeout: 5_000 },
  );
  ...
  if (await search.isVisible({ timeout: 1_500 }).catch(() => false)) {
    ...
    await page
      .waitForResponse((r) => (r.url().includes('/api/dynamic/page_schema/list') || r.url().includes('/dynamic/page_schema_list')) && r.status() === 200, {
        timeout: 5_000,
      })
      .catch(() => null);
  }
