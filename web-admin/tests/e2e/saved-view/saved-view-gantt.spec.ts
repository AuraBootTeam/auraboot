...
/** Navigate to e2et-order page, click gantt type, and select the gantt view */
async function gotoAndSelectGanttView(page: import('@playwright/test').Page) {
  ...
  await expect(ganttBtn).toBeVisible({ timeout: 5000 });
  await ganttBtn.click();

  // Open dropdown and select the gantt view
...
test.describe('SavedView — GANTT View', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    if (existing.ok()) {
      ...
      for (const v of (body.data ?? []).filter((v: any) => v.viewType === 'gantt' && v.name === VIEW_NAME)) {
  ...
  test('SV-040: GANTT — timeline renders @smoke', async ({ page }) => {
    ...
    const content = page.getByText(/\d+ tasks/i)
    ...
    await expect(page.locator('[data-testid="gantt-config-warning"]')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('SV-041: GANTT — time zoom (day/week/month)', async ({ page }) => {
    ...
    const ganttContent = page.getByText(/\d+ tasks/i)
      .or(page.getByText('Gantt chart not configured'))
      .or(page.locator('[data-testid="gantt-empty-diagnostics"]'))
      .or(page.getByText('not configured'));
