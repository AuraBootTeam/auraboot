...
| Quick filter / personal view boundary | Browser verifies quick filters are active only in default-view mode; after saving a preset as personal view the toolbar chip is not highlighted and no saved/edited/reset preset controls appear; clicking a quick filter from a personal view clears `view` and applies `preset` | `/api/views` create or existing-copy readback proves the personal view exists; URL assertions prove `view` and `preset` are mutually exclusive | `saved-view-follow-up-golden.spec.ts` `SV-FU-003`, `saved-view-quick-filters.spec.ts` `QF-003b`, `ListToolbar.i18n.test.tsx`, `quickFilterPresets.test.ts` | Covered for current simplified contract: quick filters belong to default view mode only; rename/delete remain normal personal SavedView management rows |
...
| --- | --- | --- | --- |
| SavedView targeted browser regression | `PW_SKIP_WEBSERVER=1 PLAYWRIGHT_BASE_URL=http://127.0.0.1:5187 BACKEND_URL=http://127.0.0.1:6486 BE_PORT=6486 BFF_PORT=6187 NO_PROXY=localhost,127.0.0.1 PW_WORKERS=1 npx playwright test tests/e2e/saved-view/saved-view-management.spec.ts tests/e2e/saved-view/saved-view-quick-filters.spec.ts tests/e2e/saved-view/saved-view-follow-up-golden.spec.ts --project=chromium --no-deps` | `16 passed` | Current branch Personal-only targeted regression. Covers duplicate quick-filter prevention, discard behavior, hide-column autosave, quick preset save-as-personal, personal quota, and quick filter / personal view boundary |
| Frontend unit/component | `pnpm test:unit:run -- app/framework/meta/rendering/pages/__tests__/ListPageContent.test.ts app/framework/meta/rendering/pages/list/__tests__/ListToolbar.i18n.test.tsx app/framework/meta/rendering/pages/list/__tests__/quickFilterPresets.test.ts app/framework/meta/rendering/pages/list/__tests__/dsl-list-i18n-resources.test.ts app/framework/smart/components/view/__tests__/ViewSelector.test.tsx app/framework/smart/hooks/__tests__/useSavedViews.test.ts app/framework/smart/utils/__tests__/savedViewPersistence.test.ts` | `476 files passed, 4875 tests passed` | This invocation ran the full web-admin unit suite; it covers URL precedence, selector, toolbar quick filters, quick preset request generation, i18n resources, persistence, and saved-view hooks |
| TypeScript | `pnpm typecheck` | PASS | React Router typegen and `tsc` passed |
| Diff hygiene | `git diff --check` | PASS | No whitespace errors in current diff |
| e2e-truth audit | Manual grep for skip/fixme/waitForTimeout/retry/threshold/direct-route plus Playwright output check | PASS with known exceptions | Direct `/p/` navigation is only accepted for URL-contract regression `SV-PER-003b`; historical API-heavy specs remain contract evidence, not UI completion |
| Golden screenshots | `web-admin/test-results/saved-view-personal-golden/*.png`, `web-admin/test-results/saved-view-follow-up-golden/04-preset-saved-as-personal-clean-state.png` | 8 PNG screenshots | Current Personal-only golden evidence plus quick filter / personal view boundary screenshot; historical collaborator/audit/team quota screenshots are roadmap-only |
...
- Core Personal-only redline grep found no `test.skip`, `test.fixme`, `skip(true)`, `waitForTimeout`, `retries:`, `toBeLessThanOrEqual`, `toBeGreaterThanOrEqual`, `threshold`, `baseline`, or `.only(` in:
  - `saved-view-management.spec.ts`
...
- 2026-06-23 current branch run targeted the three active Personal-only files above. Result was `16 passed`; this does not reopen team/global/collaborator/audit scope.
