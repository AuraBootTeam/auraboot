...
test.describe('Quick Filters (GAP-130)', () => {
  ...
  test('QF-003: clicking a quick filter toggles active state', async ({ page }) => {
    ...
    // Initially not active (gray background)
    await expect(myRecords).toHaveClass(/bg-gray-100/);

    // Click to activate
    await myRecords.click();
    await expect(myRecords).toHaveClass(/bg-blue-100/);
  ...
  test('QF-004: only one quick filter active at a time', async ({ page }) => {
    ...
    await expect(createdToday).toHaveClass(/bg-blue-100/);
    await expect(myRecords).toHaveClass(/bg-gray-100/);
  });
