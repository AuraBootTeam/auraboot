...
test.describe('Menu Route Contract', () => {
  ...
  test('MRC-001: menu leaf paths are resolvable and navigable @critical', async ({ page }) => {
    ...
    for (const path of sample) {
      ...
      const byPath = await page.request.get(`/api/menu/by-path?path=${encodeURIComponent(path)}`, {
        timeout: 8000,
      });
      ...
      const hasCrash = /Application Error|Unhandled Runtime Error|TypeError:|ReferenceError:/i.test(
        bodyText,
      );
      ...
      const hasBusinessContent = await page
        .locator(
      ...
      const hasAccessState =
        /403|Forbidden|无权限|权限不足|404|Not Found|页面不存在|加载失败|Page not found/i.test(
          bodyText,
        );
      expect(
        hasBusinessContent || hasAccessState,
        `path ${path} should present business content or explicit state`,
      ).toBe(true);
    }
  });
});
