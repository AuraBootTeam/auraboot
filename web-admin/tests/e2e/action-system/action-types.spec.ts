...
import { uniqueId, navigateToDynamicPage, waitForDynamicPageLoad, waitForFormReady, clickRowActionByLocator } from '../helpers';
...
test.describe('Action System — Action Types', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    orderPid = await order.createViaApi({ e2et_order_title: `ActionTest ${uniqueId()}` });
  ...
  test('AC-001: navigate action — create button navigates to form @smoke', async ({ page }) => {
    ...
    const addBtn = page.locator('[data-testid="toolbar-btn-create"], button:has-text("新建"), button:has-text("Create")').first();
  ...
  test('AC-002: back action — cancel navigates back', async ({ page }) => {
    ...
    const cancelBtn = page.locator('button:has-text("取消"), button:has-text("Cancel"), button:has-text("返回"), button:has-text("Back"), a:has-text("取消"), a:has-text("Cancel"), a:has-text("返回"), a:has-text("Back")').first();

    const hasBackLink = await backLink.isVisible({ timeout: 5000 }).catch(() => false);
    const hasCancelBtn = !hasBackLink && await cancelBtn.isVisible({ timeout: 3000 }).catch(() => false);
  ...
  test('AC-004: showModal action — opens modal', async ({ page }) => {
    ...
    const modalTrigger = row.locator(
      'button:has-text("详情"), button:has-text("查看"), [data-testid*="action"]'
    ).first();
  ...
  test('AC-005: closeModal action — closes dialog', async ({ page }) => {
    ...
    const isOpen = await modal.first().isVisible({ timeout: 2000 }).catch(() => false);
    ...
    const closeBtn = modal.locator('button:has-text("关闭"), button:has-text("Close"), [aria-label="Close"]').first();
  ...
  test('AC-006: showDrawer action', async ({ page }) => {
    ...
    if (hasTrigger) {
      ...
      const hasDrawer = await drawer.first().isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('AC-007: showToast action — success message', async ({ page }) => {
    ...
    const addBtn = page.locator('[data-testid="toolbar-btn-create"], button:has-text("新建")').first();
    const hasAddBtn = await addBtn.isVisible({ timeout: 5000 }).catch(() => false);
    if (!hasAddBtn) {
      throw new Error(String('Create button not available'))
      return;
    }
    await addBtn.click();
    await page.waitForURL((url) => url.pathname.includes('/new'), { timeout: 10000 });
    ...
    const titleInput = page
      ...
      .first();
    const hasTitle = await titleInput.count().then((count) => count > 0).catch(() => false);
    if (!hasTitle) {
    ...
    // Save
    const saveBtn = page.locator('[data-testid^="form-btn-"], button:has-text("保存"), button:has-text("Save")').first();
    await saveBtn.click();
  ...
  test('AC-008: showConfirmDialog action — delete confirm', async ({ page }) => {
    ...
    // Go to draft tab
    const draftTab = page.locator('nav button').filter({ hasText: /草稿|Draft/i }).first();
    ...
    // Click delete on first row — allow extra time for tab data to load
    const firstRow = page.locator('tbody tr').first();
    const hasFirstRow = await firstRow.isVisible({ timeout: 10000 }).catch(() => false);

    if (!hasFirstRow) {
      throw new Error(String('No rows available for delete test'))
      return;
    }

    await firstRow.scrollIntoViewIfNeeded();
    await clickRowActionByLocator(page, firstRow, 'delete');

    // Confirm dialog should appear
    const confirmDialog = page.locator('[data-testid="confirm-dialog"], [role="alertdialog"], .ant-modal-confirm');
    const hasDialog = await confirmDialog.first().isVisible({ timeout: 5000 }).catch(() => false);

    if (hasDialog) {
      // Dismiss the dialog (don't actually delete)
      const cancelBtn = page.locator('[data-testid="confirm-cancel"], button:has-text("取消"), button:has-text("Cancel")').first();
  ...
  test('AC-013: if action — conditional rendering', async ({ page }) => {
    ...
    if (hasDraftTab) {
      ...
      const submittedTab = page.locator('nav button').filter({ hasText: /已提交|Submitted/i }).first();
  ...
  test('AC-015: redirect action', async ({ page }) => {
    ...
    const hasRow = await row.isVisible({ timeout: 5000 }).catch(() => false);
    if (!hasRow) {
      throw new Error(String('No rows available'))
      return;
    }
  ...
  test('AC-017: callApi action — API call triggered', async ({ page }) => {
    ...
    const submitBtn = page.locator('[data-testid="row-action-submit"], button:has-text("提交")').first();
