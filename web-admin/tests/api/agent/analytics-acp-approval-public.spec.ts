...
for (const mode of ['approve', 'reject', 'race', 'expired']) {
  test(`ACP authorized decision after foreign-user denial: ${mode}`, async ({ request }) => {
    ...
    test.setTimeout(mode === 'expired' ? 360000 : 90000);
    ...
    try {
      ...
      const verifyFailedOutcome = async () => {
        const terminalFacts = (
          await db.query(
        ...
        await expect
          .poll(
            async () =>
              (
                await db.query(
                  `SELECT a.approval_status, r.run_status FROM ab_agent_approval a
             JOIN ab_agent_run r ON r.pid=a.run_id AND r.tenant_id=a.tenant_id
             WHERE a.pid=$1 AND a.tenant_id=$2`,
                  [approval.pid, owners[0].tenant_id],
                )
              ).rows,
            { timeout: 330000, intervals: [1000, 5000] },
          )
          .toEqual([{ approval_status: 'expired', run_status: 'failed' }]);
        expect(await actions()).toEqual([]);
        await verifyFailedOutcome();
        const afterExpiry = await decide(true);
        expect(afterExpiry).toContain('event:error');
        expect(afterExpiry).not.toContain('event:done');
        expect(await actions()).toEqual([]);
        return;
      }
      let text: string;
