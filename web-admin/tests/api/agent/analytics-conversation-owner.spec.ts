...
for (const scope of ['same-tenant', 'cross-tenant']) {
  test(`new analysis conversations remain private for reads and forged message writes: ${scope}`, async ({
    request,
  }) => {
    ...
    const messages = async (id: number) =>
      (
        await db.query(
          'SELECT id, content FROM ab_im_message WHERE conversation_id=$1 ORDER BY id',
          [id],
        )
      ).rows;
    ...
    try {
      ...
      if (scope === 'cross-tenant') {
        const seed = await request.post('/api/test/seed', {
          timeout: 90000,
          params: { testRunId: `conversation-owner-${randomUUID()}` },
        });
        expect(seed.status()).toBe(200);
        jwt = (await seed.json()).jwt;
      } else {
        ...
        jwt = (await login.json()).data.jwt;
      }
      const raw = Buffer.from(jwt.split('.')[1], 'base64url').toString('utf8');
      const tokenTenant = raw.match(/"tenantId"\s*:\s*"?(\d+)/)?.[1];
      expect(tokenTenant).toBeTruthy();
      if (scope === 'cross-tenant') expect(tokenTenant).not.toBe(String(owner.tenant_id));
      else expect(tokenTenant).toBe(String(owner.tenant_id));
      ...
      expect(String(foreignOwner.tenant_id)).toBe(tokenTenant);
      if (scope === 'cross-tenant') expect(foreignOwner.tenant_id).not.toBe(owner.tenant_id);
      else expect(foreignOwner.tenant_id).toBe(owner.tenant_id);
