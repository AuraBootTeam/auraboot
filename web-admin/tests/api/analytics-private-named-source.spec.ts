...
for (const shape of ['select', 'cte'] as const) {
  test(`private physical sources cannot be listed or aggregated through named queries (${shape})`, async ({
    request,
  }) => {
    ...
    for (const table of [
      ...
      const created = await request.post('/api/meta/named-queries', {
        data: {
          ...
          fromSql:
            (shape === 'cte' ? `WITH source AS (` : '') +
            `SELECT pid FROM public."${table}"${table === 'mt_e2et_order' ? ' WHERE e2et_order_title = #{params.marker} AND tenant_id = #{params.tenantId} AND created_by = CAST(#{params.currentUserId} AS bigint)' : ''}` +
            (shape === 'cte' ? ') SELECT pid FROM source' : ''),
          status: 'published',
          fields: [
