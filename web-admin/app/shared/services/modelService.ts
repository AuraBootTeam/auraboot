...
export interface ModelPublishReplayStep {
  consumerType?: string;
...
export interface ModelPublishGovernance {
  modelCode: string;
  draftVersion?: number;
...
export interface PublishPreview {
  ...
  governance?: ModelPublishGovernance | null;
}

export interface PublishModelOptions {
  versionNote?: string;
  impactAcknowledged?: boolean;
  acknowledgementNote?: string;
}

export interface ModelPublishReplayRequest {
  executeAutomated?: boolean;
  correlationId?: string;
  sampleContext?: Record<string, Record<string, unknown>>;
}

export interface ModelPublishReplayResult {
...
export interface ModelPublishReplayReport {
  modelCode?: string;
  draftVersion?: number;
  latestPublishedVersion?: number | null;
  generatedAt?: string;
  governance?: ModelPublishGovernance;
  totalCount?: number;
...
export class ModelService implements IModelService {
  ...
  async publish(
    pid: string,
    versionNoteOrOptions?: string | PublishModelOptions,
    request?: Request,
  ): Promise<MetaModelDTO> {
    const options =
      typeof versionNoteOrOptions === 'string'
        ? { versionNote: versionNoteOrOptions }
        : versionNoteOrOptions;
    const params = options?.versionNote
      ? `?versionNote=${encodeURIComponent(options.versionNote)}`
      : '';
    const body =
      options && (options.impactAcknowledged !== undefined || options.acknowledgementNote)
        ? options
        : undefined;
    const result = await post<MetaModelDTO>(
  ...
  /**
   * Run or collect post-publish replay checks generated by model governance.
   */
  async replayPublishImpact(
    pid: string,
    options?: ModelPublishReplayRequest,
    request?: Request,
  ): Promise<ModelPublishReplayReport> {
    const result = await post<ModelPublishReplayReport>(
      `${this.baseUrl}/${pid}/publish/replay`,
      options,
      undefined,
      request,
    );
    return handleResponse(result, 'Failed to replay model publish impact');
  }
