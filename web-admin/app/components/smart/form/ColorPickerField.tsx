...
export function ColorPickerField({
  ...
  if (readOnly) {
    return (
      <div className="flex items-center gap-2">
        <div className="h-6 w-6 rounded border border-gray-300" style={{ backgroundColor: currentColor }} />
  ...
  return (
    <div ref={containerRef} className="relative inline-block">
      ...
      >
        <div className="h-5 w-5 rounded border border-gray-200" style={{ backgroundColor: currentColor }} />
          ...
          <div className="mb-2 grid grid-cols-6 gap-1.5">
            {PRESET_COLORS.map((color) => (
              <button
                key={color}
                type="button"
                className={`h-7 w-7 rounded-md border-2 ${currentColor === color ? 'border-blue-500' : 'border-transparent'} hover:scale-110 transition-transform`}
                style={{ backgroundColor: color }}
                onClick={() => { setCurrentColor(color); onChange?.(color); }}
              />
            ))}
          </div>
          <input
            type="color"
            value={currentColor}
            onChange={(e) => { setCurrentColor(e.target.value); onChange?.(e.target.value); }}
