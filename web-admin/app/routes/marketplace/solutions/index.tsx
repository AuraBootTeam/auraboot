...
export default function SolutionMarketplacePage() {
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="flex items-center gap-2 text-2xl font-bold text-gray-900">
                <CubeTransparentIcon className="h-7 w-7 text-indigo-600" />
                {locale === 'zh-CN' ? '行业解决方案' : 'Industry Solutions'}
              </h1>
              <p className="mt-1 text-sm text-gray-500">
                {locale === 'zh-CN'
                  ? '一键安装预配置的行业解决方案，包含多个插件和配置模板'
                  : 'Install pre-configured industry solutions bundling multiple plugins and templates'}
              </p>
            </div>
            <div className="flex items-center gap-3">
              ...
              >
                {locale === 'zh-CN' ? '查看插件市场' : 'View Plugin Marketplace'} &rarr;
              </Link>
              <form onSubmit={handleSearch} className="flex items-center gap-2">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-64 rounded-lg border border-gray-300 py-2 pr-4 pl-9 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500"
                  />
                </div>
              </form>
            </div>
              ...
              <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
                {solutions.map((sol) => (
                  <div
                    ...
                    onClick={() =>
                      navigate(`/marketplace/solutions/${encodeURIComponent(sol.code)}`)
                    }
