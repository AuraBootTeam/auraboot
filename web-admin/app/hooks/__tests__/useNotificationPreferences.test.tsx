...
describe('useNotificationPreferences', () => {
  ...
  it('updatePreference does optimistic update then confirms', async () => {
    ...
    const pref = result.current.preferences.find(
      (p: { channel: string; category: string; enabled: boolean }) => p.channel === 'email' && p.category === 'business',
    );
  ...
  it('updatePreference reverts on exception', async () => {
    ...
    const pref = result.current.preferences.find(
      (p: { channel: string; category: string; enabled: boolean }) => p.channel === 'slack' && p.category === 'alert',
    );
    expect(pref?.enabled).toBe(true);
    expect(showErrorToast).toHaveBeenCalled();
  });
