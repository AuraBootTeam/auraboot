...
export default function OpenPlatformPage() {
  ...
  const [operationsInstallation, setOperationsInstallation] = useState<string | null>(null);
  const [pendingRotate, setPendingRotate] = useState<{
    installationPid: string;
    credential: Credential;
  } | null>(null);
  const [rotationGraceMinutes, setRotationGraceMinutes] = useState(1440);
  ...
  const [pendingDisable, setPendingDisable] = useState<
    | { kind: 'application'; pid: string; name: string }
    ...
    try {
      ...
      setError(
        loadError instanceof Error
          ? loadError.message
          : t('openPlatform.loadFailed', undefined, 'Unable to load Open Platform'),
      );
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      showToast(
        createError instanceof Error
          ? createError.message
          : t('openPlatform.createFailed', undefined, 'Unable to create application'),
        'error',
      );
    } finally {
      setCreating(false);
  ...
  const install = async () => {
    ...
    try {
      await apiFetch<Installation>(
        `/api/open-platform/applications/${installingApp}/installations`,
        {
          method: 'POST',
          body: JSON.stringify({ environment, scopes: [...selectedScopes], rateLimitPerMinute }),
        },
      );
      ...
      showToast(
        installError instanceof Error
          ...
          : t('openPlatform.installFailed', undefined, 'Unable to install application'),
        'error',
      );
    }
  };
  ...
  const loadCredentials = async (installationPid: string) => {
    try {
      const result = await apiFetch<Credential[]>(
        `/api/open-platform/installations/${installationPid}/credentials`,
      );
      ...
      showToast(
        credentialError instanceof Error
          ...
          : t('openPlatform.credentialsFailed', undefined, 'Unable to load credentials'),
        'error',
      );
    }
  };
  ...
  const createCredential = async (installationPid: string) => {
    try {
      const created = await apiFetch<CredentialSecret>(
        `/api/open-platform/installations/${installationPid}/credentials`,
        { method: 'POST' },
      );
      setSecret(created);
      ...
      showToast(
        credentialError instanceof Error
          ? credentialError.message
          : t('openPlatform.credentialCreateFailed', undefined, 'Unable to create credential'),
        'error',
      );
    }
  };
  ...
  const revokeCredential = async () => {
    ...
    try {
      await apiFetch<void>(
        `/api/open-platform/installations/${installationPid}/credentials/${credential.pid}`,
        { method: 'DELETE' },
      );
      ...
      showToast(
        revokeError instanceof Error
          ? revokeError.message
          : t('openPlatform.revokeFailed', undefined, 'Unable to revoke credential'),
        'error',
      );
    }
  };
  ...
  const rotateCredential = async () => {
    ...
    try {
      const rotated = await apiFetch<{ credential: CredentialSecret }>(
        `/api/open-platform/installations/${pendingRotate.installationPid}/credentials/${pendingRotate.credential.pid}/rotate`,
        { method: 'POST', body: JSON.stringify({ graceMinutes: rotationGraceMinutes }) },
      );
      setSecret(rotated.credential);
      await loadCredentials(pendingRotate.installationPid);
      setPendingRotate(null);
      showToast(
        t(
          'openPlatform.credentialRotated',
          undefined,
          'Credential rotated; the previous secret remains valid during the grace period',
        ),
        'success',
      );
    } catch (rotateError) {
      showToast(
        rotateError instanceof Error
          ? rotateError.message
          : t('openPlatform.rotateFailed', undefined, 'Unable to rotate credential'),
        'error',
      );
    }
  };
  ...
  const updateScopes = async () => {
    ...
    try {
      ...
      showToast(
        t('openPlatform.scopesUpdated', undefined, 'Scopes updated; existing tokens were revoked'),
        'success',
      );
      await load();
    } catch (scopeError) {
      showToast(
        scopeError instanceof Error
          ? scopeError.message
          : t('openPlatform.scopeUpdateFailed', undefined, 'Unable to update scopes'),
        'error',
      );
    }
  };
  ...
  const disableTarget = async () => {
    ...
    const path =
      pendingDisable.kind === 'application'
    ...
    try {
      ...
      showToast(
        t('openPlatform.disabled', undefined, 'Access disabled and active tokens revoked'),
        'success',
      );
      await load();
    } catch (disableError) {
      showToast(
        disableError instanceof Error
          ...
          : t('openPlatform.disableFailed', undefined, 'Unable to disable access'),
        'error',
      );
    }
  };
  ...
  return (
    <main
      ...
      data-testid="open-platform-page"
    >
      <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <h1 className="text-2xl font-semibold text-gray-950 dark:text-white">
            {t('openPlatform.title', undefined, 'Open Platform')}
          </h1>
          <p className="mt-1 max-w-full text-sm break-all whitespace-normal text-gray-500 sm:max-w-2xl sm:break-words dark:text-gray-400">
            {t(
              'openPlatform.subtitle',
              undefined,
              'Create tenant-scoped applications, grant minimum scopes, rotate credentials, and inspect the public API contract.',
            )}
          </p>
        </div>
        <div className="flex gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200"
          >
      ...
      <section
        ...
        aria-label={t('openPlatform.connectionFlow', undefined, 'Connection flow')}
      >
        {[
          ...
          >
            <span className="mb-2 flex h-6 w-6 items-center justify-center rounded-full bg-indigo-100 text-xs font-semibold text-indigo-700 dark:bg-indigo-900 dark:text-indigo-200">
              {step}
            </span>
      ...
      {loading ? (
        ...
        <div
          className="rounded-xl border border-dashed border-gray-300 bg-white px-6 py-16 text-center dark:border-gray-700 dark:bg-gray-900"
          data-testid="open-platform-empty"
        >
          <KeyIcon className="mx-auto h-10 w-10 text-gray-300" />
          <h2 className="mt-3 font-medium text-gray-900 dark:text-white">
            {t('openPlatform.emptyTitle', undefined, 'No external applications yet')}
          </h2>
              ...
              <div className="flex flex-col gap-3 border-b border-gray-100 p-5 sm:flex-row sm:items-center sm:justify-between dark:border-gray-800">
                ...
                <div className="flex gap-2">
                  ...
                  {application.status === 'active' && (
                    ...
                    >
                      {t('openPlatform.disable', undefined, 'Disable')}
                    </button>
                  )}
                </div>
              </div>
              <div className="space-y-3 p-5">
                {application.installations.length === 0 ? (
                  ...
                  application.installations.map((installation) => (
                    <div
                      key={installation.pid}
                      className="min-w-0 rounded-lg border border-gray-200 p-4 dark:border-gray-700"
                      data-testid={`open-platform-installation-${installation.pid}`}
                    >
                      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-medium text-gray-900 dark:text-white">
                              {t(
                                `openPlatform.environment.${installation.environment}`,
                                undefined,
                                installation.environment,
                              )}
                            </span>
                            <StatusBadge status={installation.status} t={t} />
                          </div>
                        </div>
                        <div className="grid w-full min-w-0 grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-2 sm:flex sm:w-auto sm:flex-wrap sm:justify-end">
                          <button
                            ...
                            onClick={() =>
                              setOperationsInstallation((current) =>
                                current === installation.pid ? null : installation.pid,
                              )
                            }
                            className="inline-flex min-w-0 items-center justify-center gap-1 rounded-md border border-gray-300 px-2.5 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200"
                            aria-expanded={operationsInstallation === installation.pid}
                          >
                          ...
                          <button
                            ...
                            disabled={installation.status !== 'active'}
                            className="inline-flex min-w-0 items-center justify-center gap-1 rounded-md bg-gray-900 px-2.5 py-1.5 text-xs font-medium text-white hover:bg-gray-700 disabled:opacity-40 dark:bg-white dark:text-gray-900"
                          >
                          ...
                          {installation.status === 'active' && (
                            <button
                              ...
                              aria-label={t(
                                'openPlatform.disableInstallation',
                                undefined,
                      ...
                      <p className="mt-3 text-xs text-gray-400">
                        {t('openPlatform.rateLimit', undefined, 'Rate limit')}:{' '}
                        {installation.rateLimitPerMinute}/min
                      </p>
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {installation.scopes.map((scope) => (
                          <span
                            key={scope}
                            className="rounded-full bg-slate-100 px-2 py-1 font-mono text-[11px] text-slate-700 dark:bg-slate-800 dark:text-slate-200"
                          >
                            {scope}
                          </span>
                        ))}
                      ...
                      {credentials[installation.pid] && (
                        <CredentialList
                          ...
                          onRevoke={(credential) =>
                            setPendingRevoke({ installationPid: installation.pid, credential })
                          }
                          onRotate={(credential) => {
                            setRotationGraceMinutes(1440);
                            setPendingRotate({ installationPid: installation.pid, credential });
                          }}
                          t={t}
                        />
                      )}
      ...
      {showCreate && (
        <Dialog
          title={t('openPlatform.createTitle', undefined, 'Create external application')}
          onClose={() => setShowCreate(false)}
        >
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-200">
            {t('openPlatform.name', undefined, 'Application name')}
            <input
              value={appName}
              ...
              data-testid="open-platform-app-name"
            />
          </label>
          <label className="mt-4 block text-sm font-medium text-gray-700 dark:text-gray-200">
            {t('openPlatform.description', undefined, 'Description')}
            <textarea
      ...
      {installingApp && (
        <Dialog
          title={t('openPlatform.installTitle', undefined, 'Install application')}
          onClose={() => setInstallingApp(null)}
        >
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-200">
            {t('openPlatform.environmentLabel', undefined, 'Environment')}
            <select
              value={environment}
              onChange={(event) =>
                setEnvironment(event.target.value as Installation['environment'])
              }
              className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 dark:border-gray-700 dark:bg-gray-950"
            >
              <option value="development">
                {t('openPlatform.environment.development', undefined, 'Development')}
              </option>
              <option value="staging">
                {t('openPlatform.environment.staging', undefined, 'Staging')}
              </option>
              <option value="production">
                {t('openPlatform.environment.production', undefined, 'Production')}
              </option>
            </select>
          </label>
          <label className="mt-4 block text-sm font-medium text-gray-700 dark:text-gray-200">
            {t('openPlatform.rateLimitPerMinute', undefined, 'Requests per minute')}
            <input
              type="number"
              min={1}
              max={100000}
              ...
              className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 dark:border-gray-700 dark:bg-gray-950"
              data-testid="open-platform-rate-limit"
            />
          </label>
          <fieldset className="mt-4">
            <legend className="text-sm font-medium text-gray-700 dark:text-gray-200">
              {t('openPlatform.scopes', undefined, 'Scopes')}
            </legend>
            <p className="mb-2 text-xs text-gray-500">
              {t(
                'openPlatform.minimumScopes',
                undefined,
                'Grant only the capabilities this installation needs.',
              )}
            </p>
            <div className="space-y-2">
              {scopeCatalog.map((scope) => (
                <label
                  key={scope}
                  className="flex cursor-pointer items-center gap-2 rounded-lg border border-gray-200 p-3 text-sm dark:border-gray-700"
                >
                  <input
                    type="checkbox"
          ...
          <DialogActions
            onCancel={() => setInstallingApp(null)}
            onConfirm={() => void install()}
            confirmDisabled={
              selectedScopes.size === 0 || rateLimitPerMinute < 1 || rateLimitPerMinute > 100000
            }
            confirmText={t('openPlatform.install', undefined, 'Install')}
            cancelText={t('common.cancel', undefined, 'Cancel')}
          />
        </Dialog>
      )}
      ...
      {secret && (
        <Dialog
          title={t('openPlatform.secretTitle', undefined, 'Save this credential now')}
        ...
        >
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-200">
            {t(
              'openPlatform.secretWarning',
              undefined,
              'The client secret is shown once. AuraBoot stores only its hash.',
            )}
          </div>
          <SecretRow
            label="client_id"
            ...
            onCopy={copy}
            copyLabel={t('common.copy', undefined, 'Copy')}
          />
          <SecretRow
            label="client_secret"
          ...
          <DialogActions
            ...
            confirmText={t('openPlatform.saved', undefined, 'I saved it')}
            cancelText={t('common.cancel', undefined, 'Cancel')}
            hideCancel
          />
        </Dialog>
      ...
      {pendingRevoke && (
        <Dialog
          title={t('openPlatform.revokeTitle', undefined, 'Revoke credential?')}
          onClose={() => setPendingRevoke(null)}
        >
          <p className="text-sm text-gray-600 dark:text-gray-300">
            {t(
              'openPlatform.revokeWarning',
              undefined,
              'Existing access tokens issued by this credential will stop working immediately.',
            )}
          </p>
          <code className="mt-3 block rounded bg-gray-100 p-2 text-xs dark:bg-gray-800">
            {pendingRevoke.credential.clientId}
          </code>
          <DialogActions
            onCancel={() => setPendingRevoke(null)}
            onConfirm={() => void revokeCredential()}
            confirmText={t('openPlatform.revoke', undefined, 'Revoke')}
            cancelText={t('common.cancel', undefined, 'Cancel')}
            destructive
          />
        </Dialog>
      ...
      {pendingRotate && (
        <Dialog
          title={t('openPlatform.rotateTitle', undefined, 'Rotate credential')}
          onClose={() => setPendingRotate(null)}
        >
          <p className="text-sm text-gray-600 dark:text-gray-300">
            {t(
              'openPlatform.rotateWarning',
              undefined,
              'A new secret is created now. The previous credential can issue tokens until its grace period ends; tokens already issued expire at their normal short TTL.',
            )}
          </p>
          <label className="mt-4 block text-sm font-medium text-gray-700 dark:text-gray-200">
            {t('openPlatform.graceMinutes', undefined, 'Grace period (minutes)')}
            <input
              type="number"
              min={5}
              max={10080}
              value={rotationGraceMinutes}
              onChange={(event) => setRotationGraceMinutes(Number(event.target.value))}
              className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 dark:border-gray-700 dark:bg-gray-950"
            />
          </label>
          <DialogActions
            ...
            confirmDisabled={rotationGraceMinutes < 5 || rotationGraceMinutes > 10080}
            confirmText={t('openPlatform.rotate', undefined, 'Rotate')}
            cancelText={t('common.cancel', undefined, 'Cancel')}
          />
        </Dialog>
      )}
      ...
      {editingScopes && (
        <Dialog
          title={t('openPlatform.editScopes', undefined, 'Edit scopes')}
          onClose={() => setEditingScopes(null)}
        >
          <p className="text-sm text-amber-700 dark:text-amber-300">
            {t(
              'openPlatform.scopeRevokeWarning',
              undefined,
              'Saving a new scope set immediately revokes existing access tokens for this installation.',
            )}
          </p>
          ...
          <DialogActions
            onCancel={() => setEditingScopes(null)}
            onConfirm={() => void updateScopes()}
            confirmDisabled={selectedScopes.size === 0}
      ...
      {pendingDisable && (
        <Dialog
          title={t('openPlatform.disableTitle', undefined, 'Disable access?')}
          onClose={() => setPendingDisable(null)}
        >
          <p className="text-sm text-gray-600 dark:text-gray-300">
            {t(
              'openPlatform.disableWarning',
              undefined,
              'This immediately revokes active tokens. The action cannot be undone from this screen.',
            )}
          </p>
          <code className="mt-3 block rounded bg-gray-100 p-2 text-xs dark:bg-gray-800">
            {pendingDisable.name}
          ...
          <DialogActions
            ...
            onConfirm={() => void disableTarget()}
            confirmText={t('openPlatform.disable', undefined, 'Disable')}
            cancelText={t('common.cancel', undefined, 'Cancel')}
            destructive
          />
        </Dialog>
...
function CredentialList({
  ...
  return (
    <div
      ...
      data-testid={`open-platform-credentials-${installationPid}`}
    >
      {credentials.length === 0 ? (
        <p className="text-xs text-gray-400">
          {t('openPlatform.noCredentials', undefined, 'No credentials')}
        </p>
      ) : (
        <div className="space-y-2">
          {credentials.map((credential) => (
            <div
              key={credential.pid}
              className="flex items-center justify-between rounded bg-gray-50 px-3 py-2 dark:bg-gray-950"
            >
              <div>
                ...
                <div className="mt-1 flex items-center gap-2">
                  ...
                  {credential.lastUsedAt && (
                    <span className="text-[11px] text-gray-400">
                      {t('openPlatform.lastUsed', undefined, 'Last used')}{' '}
              ...
              {credential.status === 'active' && (
                <div className="flex items-center gap-1">
                  ...
                  >
                    {t('openPlatform.rotate', undefined, 'Rotate')}
                  </button>
                  <button
                    type="button"
                    onClick={() => onRevoke(credential)}
                    className="rounded p-2 text-red-600 hover:bg-red-50"
                    aria-label={t('openPlatform.revoke', undefined, 'Revoke')}
                  >
                    <TrashIcon className="h-4 w-4" />
                  </button>
                </div>
...
function SecretRow({
  ...
  return (
    <div className="mt-4">
      ...
      <div className="mt-1 flex items-center gap-2 rounded-lg bg-gray-950 p-3 text-white">
    ...
    <div className="mt-6 flex justify-end gap-2">
      {!hideCancel && (
        <button
          type="button"
          onClick={onCancel}
          className="rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 dark:border-gray-700 dark:text-gray-200"
        >
          {cancelText}
        </button>
      )}
      <button
        ...
        className={`rounded-lg px-3 py-2 text-sm font-medium text-white disabled:opacity-40 ${destructive ? 'bg-red-600 hover:bg-red-700' : 'bg-indigo-600 hover:bg-indigo-700'}`}
      >
        {confirmText}
      </button>
    </div>
