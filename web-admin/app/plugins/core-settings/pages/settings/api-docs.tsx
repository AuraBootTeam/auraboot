...
export default function OpenPlatformPage() {
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-6 p-6" data-testid="open-platform-page">
          ...
          {applications.map((application) => (
            <article key={application.pid} className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
              <div className="flex flex-col gap-3 border-b border-gray-100 p-5 sm:flex-row sm:items-center sm:justify-between dark:border-gray-800">
                <div>
                  <div className="flex items-center gap-2"><h2 className="font-semibold text-gray-950 dark:text-white">{application.name}</h2><StatusBadge status={application.status} t={t} /></div>
                ...
                {application.installations.length === 0 ? <p className="text-sm text-gray-400">{t('openPlatform.noInstallations', undefined, 'No installations')}</p> : application.installations.map((installation) => (
                  <div key={installation.pid} className="rounded-lg border border-gray-200 p-4 dark:border-gray-700" data-testid={`open-platform-installation-${installation.pid}`}>
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div><div className="flex items-center gap-2"><span className="text-sm font-medium text-gray-900 dark:text-white">{t(`openPlatform.environment.${installation.environment}`, undefined, installation.environment)}</span><StatusBadge status={installation.status} t={t} /></div></div>
                      <div className="flex flex-wrap gap-2">
                        ...
                        {installation.status === 'active' && <button type="button" onClick={() => setPendingDisable({ kind: 'installation', pid: installation.pid, name: t(`openPlatform.environment.${installation.environment}`, undefined, installation.environment) })} className="rounded-md p-2 text-red-600 hover:bg-red-50" aria-label={t('openPlatform.disableInstallation', undefined, 'Disable installation')}><TrashIcon className="h-4 w-4" /></button>}
                      </div>
                    </div>
      ...
      {installingApp && <Dialog title={t('openPlatform.installTitle', undefined, 'Install application')} onClose={() => setInstallingApp(null)}><label className="block text-sm font-medium text-gray-700 dark:text-gray-200">{t('openPlatform.environmentLabel', undefined, 'Environment')}<select value={environment} onChange={(event) => setEnvironment(event.target.value as Installation['environment'])} className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 dark:border-gray-700 dark:bg-gray-950"><option value="development">{t('openPlatform.environment.development', undefined, 'Development')}</option><option value="staging">{t('openPlatform.environment.staging', undefined, 'Staging')}</option><option value="production">{t('openPlatform.environment.production', undefined, 'Production')}</option></select></label><label className="mt-4 block text-sm font-medium text-gray-700 dark:text-gray-200">{t('openPlatform.rateLimitPerMinute', undefined, 'Requests per minute')}<input type="number" min={1} max={100000} value={rateLimitPerMinute} onChange={(event) => setRateLimitPerMinute(Number(event.target.value))} className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 dark:border-gray-700 dark:bg-gray-950" data-testid="open-platform-rate-limit" /></label><fieldset className="mt-4"><legend className="text-sm font-medium text-gray-700 dark:text-gray-200">{t('openPlatform.scopes', undefined, 'Scopes')}</legend><p className="mb-2 text-xs text-gray-500">{t('openPlatform.minimumScopes', undefined, 'Grant only the capabilities this installation needs.')}</p><div className="space-y-2">{scopeCatalog.map((scope) => <label key={scope} className="flex cursor-pointer items-center gap-2 rounded-lg border border-gray-200 p-3 text-sm dark:border-gray-700"><input type="checkbox" checked={selectedScopes.has(scope)} onChange={() => setSelectedScopes((current) => { const next = new Set(current); next.has(scope) ? next.delete(scope) : next.add(scope); return next; })} /><code>{scope}</code></label>)}</div></fieldset><DialogActions onCancel={() => setInstallingApp(null)} onConfirm={() => void install()} confirmDisabled={selectedScopes.size === 0 || rateLimitPerMinute < 1 || rateLimitPerMinute > 100000} confirmText={t('openPlatform.install', undefined, 'Install')} cancelText={t('common.cancel', undefined, 'Cancel')} /></Dialog>}
...
function CredentialList({ installationPid, credentials, onRevoke, t }: { installationPid: string; credentials: Credential[]; onRevoke: (credential: Credential) => void; t: (key: string, params?: Record<string, any>, fallback?: string) => string }) {
  return <div className="mt-4 border-t border-gray-100 pt-3 dark:border-gray-800" data-testid={`open-platform-credentials-${installationPid}`}>{credentials.length === 0 ? <p className="text-xs text-gray-400">{t('openPlatform.noCredentials', undefined, 'No credentials')}</p> : <div className="space-y-2">{credentials.map((credential) => <div key={credential.pid} className="flex items-center justify-between rounded bg-gray-50 px-3 py-2 dark:bg-gray-950"><div><code className="text-xs text-gray-700 dark:text-gray-200">{credential.clientId}</code><div className="mt-1 flex items-center gap-2"><StatusBadge status={credential.status} t={t} />{credential.lastUsedAt && <span className="text-[11px] text-gray-400">{t('openPlatform.lastUsed', undefined, 'Last used')} {new Date(credential.lastUsedAt).toLocaleString()}</span>}</div></div>{credential.status === 'active' && <button type="button" onClick={() => onRevoke(credential)} className="rounded p-2 text-red-600 hover:bg-red-50" aria-label={t('openPlatform.revoke', undefined, 'Revoke')}><TrashIcon className="h-4 w-4" /></button>}</div>)}</div>}</div>;
}

function StatusBadge({ status, t }: { status: string; t: (key: string, params?: Record<string, any>, fallback?: string) => string }) {
  const active = status === 'active' || status === 'success';
  return <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-medium ${active ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300'}`}>{active && <CheckIcon className="h-3 w-3" />}{t(`openPlatform.status.${status}`, undefined, status)}</span>;
}
