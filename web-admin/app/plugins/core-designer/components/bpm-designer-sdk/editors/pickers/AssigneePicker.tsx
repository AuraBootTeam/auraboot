/**
 * B2b batch3 — AssigneePicker (remote-data picker)
 *
 * Ported byte-equivalent from
 * bpmn-designer/components/property-editors/AssigneePicker.tsx (267 LOC).
 *
 * Real remote endpoints (grep-verified, not mocked):
 *   - POST /tenant/members/search  → user options
 *   - GET  /roles/all              → role options (client-side filtered)
 *   - GET  /org/teams              → team/dept options (client-side filtered)
 *
 * The picker is a controlled component: (value: string[], onChange) — host
 * editor (UserTaskEditor) maps it into the AssigneeConfig.userIds/roleIds/
 * deptIds slice that matches the currently-selected assignee.type. JSON
 * round-trip is unchanged from the legacy editor.
 */
...
// API response shapes (real endpoints, verified via grep on legacy port)
...
// ---------------------------------------------------------------------------
// Data fetching helpers (exposed for tests via __assigneeInternals)
// ---------------------------------------------------------------------------
...
async function fetchRoles(_keyword: string): Promise<OptionItem[]> {
  ...
  return result.data
    .filter(
      (r) =>
        !_keyword ||
        r.name.toLowerCase().includes(_keyword.toLowerCase()) ||
        r.code.toLowerCase().includes(_keyword.toLowerCase()),
    )
...
export const __assigneeInternals = { fetchUsers, fetchRoles, fetchTeams, FETCHERS };
...
export function AssigneePicker({ type, value, onChange, placeholder }: AssigneePickerProps) {
  ...
  return (
    <div ref={containerRef} className="relative" data-testid="bpm-sdk-assignee-picker">
      {/* Selected tags */}
      {value.length > 0 && (
        <div className="mb-1.5 flex flex-wrap gap-1" data-testid="bpm-sdk-assignee-tags">
          {value.map((id) => {
      ...
      <button
        ...
        data-testid="bpm-sdk-assignee-open"
        ...
        <div className="absolute left-0 z-50 mt-1 w-full min-w-[220px] rounded-md border border-gray-200 bg-white shadow-lg">
          ...
          <div className="flex items-center gap-1.5 border-b border-gray-100 px-2 py-1.5">
            ...
            <input
              ...
              autoFocus
              data-testid="bpm-sdk-assignee-search"
            />
          </div>
          ...
          <div className="max-h-48 overflow-y-auto py-1">
            {loading && (
              <div
                ...
                data-testid="bpm-sdk-assignee-loading"
              >
            ...
            {!loading && options.length === 0 && (
              <div
                ...
                data-testid="bpm-sdk-assignee-empty"
              >
                {t('bpmn.common.noResults')}
              </div>
            )}
              ...
              options.map((item) => {
                ...
                return (
                  <button
                    ...
                    data-testid={`bpm-sdk-assignee-option-${item.id}`}
