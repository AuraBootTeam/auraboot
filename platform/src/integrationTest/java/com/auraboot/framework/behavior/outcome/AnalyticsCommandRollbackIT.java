...
class AnalyticsCommandRollbackIT {
    ...
    @Test void actualCommandRollsBackThenNormalCommandCommits() throws Exception {
        fault.fail(true);
        try {
            runApi(true);
            assertThat(fault.injectedCount()).isEqualTo(1);
        } finally {
            fault.fail(false);
        }
        runApi(false);
    ...
    private void runApi(boolean expectRollback) throws Exception {
        ...
        var process = new ProcessBuilder("pnpm", "exec", "playwright", "test", "-c", "playwright.noweb.config.ts",
                ...
                .redirectOutput(evidence.resolve(expectRollback ? "rollback-api.log" : "commit-api.log").toFile());
