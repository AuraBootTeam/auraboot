...
/**
 * Integration tests for AgentWorkflowBridge.
...
public class AgentWorkflowBridgeTest extends BaseIntegrationTest {

    @Autowired
    private AgentWorkflowBridge agentWorkflowBridge;
    ...
    void delegateToAgent_createsTaskWithBpmContext() {
        ...
        String taskPid = agentWorkflowBridge.delegateToAgent(
                tenantId,
                processInstanceId,
                activityId,
                bridgeAgentCode,
        ...
        assertTrue(inputData.contains("_workflow_process_id"),
                "input_data must contain _workflow_process_id key");
    ...
    void pollAgentTaskStatus_returnsNotFoundForInvalidPid() {
        ...
        Map<String, Object> result = agentWorkflowBridge.pollAgentTaskStatus(tenantId, nonExistentPid);
    ...
    @Test
    @Order(3)
    void startWorkflow_handlesMissingCapability() {
        Long tenantId = getTestTenant().getId();

        Map<String, Object> result = agentWorkflowBridge.startWorkflow(
                tenantId,
                "agent-run-" + testRunId,
    ...
    void pollWithBackoff_timesOutGracefully() {
        ...
        Map<String, Object> result = agentWorkflowBridge.pollWithBackoff(tenantId, nonExistentPid, 1);
    ...
    void variableMapping_usesBpmPrefix() {
        ...
        assertTrue(inputData.contains("\"_workflow_process_id\""),
                "Must contain _workflow_process_id");
        assertTrue(inputData.contains("\"_bpm_activity_id\""),
