...
class CapabilityViewServiceImplTest {
    ...
    @Test
    void applyingSelectionLeavesNonCapabilityCodesUntouched() {
        when(registry.listDeclarations(any())).thenReturn(List.of(
                decl("crm.cap.account", "客户管理", "crm.account.read", "crm.account.manage")));
        when(permissionService.findAllActive()).thenReturn(List.of(
                perm(1L, "p1", "crm.account.read"), perm(2L, "p2", "crm.account.manage"),
                perm(9L, "p9", "meta.command.execute")));
        // role already has exactly the account capability's codes + a platform code outside any capability
        when(permissionService.findRolePermissions(5L)).thenReturn(List.of(
                perm(1L, "p1", "crm.account.read"), perm(2L, "p2", "crm.account.manage"),
                perm(9L, "p9", "meta.command.execute")));

        MetaContext.setContext(1L, 1L, "p", "u");
        service.applyCapabilitySelection(5L, Set.of("crm.cap.account")); // account stays selected

        // nothing to grant (already has account codes) and nothing to revoke (meta.command.execute is
        // outside the capability universe, account codes are desired) -> no role mutation at all
