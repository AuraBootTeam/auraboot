...
class LoginCompletionHelperTest {
    ...
    void completeLogin_activeMember_returnsMemberStatus() {
        ...
        when(jwtUtil.generateTokenWithTenantId(any(), any(), any(), anyInt())).thenReturn("jwt-token-abc");
    ...
    void completeLogin_noTenant_returnsNoneStatus() {
        ...
        when(jwtUtil.generateTokenWithTenantId(any(), any(), isNull(), anyInt())).thenReturn("jwt-token-def");
        when(passwordManagementService.isPasswordExpired(user)).thenReturn(false);
    ...
    void completeLogin_pendingMember_returnsPendingStatus() {
        ...
        when(jwtUtil.generateTokenWithTenantId(any(), any(), any(), anyInt())).thenReturn("jwt-token-ghi");
        when(passwordManagementService.isPasswordExpired(user)).thenReturn(false);
    ...
    void completeLogin_mustChangePassword_flagsInResponse() {
        ...
        when(jwtUtil.generateTokenWithTenantId(any(), any(), any(), anyInt())).thenReturn("jwt");
    ...
    void completeLogin_sessionCreationFails_stillReturnsResponse() {
        ...
        when(jwtUtil.generateTokenWithTenantId(any(), any(), any(), anyInt())).thenReturn("jwt-fallback");
    ...
    void completeLogin_tenantServiceFails_returnsNoneStatus() {
        ...
        when(jwtUtil.generateTokenWithTenantId(any(), any(), any(), anyInt())).thenReturn("jwt-ok");
        when(passwordManagementService.isPasswordExpired(user)).thenReturn(false);
    ...
    void completeLogin_nullSecurityVersion_usesZero() {
        ...
        when(jwtUtil.generateTokenWithTenantId(any(), any(), any(), eq(0))).thenReturn("jwt-sv0");
        when(passwordManagementService.isPasswordExpired(user)).thenReturn(false);

        helper.completeLogin(user, null, null);

        verify(jwtUtil).generateTokenWithTenantId(any(), any(), any(), eq(0));
    }
    ...
    void completeLogin_withSecurityVersion_usesProvidedValue() {
        ...
        when(jwtUtil.generateTokenWithTenantId(any(), any(), any(), eq(5))).thenReturn("jwt-sv5");
        when(passwordManagementService.isPasswordExpired(user)).thenReturn(false);

        helper.completeLogin(user, null, null);

        verify(jwtUtil).generateTokenWithTenantId(any(), any(), any(), eq(5));
    }
