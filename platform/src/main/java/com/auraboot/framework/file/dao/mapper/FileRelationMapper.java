...
public interface FileRelationMapper extends BaseMapper<FileRelationEntity> {
     ...
     * 根据实体信息查询关联的文件ID列表
     */
    @Select("SELECT file_id FROM file_relations WHERE entity_type = #{entityType} AND entity_id = #{entityId} AND deleted_flag = false ORDER BY sort_order ASC")
    List<String> findFileIdsByEntity(@Param("entityType") String entityType, @Param("entityId") String entityId);

    /**
     * 根据实体和字段查询文件ID列表
     */
    @Select("SELECT file_id FROM file_relations WHERE entity_type = #{entityType} AND entity_id = #{entityId} AND field_name = #{fieldName} AND deleted_flag = false ORDER BY sort_order ASC")
