...
/**
 * Adapter that delegates DRL execution to the installed workflow capability under the unified Decision Runtime
 ...
 */
public class WorkflowDrlAdapter implements DecisionAdapter {

    private final WorkflowCapabilityRegistry workflowCapabilities;

    public WorkflowDrlAdapter(WorkflowCapabilityRegistry workflowCapabilities) {
    ...
    public DecisionValidateResult validate(ResolvedDecision decision) {
        ...
        Object rawErrors = workflowCapabilities.execute("rule.validate-drl",
                new WorkflowCapability.WorkflowRequest(null, null, Map.of("drl", drl)))
                .payload().get("errors");
        List<String> errors = rawErrors instanceof List<?> values
    ...
    public DecisionResult evaluate(ResolvedDecision decision, DecisionContext context, DecisionEvaluateOptions options) {
        ...
        Map<String, Object> outputs = workflowCapabilities.execute("rule.evaluate-inline",
                new WorkflowCapability.WorkflowRequest(null, null, Map.of(
                        "ruleCode", "drt:" + decision.decisionCode(),
                        "ruleContent", drl,
                        "version", decision.version() == null ? 0 : decision.version(),
                        "facts", facts))).payload();
