...
CREATE TABLE IF NOT EXISTS ab_role_permission (
    ...
    snapshot_id BIGINT,     -- owning publish snapshot (ab_permission_snapshot.id)
    status TEXT NOT NULL DEFAULT 'active',
    deleted_flag BOOLEAN NOT NULL DEFAULT FALSE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
-- Permission Governance S1: publish snapshot — every compile/publish writes a snapshot of the
-- materialized grant set for a role/package/global scope. Basis for rollback, diff, audit, cache
-- warm-up. Written only by the enterprise PermissionCompiler; read by OSS for rollback.
CREATE TABLE IF NOT EXISTS ab_permission_snapshot (
    ...
    scope_ref   BIGINT,                     -- role_id when scope=ROLE
...
CREATE TABLE IF NOT EXISTS ab_scheduled_task (
    ...
    enabled         BOOLEAN NOT NULL DEFAULT TRUE,
    scheduler_type  VARCHAR(16) NOT NULL DEFAULT 'local',
    external_job_id VARCHAR(64),
    external_executor_app VARCHAR(128),
    external_sync_status VARCHAR(32) NOT NULL DEFAULT 'active',
    external_sync_error TEXT,
    route_strategy  VARCHAR(32),
    block_strategy  VARCHAR(32),
    misfire_strategy VARCHAR(32),
    sharding_enabled BOOLEAN NOT NULL DEFAULT FALSE,
...
-- G-8: enforce per-tenant uniqueness on scheduled task name. The handler
-- pre-check (AutoSetPhase) returns a friendly business error first; this
-- index is the last-line defence against races / direct DB writes.
CREATE UNIQUE INDEX IF NOT EXISTS idx_scheduled_task_name_tenant
    ON ab_scheduled_task(tenant_id, name);
...
-- Half A (global pins) lives in ab_saved_view.view_config.meta.pinnedAsQuickFilter;
-- Half B/M3 (a user/team pinning a shared view) cannot mutate that view's meta, so
-- the pin is a per-(principal, view) association here. Mirror of migration
-- V20260715000000__saved_view_chip_pin.sql.
CREATE TABLE IF NOT EXISTS ab_saved_view_chip_pin (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    pid VARCHAR(26) UNIQUE NOT NULL,
    tenant_id BIGINT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_chip_pin_lookup ON ab_saved_view_chip_pin(tenant_id, model_code, page_key);
...
-- Permission v2 capability registry: plugin-shipped business-language permission bundles.
-- Distinct from ab_capability (agent/MCP capability registry).
CREATE TABLE IF NOT EXISTS ab_permission_capability (
    ...
    name_en             VARCHAR(255),
    description         TEXT,
    include_codes       TEXT NOT NULL,                  -- comma-separated permission codes (not 'includes': jsqlparser keyword)
...
COMMENT ON TABLE ab_permission_capability IS 'Permission v2 capability registry (plugin-shipped business-language permission bundles)';
...
CREATE TABLE IF NOT EXISTS ab_kb_document (
    ...
    CONSTRAINT chk_doc_type CHECK (doc_type IN ('pdf', 'docx', 'md', 'txt', 'csv', 'html', 'pptx', 'xlsx', 'image', 'ppt', 'xls')),
...
-- =====================================================================
-- Table: ab_admin_event_log (env-layering backlog #3 + USP §5 followup)
-- Description: Cross-cutting administrative-action audit log. Domain-agnostic
--              ledger for ops-relevant administrative actions (env lock/unlock,
--              promotion apply, plugin import / disable, admin user disable,
--              etc.) so compliance / oncall can answer "who did what to which
--              resource and when".
--              Distinct from:
--                - ab_permission_audit_log: only DENY decisions on permission
--                  evaluation hot path (high volume, narrow purpose)
--                - ab_command_audit_log:    runtime command pipeline events
--                - ab_query_audit_log:      runtime data-query observability
--              Writes are fire-and-forget; service swallows exceptions and
--              logs to slf4j so audit-write failures never break the action.
-- =====================================================================
CREATE TABLE IF NOT EXISTS ab_admin_event_log (
    id              BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    pid             VARCHAR(26) UNIQUE NOT NULL DEFAULT '',
    tenant_id       BIGINT          NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_ab_admin_event_log_resource
...
COMMENT ON COLUMN ab_admin_event_log.action_type IS 'Domain.action style: environment.lock / environment.unlock / promotion.apply / plugin.import …';
COMMENT ON COLUMN ab_admin_event_log.resource_type IS 'environment | promotion | plugin | … (free text; mirrors action_type domain)';
...
-- ============================================================
-- User Soul Profile (PR-75 Phase 1 schema, PR-77 Phase 3 reader)
-- Per-user derived LLM-rendered profile. One ACTIVE row per (tenant,user).
-- Older versions remain as SUPERSEDED for audit. See
-- docs/plans/2026-04/2026-04-19-user-soul-profile-design.md
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_user_soul_profile_admin_action_tenant_time
    ON ab_agent_user_soul_profile_admin_action (tenant_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_user_soul_profile_admin_action_target
...
-- =====================================================================
-- SaaS Kernel Phase 5: Billing Account Identity Spine (G1)
-- Data Domain: G1 (Identity / Control Plane) — no tenant_id
-- System ① commercialization tables superseded by System ② billing engine.
-- Spine lives in OSS so ab_tenant + quota tables can FK it without
-- cross-repo dependency.
-- =====================================================================

-- System ① commercialization tables removed (superseded by ab_billing_account + System ②)
DROP TABLE IF EXISTS ab_platform_license;
DROP TABLE IF EXISTS ab_platform_account;

-- ============ Billing Account (identity spine, OSS core) ============
CREATE TABLE IF NOT EXISTS ab_billing_account (
    id            BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    pid           VARCHAR(26) UNIQUE NOT NULL DEFAULT '',
...
-- ab_tenant: drop orphaned System ① column, add billing_account_id spine FK
ALTER TABLE ab_tenant DROP COLUMN IF EXISTS platform_account_id;
ALTER TABLE ab_tenant ADD COLUMN IF NOT EXISTS billing_account_id BIGINT
    REFERENCES ab_billing_account(id);
CREATE INDEX IF NOT EXISTS idx_tenant_billing_account ON ab_tenant(billing_account_id);
...
CREATE TABLE IF NOT EXISTS ab_im_conversation (
    id              BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    pid             VARCHAR(26)     NOT NULL DEFAULT SUBSTRING(REPLACE(gen_random_uuid()::text, '-', ''), 1, 26),  -- public record id; DEFAULT covers insert paths that predate it (V20260714000000)
    tenant_id       BIGINT          NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_ab_im_conversation_pid ON ab_im_conversation(pid);
...
-- =====================================================================
-- SECTION: Decision Runtime (ab_drt_*)
-- =====================================================================
-- NOTE: These tables use the ab_drt_ prefix to avoid collision with the
-- existing ab_decision_definition / ab_decision_record tables belonging
-- to the meta-adjudication concept (framework.meta...DecisionController
-- @ /api/meta/decisions).
...
CREATE TABLE IF NOT EXISTS ab_drt_version (
    ...
    runtime_adapter     VARCHAR(50) NOT NULL,
...
CREATE TABLE IF NOT EXISTS ab_drt_log (
    ...
    rollout_policy_pid  VARCHAR(26),
    rollout_bucket      INTEGER,
    rollout_arm         VARCHAR(20),
    routing_key         VARCHAR(200),
    rollout_result_key  VARCHAR(200),
    kind                VARCHAR(50),
...
COMMENT ON TABLE ab_drt_rollout_policy IS 'Decision Runtime — progressive rollout policy between baseline and candidate versions';
...
COMMENT ON TABLE ab_drt_rollout_metric_bucket IS 'Decision Runtime — pre-aggregated rollout metrics for long-window version comparisons';
...
COMMENT ON COLUMN ab_drt_usage_ref.target_type IS 'DECISION | CONDITION_FRAGMENT | FIELD | FUNCTION | CONNECTOR | WEBHOOK | POLICY';
...
-- SECTION: Event Policy (ab_drt_policy_*)
-- Enum values copied verbatim from Java enums; see migration 2026-06-08-event-policy-foundation.sql.
...
-- EventPolicy action execution log — restart-durable idempotency for the PolicyExecutor (docs/2.md §8.2)
...
CREATE INDEX IF NOT EXISTS idx_drt_outbox_pending
    ON ab_drt_outbox (tenant_id, status, created_at) WHERE status = 'PENDING';
COMMENT ON TABLE ab_drt_outbox IS 'EventPolicy transactional outbox; PENDING events processed after commit (docs/2.md §9)';

-- EventPolicy WRITE_AUDIT action trail (docs/2.md §7): user-configured business audit side effect
...
-- =====================================================================
-- Billing & Quota Foundation (M1: resource catalog / quota / metering)
-- =====================================================================
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 1a. Resource Catalog seed — 13 standard platform resources
--     Idempotent: ON CONFLICT (resource_code) DO NOTHING
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 4. Quota Reservation (pre-authorization)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 5. Quota Reservation Line (per-bucket breakdown)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 6. Quota Ledger (immutable audit trail)
-- ─────────────────────────────────────────────────────────────────────────────
